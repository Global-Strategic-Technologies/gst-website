...
| script-src                | 'self' 'unsafe-inline' googletagmanager.com va.vercel-scripts.com cdn.vercel-insights.com challenges.cloudflare.com                                                                                                  | GA4, Vercel Speed Insights (both hosts); inline needed for theme/palette init; `challenges.cloudflare.com` is Cloudflare Turnstile's `api.js`, loaded lazily by `/hub/mcp/trial/` on the first click (BL-155)                                                                                                                                                                                                                                                                                                                                                                  |
| connect-src               | 'self' googletagmanager.com google-analytics.com analytics.google.com www.google.com \*.ingest.sentry.io \*.ingest.us.sentry.io vitals.vercel-insights.com mcp.globalstrategic.tech mcp-staging.globalstrategic.tech | GA4 beacons (primary + alternate), Sentry error reports (US region), Vercel vitals; the two MCP Worker hosts are the trial mint `POST /trial/signup` (BL-155) — staging is in the ONE production CSP deliberately, because preview deploys mint against staging and the widening admits only GST's own host. **Google Signals is disabled at the GA4 property level (2026-05-30)** — do NOT add `stats.g.doubleclick.net` to this list. Re-enabling Signals in GA4 admin would require adding it, but the B2B/GDPR posture is to keep Signals off; the omission is intentional |
...
| frame-src                 | 'self' challenges.cloudflare.com                                                                                                                                                                                     | Brand responsive demo iframe, and the hidden iframe Cloudflare Turnstile uses even in Invisible mode (`/hub/mcp/trial/`, BL-155). NOTE: `'self'` permits the _parent_ to load a frame; the framed page must also permit being framed via its own `frame-ancestors` — see [Route Exceptions](#route-exceptions)                                                                                                                                                                                                                                                                 |
