...
>
> ✅ **§ Website marketing surface shipped 2026-08-09 — third re-engage trigger fired** (operator go-decision on building the front door ahead of demand; not a slice pick against the standing instruction above). Delivered: `/hub/mcp/`, the `services.astro` offer section + `/hub/` cross-link, and the radar human-in-the-loop caveat — which had existed nowhere as shipped copy, only as the risk line in [BL-033 § Risks & mitigations](#bl-033-mcp-server--external-pilot-phase-3). Per-AC dispositions below.
>
...
- [x] Page meets the existing hub-page bar: design-system tokens only, works in light/dark themes and all 6 palettes, desktop-first responsive; E2E coverage per [`TEST_STRATEGY.md`](../testing/TEST_STRATEGY.md); if any existing copy strings change, the Directive-11 `grep tests/` check applies — ✅ built on the section-index archetype (`/hub/tools/index.astro`) over global card families — `.brutal-trust-card` (this is its first production consumer; it was specimen-only), `.brutal-gateway-card`, `.brutal-stat-tile`, `.brutal-callout--warning`. Tokens only, stylelint clean. `tests/e2e/hub-mcp-page.test.ts` covers page shape, both cross-links, and the CTA prefill; the route is added to `tests/e2e/accessibility.test.ts` (22 → 23 routes). **One deviation worth recording**: the tier grid needs a page-scoped `grid-template-columns` override — the shared `.brutal-gateway-grid` is `repeat(auto-fill, minmax(420px, 1fr))` at a 3rem gap, so three tracks need ≈1452px of viewport and the row would have broken to 2 + 1 at 1280/1366/1440. That scoped rule is also what produces the mobile stack, since it out-specifies the global single-column rule in `cards.css`
- [x] Copy includes the human-in-the-loop caveat for radar content (per BL-033 § Risks & mitigations: radar output should not be auto-actioned by client agents) — ✅ **first appearance anywhere as shipped copy** — it had lived only as a BACKLOG risk line; it is in neither the radar tool descriptions nor the radar CONTRACT/USAGE docs nor `/hub/radar`. Rendered in a `.brutal-callout--warning` and asserted by both the E2E suite and the parity test so a later copy edit cannot silently drop it

**Request-access front door + provisioning automation**

- [ ] Request-access form/CTA (name, firm, use case, email) delivering to the operator — explicitly NOT self-serve credential issuance and NOT a user directory (preserves ADR-0008's pre-registration / no-DCR stance) — **still open.** The 2026-08-09 marketing page ships a prefilled `mailto:` CTA (subject + name/firm/use-case/volume prompts, one per tier plus a page-level one) which delivers into the same operator inbox [`PILOT_ONBOARDING.md`](../../../mcp-server/src/docs/operations/PILOT_ONBOARDING.md) § 0 already describes. That is a doorbell, not the form: no structured intake, no validation, no delivery confirmation. Deliberate — a real form needs an external endpoint on the `connect-src`/`form-action` allowlist in BOTH `vercel.json` and `src/middleware.ts`, plus the `privacy.astro` disclosure and BL-004 coordination below, none of which this scope reopened. § 0's "intake half pending" note therefore stays accurate
