...
> - **2026-08-22**: 1 stanza (BL-137, workers-types global shadowing) closed and pruned the same day it shipped. Last pre-prune revision: `677862fc`. Its live content — the accepted test-side residual, and the fact that a project-referenced tsconfig split was never shown to be impossible — went to [ADR-0020](../adr/0020-workers-types-global-shadowing-immunity.md) rather than staying here. The BL-136 stanza (since pruned, see the 2026-09-15 wave) linked to its anchor and was retargeted at the ADR in the same commit.
...
>   - Every per-page `.bullet-icon` / `.delta-accent` rule is deleted, not revived: the shared icon base in `cards.css` stays in charge, and the 0.7/0.8/0.5 opacities never ship.
>   - The three orphan `.arch-diligence-list` rules are deleted: no markup uses them.
>
>   **Neither item's acceptance criteria anticipated that the guards would find more:**
>   - Three more undefined tokens. `--text-light-tertiary` becomes `--text-secondary` on Diligence Machine's small italic step note, since `--text-muted` is large-text-only. `--text-md` becomes `--text-base` on ICG's comparison row, with no visible change. `--bg-muted` is removed with its whole `background` declaration on the `/brand` drawer specimen. An undefined token inside `light-dark()` invalidates the entire declaration, so the specimen had been transparent in **both** themes, not just light. A first fix to `light-dark(transparent, var(--bg-dark-secondary))` quietly gave dark theme a backdrop it never had; code review caught it.
>   - A tenth foreign-element rule on TechPar: the Trajectory tab's empty-state DeltaIcon shipped at 14px in full colour beside the Analysis tab's 64px muted native svg. Now anchored via `.tp-empty :global(.tp-empty__sym)`.
>
>   **The guards are the deliverable:** `tests/unit/undefined-css-custom-properties.test.ts` and `tests/unit/scoped-selector-foreign-element.test.ts`, each mutation-checked. The foreign-element one is per usage, not per file, with a type-selector escape rather than an allowlist.
>
>   **Two instrument failures are worth recording, because both would have produced confident wrong answers:**
>   - The foreign-element scan first missed HubMcpPage entirely. Its self-closing `<script … />` opened a regex match that swallowed the whole template down to the next `</script>`, and that is also what had hidden the TechPar finding. A fixture now pins it.
>   - The What We Do wrap sweep first reported a wrap at every width, because it counted padding and border as lines.
>
>   BL-150's own "a working detector already exists" was also wrong: none was in the repo.
>
>   Live content went to [STYLES_GUIDE.md](../styles/STYLES_GUIDE.md) (§ Core Tokens and § The scoped-rule / foreign-element trap), the `DeltaIcon.astro` header, and the re-measured `WhatWeDo.astro` wrap comment.
>
...
**Source**: operator directive 2026-08-15 — build the payment rail as a reusable capability, first consumer being self-serve MCP client purchase + provisioning | **Effort**: ~2–3 weeks engineering across the slices below, plus vendor/tax lead time | **Status**: Open — designed, implementation deferred 2026-09-01; self-serve trial directive parked 2026-09-06 (see below) | **Architecture & plan**: [PAYMENTS_PLATFORM_BL-133.md](PAYMENTS_PLATFORM_BL-133.md) | **Reverses**: [BL-093](#bl-093-mcp-server--commercialization-phase-4) § Out of scope, which lists "public checkout / webhook-driven tier automation" as deferred pending a volume trigger — this item is the operator go-decision that supersedes it
...
>
> - **GST-side trial, no card** — a `trial`-tier client with a hard expiry, no Stripe involvement. Preserves the expire-don't-charge shape and is independent of [BL-145](#bl-145-design-partner-program--set-the-price-from-evidence-not-from-a-guess)'s unset price, but the identity gate becomes email verification, which needs the still-undecided [BL-004](#bl-004-email-capture-system) vendor, and it is the weakest abuse control.
> - **Stripe-native `trial_period_days`** on the subscription Payment Link — nearly zero new machinery, riding the webhook rail already designed. But it auto-converts, reversing the operator's stated preference, and needs BL-145's price before a Payment Link can exist.
> - **Stripe direct with setup-mode Checkout** — restores API access and a card-on-file gate, but reverses the Managed Payments decision and hands GST the VAT/sales-tax registration and remittance obligation that decision was taken to avoid.
>
> **This costing is unexercised inference, not established fact.** "Payment Links only, no Stripe API key" is corroborated by the design doc's § Why Managed Payments removes most of the build — but that same document flags every vendor figure it carries as _"read from Stripe's docs, not exercised"_, and the claim that `trial_period_days` auto-converts was **not verified against Stripe's documentation** when this was written. Treat all three as **expected, unverified — confirm in the same staging pass**.
>
> **What unparks it**: that staging pass. The design doc's § _Vendor behaviour: documented, not executed_ is already named as the first task for whoever picks BL-133 up; the entire lifecycle rests on Stripe behaviour inferred from docs. Designing a trial on top of it stacks a second layer of inference on the first.
>
> **Hazard this parked state creates, recorded so it is not found late.** A trial is self-serve credential issuance **without a payment**, so it does _not_ ride the ADR-0008 amendment the design doc already schedules for the post-payment case ("the operator's checkout is the registration authority, so it is not DCR"). Whoever unparks this must widen that amendment, and must revisit three places whose "no self-serve signup" wording is true **today** and would not be then: [BL-093](#bl-093-mcp-server--commercialization-phase-4) § Out of scope (the self-serve-signup bullet, and the closing sentence of the public-checkout bullet), [`src/pages/hub/mcp/get-started/index.astro`](../../pages/hub/mcp/get-started/index.astro) ("there is no self-serve signup and no dynamic client registration"), and [`src/data/mcp/capabilities.ts`](../../data/mcp/capabilities.ts) ("no dynamic client registration and no user directory"). The last two are public-facing copy, and a change there is a Directive-11 `grep tests/` event.

#### Acceptance Criteria

**Slice 1 — Vendor selection (decision, ships as an ADR)**

> **Decided 2026-09-01 — the ACs below are the brief that produced it, not open work.** The operator selected **Stripe with Managed Payments** (Stripe/Link is merchant of record, so GST carries no VAT/sales-tax registration or remittance obligation, at 3.5% on top of processing). The matrix, the rationale and the switch trigger are recorded in [`PAYMENTS_PLATFORM_BL-133.md`](PAYMENTS_PLATFORM_BL-133.md) § Decisions taken and § Slice 1. What remains unchecked here is only writing that decision up as an ADR. Read the boxes below as the criteria it was judged against.
...
**Slice 3 / 3a — Signup page and its UX design** (Tier A: `en`, `es`, `pt-BR`) — ✅ **built 2026-09-07** (plan-reviewed REVISE → REVISE → APPROVE); page at [`/hub/mcp/trial/`](../../page-templates/HubMcpTrialPage.astro), one page serving both flows (the "From code" presentation is BL-156's UX; since 2026-09-14 its issued state links to [`/hub/mcp/from-code/`](../../page-templates/HubMcpFromCodePage.astro) and the inline steps are a summary)
...
### BL-152: Marketing Blitz — promote the released GST MCP across online channels

**Source**: operator directive 2026-09-04 — the GST MCP server is officially released (the `/hub/mcp/` family plus the three onboarding guides and their six recorded clips shipped 2026-08-27 → 09-04); promote it across LinkedIn, the Claude connector directory, and Google Ads, with Google Analytics verified first so lift is measurable. Google has offered spend incentives, which puts a clock on the Ads slice | **Effort**: ~1 day engineering (measurement + privacy copy), then ~2–3 weeks of operator time in parallel with [BL-145](#bl-145-design-partner-program--set-the-price-from-evidence-not-from-a-guess) | **Status**: Open — **engineering shipped 2026-09-14** (Slice 0 + the Slice 2 engineering, one PR); the operator slices are what remain

**As the** GST operator with a released product and zero inbound, **I want** a coordinated push across the channels where PE/M&A buyers and agent-curious technologists already are, **so that** the website, the brand, the MCP server, and the advisory services get traffic that can be attributed to a channel — turning "distribution is the constraint" ([BL-145 § Technical Context](#bl-145-design-partner-program--set-the-price-from-evidence-not-from-a-guess)) from a diagnosis into a program.

> **Three things this item must not do.** (1) It must not spend on traffic before the site can attribute it — Slice 0 is the gate for every other slice, and it is the only slice with a build in it. (2) It must not silently bypass the **hard gate** on the Claude directory listing that [BL-093 § MCP directory listing](#bl-093-mcp-server--commercialization-phase-4) already carries (independent pen test passed, findings remediated, plus a promotion trigger). The 2026-09-04 "it is released" directive is a candidate **operator go-decision**, which is one of BL-093's named triggers — but the pen-test gate is not a trigger, and the operator has to say explicitly whether it is met, waived, or still binding before Slice 2 submits anything. (3) It must not lead with prices or rate-limit ceilings in any channel — the same ADR-0010 constraint BL-145 Slice 2 and BL-133 Slice 4 carry binds outbound copy before it binds any page.

> **Decisions taken 2026-09-14** (operator, at planning): **scope** — Slice 0 and the Slice 2 engineering ship together in one PR; **consent** — paid targeting is geo-limited to the US and the organic-EU exposure is accepted, so BL-001 is NOT pulled into this item; **BL-093 hard gate** — "still binding; build the Slice 2 engineering anyway", so everything below marked shipped is built, and the directory **submission** stays blocked until the pen test is met or explicitly waived here.

#### Acceptance Criteria

**Slice 0 — Measurement before traffic (engineering, ~1 day, blocks all other slices)**

- [x] **Every `/hub/mcp/*` page reports engagement.** Shipped 2026-09-14: `src/utils/mcp-analytics.ts`, six `mcp_` events (incl. `mcp_trial_signup`), wired through `initCopyButtons`'s new `onCopy` hook, the clip player, and the landing/trial scripts; convention registered in `tests/unit/tool-analytics.test.ts`, documented in [GOOGLE_ANALYTICS.md § 9](../analytics/GOOGLE_ANALYTICS.md). Before: only the landing page fired anything (`trackCTA` on its guide, tier, and request-access links — `src/pages/hub/mcp/index.astro`); `get-started/`, `using/`, `advanced-operations/`, and `docs/` fire **no events at all**, so a LinkedIn reader who lands on the guide and copies the endpoint is invisible. Add a hub-tool-style prefix (`mcp_`, per [GOOGLE_ANALYTICS.md § 9 Hub Tool Events](../analytics/GOOGLE_ANALYTICS.md)) with at minimum: guide viewed, endpoint copied, clip played, request-access clicked, and register it in `tests/unit/tool-analytics.test.ts` so the convention is enforced
- [x] **A conversion set is declared in GA4 as key events, and written down here.** **Applied to the property 2026-09-17** and each one verified in DebugView from a real click on production: `mcp_request_access`, `mcp_endpoint_copied`, `mcp_trial_signup`, `mcp_guide_complete`. **One gap, not blocking**: `booking_confirmed` has fired once ever, so GA4 has not processed it into Admin → Events and it cannot be starred yet — star it when it appears, no code change needed. `cta_click` / `scroll` / `dm_generate` confirmed unmarked. Three GA4 defaults (`close_convert_lead`, `purchase`, `qualify_lead`) are marked and left alone: the site never sends them and all three report "No stream data detected". `mcp_clip_play` is not in the conversion set and its DebugView check was inconclusive on the final pass (the tab stayed backgrounded so no clip loaded); it was seen earlier the same day with `clip=add-connector-claudeai`. Written down 2026-09-14 in [GOOGLE_ANALYTICS.md § Key events](../analytics/GOOGLE_ANALYTICS.md): `mcp_request_access`, `mcp_trial_signup`, `booking_confirmed`, `mcp_endpoint_copied`, `mcp_guide_complete`. (The step this bullet carried as open — mark them in GA4 Admin and verify each in DebugView, per [ANALYTICS_TESTING.md § DebugView checklist](../analytics/ANALYTICS_TESTING.md) — is what was done on 2026-09-17.) Originally proposed: `mcp_request_access` (the mailto — the one action that produces a lead), `booking_confirmed` (already tracked, advisory funnel), `mcp_endpoint_copied` (install intent), and a guide-depth event. If a conversion cannot be seen in GA4 DebugView from a real click before the blitz starts, it is not a conversion
- [x] **A UTM convention exists and is used by every link this item produces** — recorded 2026-09-14 in [GOOGLE_ANALYTICS.md § 10 Campaign attribution](../analytics/GOOGLE_ANALYTICS.md): `utm_source` ∈ {linkedin, google-ads, hn, devto, cloudflare-community, newsletter}, `utm_medium` ∈ {post, article, video, cpc, listing, email}, `utm_campaign=mcp-launch-2026-09`, optional `utm_content` (`claude-directory` dropped from the source list: that channel passes no query string and is attributed by consent telemetry, see Slice 2). The next campaign inherits it instead of inventing one. Vercel Analytics, if enabled, is a cross-check, not the system of record
- [x] **The two trial timing params are registered as GA4 custom metrics** (BL-164). **Done 2026-09-17**: "Trial signup duration" (`duration_ms`) and "Trial mint duration" (`mint_ms`), event scope, unit Milliseconds — verified first by a real production signup showing both params in DebugView. `locale` needed nothing: it was **already registered as a custom dimension on 2026-09-06**, so the "outstanding since BL-153" note this item originally carried was wrong and is corrected in [GOOGLE_ANALYTICS.md](../analytics/GOOGLE_ANALYTICS.md). Registration is not retroactive, so the reportable sample starts from the next signup
- **(Standing caveat, not a task — nothing to tick)** **Abandonment stays invisible, and the conversion count understates demand because of it** (carried forward from the closed BL-164 stanza). `mcp_trial_signup` counts signups that settle and `mcp_trial_refused` counts refusals the Worker issued; a visitor who starts the flow and closes the tab mid-verification is indistinguishable in the funnel from one who never clicked. So `mcp_trial_signup` is a **lower bound** on trial demand, by however many people gave up — and the BL-164 measurement makes that concrete rather than theoretical, since ~12 s of a 15 s signup is spent in the Turnstile solve. Read campaign lift with that in mind; closing the gap would need a start-of-flow event, which is deliberately not scoped here. **It has a mirror image discovered 2026-09-17**: a visitor whose trial is still live and who signs up again is _re-issued_ rather than refused, and the event fires again — so the same number also **over**-counts returns. Segment by `outcome` (`issued` vs `reissued`) whenever reading it as a lead count
- [x] **A two-week pre-blitz baseline is captured** (sessions, `/hub/mcp/*` views, the conversion set) so "lift" has a denominator. **Pulled 2026-09-18 00:40 UTC for Sep 4–17 inclusive**: sessions **80**; `/hub/mcp/*` views **36** (`/hub/mcp/` 20, `get-started/` 7, `trial/` 7, `docs/` 1, `using/` 1); `mcp_request_access` **1**, `mcp_trial_signup` **2**, `booking_confirmed` **0**, `mcp_endpoint_copied` **4**, `mcp_guide_complete` **2**. Context: `mcp_guide_view` 19, `mcp_clip_play` 1, 518 total events, 153 page views. **Read these as raw event counts, not GA4's "key events" metric** (104 for the window) — the five were not yet marked for most of it, so the metric and the counts legitimately disagree. **And nearly every one of the five is operator test traffic from 2026-09-17**, so the honest pre-blitz lead volume is effectively **zero** — which is the denominator, recorded as a number exactly as this item demanded
- [ ] **Search Console has the sitemap index submitted** and the five `/hub/mcp/` pages are indexed with their `SoftwareApplication` / `TechArticle` JSON-LD reporting no errors ([SEO_IMPLEMENTATION.md § Sitemap & Robots](../seo/SEO_IMPLEMENTATION.md), [JSON_LD_SCHEMA.md § MCP Server Schemas](../seo/JSON_LD_SCHEMA.md)). Paid traffic to unindexed pages wastes the organic half of the spend
- [x] **Consent decision made explicitly** (2026-09-14): paid targeting is **US-only**; organic-EU exposure accepted; BL-001 unchanged and NOT scoped in. Recorded in [GOOGLE_ANALYTICS.md § Privacy](../analytics/GOOGLE_ANALYTICS.md). The privacy policy now names Google Analytics 4 explicitly. Context: GA4 loads unconditionally today and [BL-001](#bl-001-cookie-consent-and-gdpr-compliance) is open. A campaign that deliberately drives EU visitors changes BL-001 from "should" to "must" — either scope BL-001 Consent-Mode gating into this slice, or record the decision to geo-limit paid targeting to the US and accept the organic-EU exposure. Do not leave it implicit

**Slice 1 — LinkedIn (operator, ongoing)**

- [ ] **GST company page and the operator's personal profile both post**, on a stated cadence (a launch post, then ≥1/week for the campaign window). Personal-profile reach is the asset; the company page is the landing surface. Every link carries the Slice 0 UTMs
- [ ] **A launch series, not a single announcement**: (a) the launch post with the `connector-enabled` clip as native video (LinkedIn de-ranks outbound links in the post body — put the link in the first comment or the article, and measure whether that matters); (b) one post per job-to-be-done — the IRL round trip, TechPar, tech-debt cost, the radar — each showing a real tool call from Claude, with the output; (c) a **LinkedIn article** (long-form) on why an advisory firm ships an MCP server, written for the deal-team buyer not the developer — use the `gst-page-content` skill's audience and voice rules; (d) a "how to connect it in two minutes" post that is literally the get-started guide compressed
- [ ] **Media reuse, not re-recording**: the six clips and the consent still in `public/images/hub/mcp/` already exist ([MCP_ONBOARDING.md](../hub/MCP_ONBOARDING.md)); the LinkedIn cuts are re-encodes of those sources, and their constraints (poster-as-reduced-motion, do-not-re-add WebM) do not apply off-site
- [ ] Each post's CTA points at exactly one destination — the guide, the landing page, or a booking — and that destination is a page Slice 0 can measure
- [ ] **Falsifiable target set before the first post** (impressions are vanity; the target is profile→site clicks and `mcp_request_access` / bookings attributed to `utm_source=linkedin`)

**Slice 2 — Claude connector directory (operator + small engineering, gated)**

Verified 2026-09-04 against Anthropic's own docs: remote MCP servers are submitted through the **submission portal inside claude.ai organization settings, which requires a Team or Enterprise organization**; review is human; accepted connectors appear in the directory any Claude user can enable without a sales conversation; and once listed, tool changes need no resubmission. The two review requirements that decide most submissions are **every tool annotated** (`readOnlyHint` / `destructiveHint`) and a **public privacy policy**; reviewers also want a **test account with realistic data** so the connector can be exercised in ten minutes. ([Submitting to the Connectors Directory](https://claude.com/docs/connectors/building/submission), [Anthropic MCP directory policy](https://support.anthropic.com/en/articles/11697096-anthropic-mcp-directory-policy), [Connectors directory FAQ](https://support.anthropic.com/en/articles/11596036-anthropic-connectors-directory-faq).) Today the server is reachable in Claude only as a **custom connector** pasted by URL — which is what the get-started guide teaches — so the directory is a discoverability upgrade, not a capability the server lacks.

- [ ] **The BL-093 hard gate is resolved in writing before submission** — pen test met, or the operator's explicit waiver quoted here. This AC is the whole reason the slice is separate
- [ ] **Org tier confirmed**: does GST's claude.ai organization meet the Team/Enterprise requirement? If not, that is a cost line for this item, not a blocker to route around
- [x] **Annotation audit** — shipped 2026-09-14: all 16 remote + 2 stdio-only tools carry all four hints; `openWorldHint: true` only on the two Inoreader-backed radar tools; enforced by the `tools/list` loop in `mcp-server/tests/integration/protocol-roundtrip.test.ts`, policy in [tools/README.md](../../../mcp-server/src/docs/tools/README.md). Was: every registered tool carries a correct `readOnlyHint` / `destructiveHint` (measured 2026-09-05: two registered tools carry **no** `annotations` block at all — `compose-dossier-envelope.ts` and `validate-irl-provenance.ts` — so this is a fix, not a precaution; verify the rest per tool, including the xlsx `fill` tool, whose write is to the response not to state). Add a contract-parity test so a future tool cannot register without one
- [x] **Privacy policy covers the MCP server** — shipped 2026-09-14: a new "The MCP Server" section in `src/i18n/*/privacy.json` (OAuth grants in KV, Upstash counters/caches, Analytics Engine telemetry with its three-month retention, Sentry, Cloudflare hosting); `thirdParty.item2` names Google Analytics 4; es/pt-BR translated and stamped. Was: `src/pages/privacy.astro` did not mention the MCP server, the OAuth consent scopes, Upstash, Cloudflare, or Sentry as processors of connector traffic. A missing or incomplete policy is an immediate rejection; fix this before the form, not after
- [x] **The reviewer's path is a real path** — decided 2026-09-14: an unprovisioned directory user gets the **existing self-serve three-day trial** (BL-155); the consent page links to `/hub/mcp/trial/` on every render (first view and the 401 re-render share one template), so nothing new is built. **Operator step still open**: provision the reviewer's test credential as a non-production client with `--allow-radar` off. Context: access is operator-gated (pre-registered clients, `MCP_KEY_*` consent flow — [ADR-0008](../adr/0008-mcp-oauth-embedded-authorization-server.md)); a reviewer and, after listing, any Claude user who toggles it on must land somewhere sensible. Decide what an unprovisioned directory user gets (the free-pilot request path? a read-only tool subset? a clear "request access" error?) and make the reviewer's test credential a **non-production** client with `--allow-radar` **off**
- [ ] **Listing copy and screenshots** reuse the landing page's positioning and the onboarding stills; the description is the `SoftwareApplication` JSON-LD's description, not a fresh one, so the two cannot drift
- [x] Publish the `/.well-known/mcp` + `server.json` registry metadata — shipped 2026-09-14: `GET /server.json` and alias `GET /.well-known/mcp` on the Worker (`mcp-server/src/registry-metadata.ts`, official-registry schema 2025-12-11, version from `env.VERSION`, description mirrored from the JSON-LD and test-bound both ways; [ADR-0033](../adr/0033-registry-metadata-served-by-the-worker.md)). **Probed 2026-09-14 on staging** with the `ClaudeBot` / `Claude-User` / `GPTBot` user agents: `/.well-known/mcp` 200, `/server.json` 403 (the zone's AI-bot block exempts only the `/.well-known/` prefix; recorded in [SEO_IMPLEMENTATION.md § Crawler access](../seo/SEO_IMPLEMENTATION.md)). Registry submissions therefore use the `/.well-known/mcp` alias; a WAF skip rule for `/server.json` is optional, not required. The other directories (MCPMarket, Cursor's catalog, the official registry) submit from the same artefact
- [x] Post-listing: `utm_source=claude-directory` is unavailable (the directory does not pass query strings), so attribution for this channel is **new OAuth consents per week** from the Worker's own telemetry, not GA4. **Read from**: the Grafana dashboard's "OAuth consents over time, by outcome" panel (`mcp-server/observability/grafana-dashboard.json`), fed by the `oauth_consent` Analytics Engine event the consent page's approve branch emits since 2026-09-14 (before that the approval reached nothing but `safeLog`, so the number did not exist)

**Slice 3 — Google Ads (operator, time-boxed by the incentive)**

- [ ] **Incentive terms captured first**: amount, spend-by date, and any match requirement. The credit sets the budget and the deadline; nothing else in this slice is designed until those two numbers are known
- [ ] **Conversion import wired before the first impression**: GA4 key events from Slice 0 linked into Google Ads as conversions, with the Ads↔GA4 property link verified in both consoles. Ads without a conversion signal optimise for clicks, which is the one thing the incentive should not buy
- [ ] **Two campaigns, deliberately narrow**: (a) search on the diligence-buyer intent ("technical due diligence", "software due diligence checklist", "IRL template", "tech debt valuation") landing on the advisory / hub-tools pages — this is the advisory-services and brand traffic; (b) search on the agent-builder intent ("MCP server due diligence", "Claude connector M&A") landing on `/hub/mcp/get-started/`. Keep them separate so the two audiences' CPAs are visible. No Display, no Performance Max on a first run — they spend the credit fastest and attribute worst
- [ ] **Negative-keyword list seeded** before launch (job-seeker, student, "free template" without commercial intent, unrelated "GST" tax queries — that acronym collision is the obvious money pit)
- [ ] **Geo and consent aligned with the Slice 0 decision** — if BL-001 gating did not ship, paid targeting is US-only
- [ ] **Stop rule written down**: a spend threshold at which zero attributed conversions ends the campaign, and a weekly review point. The incentive should be fully consumed only if something is working
- [ ] Landing-page fit checked against the ad copy — an ad promising a checklist should not land on a page whose first CTA is a mailto

**Slice 4 — Secondary channels (operator, opportunistic)**

- [ ] MCP-specific directories and registries submitted from the Slice 2 artefacts (MCPMarket, Cursor's catalog, the official MCP registry via `server.json`) — one afternoon once Slice 2's metadata exists
- [ ] One technical write-up placed where builders read — a Hacker News "Show HN", a dev.to / Medium cross-post of the LinkedIn article, or the Cloudflare community (the Worker-hosted OAuth-server design is the genuinely novel part) — with `utm_source` set per venue. Expect brand and MCP traffic from these, not advisory leads, and measure them as such
- [ ] Any existing newsletter / email list (or [BL-004](#bl-004-email-capture-system) if it has shipped) gets the launch once; do not build email capture for this item

**Slice 5 — Measure lift and close**

- [ ] **Four weeks after the last paid impression**, a lift report against the Slice 0 baseline, per channel: sessions, `/hub/mcp/*` engagement, the conversion set, OAuth consents, and bookings — written into the [BL-145](#bl-145-design-partner-program--set-the-price-from-evidence-not-from-a-guess) pipeline home so inbound from this item and outbound from that one aggregate in one place
- [ ] Each channel classified **keep / stop / retry-with-change**, with the reason. The item closes on that report, not on the last post
- [ ] Learnings that are conventions (UTM scheme, `mcp_` events, the directory's requirements) are already in the docs by then; anything else worth keeping goes to an ADR or a successor stanza, per the initiative-doc lifecycle

#### Technical Context

- **Almost all of this is operator work; the engineering is Slice 0 plus the privacy copy and annotation test in Slice 2.** It is filed here, like BL-145, because the backlog is where GST's initiatives are triaged and because two engineering gaps (unmeasured MCP pages, a privacy policy that omits the MCP server) would otherwise be discovered mid-campaign
- **Relationship to BL-145**: complementary, not competing. BL-145 is outbound to a named list; this is inbound at scale. They share a pipeline home and a "no prices in copy" rule, and this item's Slice 5 report is an input to BL-145 Slice 3's willingness-to-pay evidence — inbound that arrives asking for a price is a data point the outbound cannot produce
- **Relationship to BL-093**: this item **does not un-defer BL-093**. It executes BL-093's directory-listing candidate under BL-093's own gate and lifts the `/.well-known/mcp` + `server.json` AC with it; the request-access form and payments stay where they are. If the blitz produces inbound that the mailto cannot absorb, that is BL-093's own "prospect asks for a surface by name" re-engage trigger firing, and BL-093 should be re-opened on that evidence rather than pre-emptively
- **Analytics substrate that already exists**: `trackCTA` / `trackEvent` in `src/utils/analytics.ts`, the hub-tool prefix convention with its unit-test enforcement, the `booking_confirmed` conversion, and the GA4 debugging checklist in [ANALYTICS_TESTING.md](../analytics/ANALYTICS_TESTING.md). Slice 0 extends the pattern; it does not add a second analytics tool
- **Why measurement is Slice 0 and not Slice 5**: the earlier pricing analysis's conclusion was that GST has no data. A campaign that spends a Google credit and cannot say which channel produced the two bookings it got has repeated that mistake with money attached
- **Directory-listing risk to keep in view**: a listed connector is discoverable by every Claude user, which means unprovisioned install attempts arrive at the OAuth consent page at a rate the operator inbox has never seen. The "what does an unprovisioned user get" decision in Slice 2 is the thing that makes the listing safe; without it, listing converts curiosity into support load
- **Related items**: [BL-001](#bl-001-cookie-consent-and-gdpr-compliance) (consent gating becomes load-bearing under paid EU traffic), [BL-004](#bl-004-email-capture-system) (the only channel this item declines to build), [BL-093](#bl-093-mcp-server--commercialization-phase-4) (owns the marketing page and the directory candidate), [BL-133](#bl-133-payments-platform--automated-mcp-access-checkout-on-cloudflare) (inbound volume is its manual-fulfillment trigger), [BL-145](#bl-145-design-partner-program--set-the-price-from-evidence-not-from-a-guess) (the outbound half of the same distribution problem)

---

### BL-153: Website localization — Spanish and Brazilian Portuguese on an architecture that can carry dialects and regions later

**Source**: operator directive 2026-09-04 — add Spanish (`es`) and Brazilian Portuguese (`pt-BR`) to the website; the UX is being designed in Claude Design and will be supplied separately; this item owns the software architecture (routing, translation storage, extensibility, SEO, preference persistence, detection) and the decision record. Dialects and regions are a design constraint on this item, **not** a deliverable of it | **Effort**: ~2–3 weeks engineering for the architecture plus Tier A content; translation effort separate and per tier (see Slice 3) | **Status**: Open · Tier A shipped 2026-09-05 (PR from `feat/website-localization`): Slices 1, 2, 4, 5 and 6 landed with the Claude Design UX; `es` (Colombian register, `/es/`) and `pt-BR` (`/pt/`) are **live**. Remaining: native-speaker review of both catalogs (owed, tracked in [LOCALIZATION.md](LOCALIZATION.md) § Draft → live), Slice 3 Tier B (tool UIs, currency control from the hand-off §4), and the Search Console post-deploy checks. Where the shipped shape deviates from an AC below, [ADR-0030](../adr/0030-website-locale-model.md) is authoritative (one catch-all instead of per-route locale files, `localStorage.gstLang` instead of `{ language, region? }`, `/pt/` instead of `/pt-br/`, `es_CO`)

**As a** Spanish- or Portuguese-speaking PE / corp-dev reader who reaches the site from LinkedIn or search, **I want** the site in my language on a URL I can share, with the browser, search engines, and the site itself all agreeing on which language a page is, **so that** GST's positioning lands without an English filter — and **as the** operator, **I want** one page template per route, one translation catalog per locale, and a guard that fails CI when they drift, **so that** the third language costs a catalog and a review, not a rebuild.

> **The decision this item has to get right is the locale model, because it is the one thing that cannot be changed cheaply afterwards.** A locale is `language[-REGION]` (BCP 47), resolved exact → language → default. Dialects (`pt-PT`, `es-MX`) are additional locales that inherit from their language; regions (currency, date shape, which regulations a map opens on) are a **separate preference axis** that never changes which catalog is loaded. Conflating the two — a `pt-BR` catalog that also decides currency — is the mistake that makes a fourth locale a rewrite. Every slice below is shaped by that split.

> **Scoping rule**: a locale launches only when its whole Tier A route set is translated. Serving English under `/es/` "as a fallback" creates duplicate-content URLs that search engines index as Spanish and readers experience as broken. Astro can do it (`fallbackType: "rewrite"`); this item declines to.

#### Acceptance Criteria

**Slice 1 — Locale model and routing (the architecture; decide first, write the ADR)**

- [ ] **Astro's built-in i18n is the routing substrate** — `i18n.locales` with explicit `{ path, codes }` entries (`{ path: "pt-br", codes: ["pt-BR"] }` so the URL segment is lowercase while the code stays canonical), `defaultLocale: "en"`, `prefixDefaultLocale: false` so **every existing English URL is unchanged** (no redirects, no lost inbound, no BL-152 UTM breakage), `redirectToDefaultLocale` left at its default — it only applies when the default locale is prefixed. `fallback` is deliberately **not** configured for the launch locales; the ADR records that dialects will use `fallback: { "es-MX": "es" }` with `fallbackType: "rewrite"` only when a dialect ships partial overrides on purpose
- [ ] **One page template per route.** Every localized route becomes a thin file: `src/pages/about.astro` (default locale) and `src/pages/[locale]/about.astro` (with `getStaticPaths` over the non-default locales) both render the same component from a new `src/page-templates/` directory, which receives `locale` and reads its copy from the catalog. Duplicating a page body per locale is the anti-pattern this AC exists to forbid; a route with two copies of its markup fails review
- [ ] **A single resolver** (`src/i18n/locale.ts`) owns: the locale list and its metadata (code, path, display name, `og:locale` value, `Intl` tag), `resolve(candidate)` implementing exact → language → default, and `alternatesFor(route)` returning every locale that route exists in. Nothing else in the codebase hardcodes a locale string — a unit test greps for `'es'` / `'pt-BR'` literals outside `src/i18n/` and fails on a hit, the same way the docs guard enforces token parity
- [ ] **Slugs stay English in v1** (`/es/services/`, not `/es/servicios/`). The resolver's route table is keyed by canonical route id so translated slugs can be added later as a per-locale path column without touching pages; record the deferral and its trigger (a locale whose organic search matters enough to want localized slugs)
- [ ] **The preference model is `{ language, region? }`**, stored and read as one object even though `region` is unset in v1. Formatting (`Intl.DateTimeFormat`, `NumberFormat`, currency in the tools) reads `region ?? language-implied-region`; catalog selection reads `language` only. The eleven `toLocaleDateString` / `Intl` call sites across eight files (`HubHeader`, `PrintReportHeader`, the radar `FyiItem` / `WireItem`, `tech-debt-engine.ts`, the tools) route through one `formatters.ts` that takes the resolved locale
- [ ] ADR filed for the locale model, the no-fallback launch rule, English slugs, and the language/region split — per [adr/TEMPLATE.md](../adr/TEMPLATE.md), in the same PR as the resolver

**Slice 2 — Translation storage and the guard**

- [ ] **Catalogs at `src/i18n/<locale>/<namespace>.json`**, namespaced per page template and per shared component (`common`, `header`, `footer`, `home`, `services`, …). English is the schema: a `t()` helper typed from the `en` catalog so a missing key is a **type error**, not a runtime blank. Interpolation and plurals via `Intl.PluralRules` / a minimal ICU subset — evaluate `i18next` vs a ~100-line in-repo helper and record the choice; the bar is "no runtime dependency in the client bundle unless the tools need plural rules"
- [ ] **Parity guard in `npm run test:docs`-style CI**: every non-default catalog has exactly the `en` key set (missing and extra keys both fail), and each translated string carries the hash of the English source it was translated from, so **an English edit marks its translations stale** and the guard reports them by key. This is what keeps a third language honest after launch
- [x] **Client-side strings** (the six `src/scripts` / tool components that write `textContent` / `innerHTML`) read from the same catalogs via a build-time inline of the current locale's namespace — no runtime fetch, no flash of English. **Done for Tier A 2026-09-17**: the only literal reaching a Tier A page was `initCopyButtons`' "Copied" on `/hub/mcp/from-code/`, now `data-copied-label` from the catalog (trial signup already used a JSON island). The tool scripts (`techpar-ui.ts`, `techpar/dom.ts`, `techpar/chart.ts` and the tool-page `<script>` blocks) render only on Tier B routes and move with Tier B. Strings in `src/data/techpar/signal-copy.ts`, `recommendations.ts`, `diligence-machine/questions.ts`, etc. are **content, not chrome**, and are scoped by tier in Slice 3
- [ ] **Translation workflow documented**: source-of-truth is the `en` catalog; first-pass translation is generated (Claude, briefed with the `gst-page-content` audience and voice rules — the register for a PE deal team is the point, not fluency); **a native-speaker review is required before a locale launches** and the reviewer is named in the PR. Machine-only translation ships to no locale
- [ ] Text-expansion tolerance proven: Spanish and Portuguese run 20–30% longer than English. The Claude Design UX is exercised with the longest real strings from the catalogs at desktop, 768px, and 480px, and the `.brutal-*` button and label vocabulary is checked for fixed widths that clip. `hyphens: auto` is enabled under `html[lang]` where the design allows it
- [ ] Font coverage verified: `public/fonts/gst-mono-var-latin-v1.woff2` is a **Latin-subset** face — confirm it carries `ã õ ç ñ ¿ ¡ á é í ó ú ü` (Latin-1 Supplement) rather than falling back to the system font mid-word. If it does not, add the `latin-ext` subset and note the byte cost in [VARIABLES_REFERENCE.md](../styles/VARIABLES_REFERENCE.md) or wherever the font is documented

**Slice 3 — Content tiers (what gets translated, decided explicitly)**

- [ ] **Tier A — required for a locale to launch**: site chrome (header, footer, theme/palette controls, 404/500), `/`, `/services/`, `/about/`, `/hub/`, `/hub/tools/` (index), `/hub/mcp/` (landing), `/privacy/`, `/terms/`. Legal pages carry a visible "English version governs" line unless the operator obtains a reviewed legal translation — decide and record
- [ ] **Tier B — second release per locale**: the tool UIs under `/hub/tools/*` (chrome + question/recommendation copy — the engines' numeric logic is untouched), `/ma-portfolio/` chrome, `/hub/library/` chrome. Tool **output** copy (TechPar signals, ICG recommendations, tech-debt narrative) is Tier B content and is the largest translation surface in the repo (~2.8k lines across `src/data/techpar`, `diligence-machine`, `infrastructure-cost-governance`)
- [ ] **Tier C — explicitly not translated in this item, labelled as such in the UI**: portfolio entries (`projects.json`, domain data), the regulatory-map regulation texts (123 per-regulation JSON files — source-language legal summaries), radar feed content (SSR, upstream English), the MCP capability registry and the four MCP guides (developer-facing, and the MCP server's own tool output is English by contract), and `public/llms.txt`. Each Tier C surface shows a one-line "content in English" note in the current locale. **The MCP server is out of scope entirely** — its contracts are English and BL-093 owns any change to that
- [ ] A locale's launch PR lists which tiers it ships, and the resolver's `alternatesFor(route)` is fed from that — so hreflang, the sitemap, and the switcher only ever claim a page that really exists in that locale

**Slice 4 — SEO**

- [ ] `<html lang>` from `Astro.currentLocale` (today hardcoded `en` in `BaseLayout.astro`); `og:locale` per locale (`es_ES` or `es_419` — decide with the ADR; `pt_BR`) plus `og:locale:alternate`; JSON-LD `inLanguage` on every schema `SEO.astro` and `mcp-schema.ts` emit; canonical is the **same-locale** URL, never the English one
- [ ] `<link rel="alternate" hreflang>` emitted by `SEO.astro` from `alternatesFor(route)` — one per existing locale **plus `x-default` → the English URL** — and a unit test that a Tier C route emits no `es` alternate
- [ ] `@astrojs/sitemap`'s `i18n` option configured (`{ defaultLocale: 'en', locales: { en: 'en-US', es: 'es-ES', 'pt-br': 'pt-BR' } }`) so the sitemap carries the same alternates, and `sitemap-filter.ts` handles locale-prefixed URLs — its pathname-prefix contract currently assumes an unprefixed path, and `/es/brand/` must be excluded exactly as `/brand/` is (extend `tests/unit/indexability.test.ts`, which already covers the filter)
- [ ] `noindex` parity: any route the English site keeps out of the index stays out in every locale — the sitemap filter and `noindex` list are already required to agree, and locales must not become a way to leak one
- [ ] Search Console: each locale verified as indexed with no hreflang errors before it is announced (ties into [BL-152](#bl-152-marketing-blitz--promote-the-released-gst-mcp-across-online-channels) Slice 0 for the English baseline; a Spanish LinkedIn post is the obvious first use of `/es/`)
- [ ] [SEO_COMPONENT.md](../seo/SEO_COMPONENT.md), [SEO_IMPLEMENTATION.md](../seo/SEO_IMPLEMENTATION.md), and [JSON_LD_SCHEMA.md](../seo/JSON_LD_SCHEMA.md) updated in the same PR — they currently document `ogLocale` as a fixed `en_US`

**Slice 5 — Preference persistence and detection (the "is region detection warranted?" answer)**

- [ ] **The URL is the source of truth for language; storage only remembers a choice.** Preference persisted in `localStorage` under one key (`locale`, JSON `{ language, region? }`) by the same inline-`<head>` pattern `BaseLayout.astro` uses for `theme` and `palette` (try/catch, silent in private browsing). No cookie: nothing server-side reads it, and a cookie would drag [BL-001](#bl-001-cookie-consent-and-gdpr-compliance) into scope for no gain
- [ ] **No automatic redirects, by decision.** IP-based or `Accept-Language`-based redirects are rejected: they fight crawlers, they break shared links, and the audience is wrong for it — a Miami PE partner reading in English is the common case, not the edge case. Record this in the ADR with its revisit trigger (a locale whose direct-traffic share is large enough that the banner's friction shows in GA4)
- [ ] **What ships instead**: on a first visit to a default-locale page, with no stored preference, if `navigator.languages` contains a supported non-English language, a **dismissible, non-blocking suggestion banner** (from the Claude Design UX) offers the same route in that locale. Accept stores the preference and navigates; dismiss stores the dismissal; neither is shown twice. The banner is `aria-live="polite"`, keyboard-dismissable, and passes the existing axe-core sweep
- [ ] **Region detection is not warranted in v1** and no geo signal is consumed. Vercel's `x-vercel-ip-country` header is only reachable from middleware on SSR routes (the site is static except `/hub/radar/`), and `vercel.json` header-conditioned redirects would reintroduce exactly the auto-redirect this slice rejects. The `region` axis exists in the preference object so a future explicit region picker (currency, default regulatory jurisdiction) has somewhere to write; its trigger is a tool that needs a regional default, not a locale launch
- [ ] The language switcher (Claude Design UX) lives in the header, preserves the current route across locales via `getRelativeLocaleUrlList` / the resolver, lists only locales the current page exists in, and marks the current one with `aria-current`. Switching updates `document.documentElement.lang`, the stored preference, and fires a GA4 `locale_switch` event; **every GA4 event gains a `locale` parameter** so BL-152's measurement can split by language

**Slice 6 — Tests, docs, and close**

- [ ] Unit: resolver (exact/language/default, unknown → default, dialect → language), catalog parity + staleness guard, hreflang emission, sitemap filter with prefixes, formatters per locale
- [ ] E2E: **do not duplicate the ~116 English text assertions per locale.** Add one per-locale smoke spec (chrome renders in-locale, switcher round-trips a deep route, `<html lang>` and hreflang are right, banner shows/dismisses/persists) and extend `tests/e2e/accessibility.test.ts` with a sampled Tier A route per locale. Directive 11 applies to every string this item moves into a catalog: `grep tests/` for the old literal
- [ ] Docs: new `src/docs/development/LOCALIZATION.md` (locale model, adding a locale, adding a string, the translation workflow, the tier table) linked from the master index and from `.claude/CLAUDE.md`; the `gst-page-content` skill gains a hand-off line for translation register; [STYLES_GUIDE.md](../styles/STYLES_GUIDE.md) gains the text-expansion and `hyphens` notes
- [ ] The item closes when `es` and `pt-BR` are live at Tier A with native review recorded, the guard is in CI, and the ADR documents how a dialect and a region would be added **without** code changes to the resolver's contract — that last sentence is the extensibility acceptance test

#### Technical Context

- **Why Astro's built-in i18n and not a library-first approach**: Astro 7 ships locale-aware routing, `astro:i18n` URL helpers, `Astro.currentLocale`, and sitemap alternates natively; the site is static, so there is no server to run a translation runtime on. What Astro does **not** provide is a message catalog or `t()` — that is the only piece this item builds or adopts, which is why Slice 2's dependency decision is small
- **Why `prefixDefaultLocale: false`**: every inbound link, every UTM in BL-152, every Search Console history entry, and every `trailingSlash` rule in `vercel.json` is on unprefixed English URLs. Prefixing English would trade all of that for symmetry
- **Why the page-template split is the load-bearing refactor**: 86 `.astro` files hold their copy inline. Moving copy to catalogs without also collapsing each route to one template would leave two render paths per page to keep in sync — the same trap as duplicated markup, one level up. The refactor is mechanical per page and is most of the Tier A engineering cost
- **Existing patterns this reuses rather than replaces**: the `<head>` inline preference bootstrap (theme/palette), the `html.<class>` convention for document-level state, `trackCTA`/`trackEvent`, the sitemap filter's URL-parsing contract, the docs-guard style of CI parity test, and the `gst-page-content` skill for voice
- **What the Claude Design hand-off must contain for Slice 5 to start**: the switcher (header placement, open/closed states, current-locale marker, ≥3 locales), the suggestion banner (copy in all three languages, dismiss affordance), and the Tier C "content in English" note — each at desktop/768/480 and in light + dark. The engineering here is independent of the visuals; the visuals are the only blocker and only for that slice
- **Radar is the one SSR route** (`prerender = false`), so it is the one place middleware or `Astro.preferredLocale` would work — and its content is upstream English feed text. Treat it as Tier C chrome and nothing more
- **Related items**: [BL-152](#bl-152-marketing-blitz--promote-the-released-gst-mcp-across-online-channels) (Spanish/Portuguese LinkedIn posts are the first consumers of `/es/` and `/pt-br/`; its `locale` event parameter lands here), [BL-001](#bl-001-cookie-consent-and-gdpr-compliance) (kept out of scope by the no-cookie decision), [BL-093](#bl-093-mcp-server--commercialization-phase-4) (owns any MCP-side language question; none is raised here), [BL-145](#bl-145-design-partner-program--set-the-price-from-evidence-not-from-a-guess) (LatAm and Iberian PE targets on its list are the demand signal for which locale ships Tier B first)
...
### BL-154: Architecture reference — five-layer diagram set, and a storage-store review

**Source**: two operator asks on 2026-09-06, both raised while reshaping [BL-133](#bl-133-payments-platform--automated-mcp-access-checkout-on-cloudflare) for its parked trial directive — "how are user credentials and expiration expected to be managed?", and "create architectural reference diagrams for the entire GST website and MCP integration" | **Effort**: Slice 1 is an audit and a decision, not a build; Slice 2 is documentation authoring across both workspaces | **Status**: Open

**As the** GST operator, **I want** the estate legible at every layer an acquirer or a new contributor would ask about, and **I want** the storage substrate decided before self-serve work forces the question, **so that** the architecture is something I can hand someone rather than something I hold in my head.

> **Filed under Infrastructure on subject matter, not on commitment level.** The distinction is deliberately _not_ "committed work here, speculative work there" — [`## Exploration`](#exploration) carries BL-117 and BL-118 as candidates and this section's own BL-141 is `Candidate — no committed consumer yet`, so commitment does not separate the two. Exploration holds prototypes and deferred substrate choices (BL-035 is explicitly "no commitment to ship"; BL-048 waits on picking a secret manager). Infrastructure is the system-and-estate section, which is what a whole-estate architecture reference and a storage-substrate review both are. The two slices ship as one stanza because they were asked for as one.

#### Slice 1 — Storage-store review

**The inventory, as the starting point.** Four stores today, none relational:

- **`OAUTH_KV`** (Workers KV) — M2M client records at `mcp:oauth:m2m-client:<id>`, plus one-shot consent and JTI nonces. [`wrangler.toml`](../../../mcp-server/wrangler.toml) records the deliberate single-substrate deviation: the OAuth library requires KV, and splitting auth state across two stores was judged worse.
- **Upstash Redis** — response caching and the rate-limit sliding windows.
- **`AUDIT_R2`** — **deactivated** per [ADR-0014](../adr/0014-deactivate-audit-pipeline.md), binding retained.
- **Cloudflare Queues** (`audit-log-{dev,staging,production}` + DLQs) — likewise retained but unbound, and `wrangler.toml` says explicitly _do not garbage-collect them_, because the re-enable procedure depends on them existing.
- **Analytics Engine `METRICS`** — per-`keyOwner` telemetry, sampled.

No D1, no Durable Objects.

- [ ] **The forcing question, answered from the code.** KV has no secondary index and no uniqueness constraint, and `listM2mClients` (`mcp-server/src/oauth/m2m-clients.ts`) is `kv.list()` plus one sequential `get` per client — a full scan. Enumerate what a self-serve future needs that this cannot do: expiring-soon queries, one-credential-per-identity, payment-customer reverse lookup, usage joined to a client record. Then decide whether that is worth a store
- [ ] **The counter-case stated, not assumed.** KV is genuinely sufficient for expiry alone — `expirationTtl` reaps the record for free — and the client-record read sits on the **hot path of every token mint** (`mcp-server/src/oauth/m2m-token.ts`), so a relational store's latency and consistency there must be **measured**, not reasoned about. A review that only asks "should we adopt D1" has already answered itself
- [ ] **Adjacent estate questions settled in the same pass**: whether R2 and the Queues stay retained-but-unbound (ADR-0014's re-enable trigger is "the first client whose contract requires compliance audit capture" — note [`PAYMENTS_PLATFORM_BL-133.md`](PAYMENTS_PLATFORM_BL-133.md) § Audit decision has _already_ ruled that a self-serve card purchase is **not** that client, so this is narrower than it looks); whether Upstash and any new store overlap; whether Analytics Engine sampling can carry [BL-145](#bl-145-design-partner-program--set-the-price-from-evidence-not-from-a-guess)'s usage-evidence need or whether that needs a durable home
- [ ] **Exit criterion**: a written ADR in [`src/docs/adr/`](../adr/README.md) recording the target shape and the trigger to act on it, with **"keep KV, and here is why" named as an acceptable outcome**. Same falsifiability standard BL-145 sets for itself: a review that can only conclude "adopt D1" is not an instrument. Re-derive the ADR number from the index at creation time
- [ ] **Out of scope**: performing any migration. The standing operator directive against speculative optimization applies — this slice measures and decides; it does not tune

#### Slice 2 — Five-layer architecture reference diagrams

Scope is **the whole GST estate** — the Astro website and the MCP integration together — not this initiative. The layers are the five in [`business-architectures/article.md`](../../data/library/business-architectures/article.md): Software (the foundation), Operational, Product, Organizational, Industry & Regulatory — read foundation-up, because each layer inherits the possibilities and limitations of the one below.

- [ ] **Structure**: `src/docs/architecture/` containing `README.md` — the layer map, plus the article's § _How the layers interact_ applied to GST, since the cross-layer constraints are exactly what per-layer files cannot hold — and five files: `1-software.md`, `2-operational.md`, `3-product.md`, `4-organizational.md`, `5-industry-regulatory.md`. Numeric prefixes preserve the reading order
- [ ] **Diagrams are mermaid in fenced blocks**, and `README.md` states this as the convention **for this directory** — explicitly not a repo-wide ruling, and explicitly not a call to convert anything. The repo is not a blank slate: mermaid, SVG, `.mmd`, `.drawio` and `.puml` are all absent, but **ASCII box-drawing diagrams already exist in 17 maintained markdown files** (plus 17 more under `development/_archive/`) — [`DEVELOPER_TOOLING.md`](DEVELOPER_TOOLING.md), [`JSON_LD_SCHEMA.md`](../seo/JSON_LD_SCHEMA.md), [`TEST_STRATEGY.md`](../testing/TEST_STRATEGY.md) and CLAUDE.md's own project-structure tree among them. (Counted by scanning `**/*.md` for U+2500–U+257F box-drawing characters. Note `ARCHITECTURE.md` contains **none** — two separate reviews asserted it as the leading example, and it is prose throughout.) This slice adds a second medium in one directory, not a first medium to the repo
- [ ] **Do not fork the maintained references — and prove it per section, not by principle.** [`ARCHITECTURE.md`](../../../mcp-server/src/docs/ARCHITECTURE.md) does not merely risk being forked; it **already holds** the layer-1 and layer-2 material in five sections: § System shape, § Remote transport & request flow, § Auth/CORS & deploy topology, § Radar pipeline, § Observability. Code comments cite its anchors as load-bearing, so a competing depiction drifts silently with no guard. **Before drawing anything at layers 1–2, record a table**: for each of those five sections, either the specific new cross-cutting view the diagram adds, or that the layer file **links rather than draws**
- [ ] **Other maintained docs at the same risk, named so they are not rediscovered**: [`DEPLOY.md`](../../../mcp-server/src/docs/operations/DEPLOY.md) (deploy topology is squarely Layer 2), [`mcp-server/src/docs/README.md`](../../../mcp-server/src/docs/README.md), [`SECRETS_INVENTORY.md`](../operations/SECRETS_INVENTORY.md), [`LOCALIZATION.md`](LOCALIZATION.md), [`STYLES_GUIDE.md`](../styles/STYLES_GUIDE.md), [`DEVELOPER_TOOLING.md`](DEVELOPER_TOOLING.md), [`SECURITY_HEADERS.md`](../security/SECURITY_HEADERS.md), and the ADR corpus. Pointedly including **`.claude/CLAUDE.md`'s 🗂️ Project Structure tree**, which a Layer-1 "two workspaces" diagram would duplicate outright. The diagrams are a map; a map is not a second territory
- [ ] **Layers 4 and 5 are not the same case, and must not be treated as a pair.** Layer 4 (Organizational) is **genuinely thin** — GST is a one-operator company — and should say so plainly: "this layer is thin because GST is a single operator; here is what would populate it at scale" is honest and useful to the M&A audience the framework serves. Layer 5 is **not thin at all**: the MCP spec, OAuth 2.1 and its RFCs, GDPR/CSP obligations, and the regulation corpus the product itself indexes make it one of the richer layers here. Manufacturing symmetry fails in both directions — inventing Layer 4 content, or under-serving Layer 5 by adjacency
- [ ] Candidate anchors, as a starting point rather than a contract: **L1** the two workspaces, the Worker/Vercel split, the three auth paths, the tool/prompt/resource surfaces; **L2** deploy topology, the four stores (Slice 1's output lands here), CI/CD, observability; **L3** the Hub tools, the MCP capability surface, the website funnel; **L4** the operator-plus-agents working model and the review gates
- [ ] **Slice 1 feeds Layer 2** — either sequence its ADR ahead of the layer-2 file, or state in that file that it depicts the _current_ four-store shape and is revised by the ADR
- [ ] **Each diagram verified to render** (GitHub preview or an editor) before the slice is done. `test:docs` validates links and anchors but **nothing renders or lints mermaid** — a syntactically broken diagram passes CI and fails silently on GitHub
- [ ] **Index updates in the same commit**: one row in [`src/docs/README.md`](../README.md)'s Directories table with a **Docs** count of **5** (the five layer files; the folder's own `README.md` is excluded, per that table's own convention note), plus one **Quick Navigation** row placed beside "Understand the MCP server's architecture" — with **both** worded so a reader can tell them apart, since the new one is the whole-estate map and the existing one is the server's internals. Add the directory to CLAUDE.md's 🗂️ Project Structure tree
- [ ] **Not a claim of completeness.** State in `README.md` that this is the article's framing, that the article itself says "other reasonable framings exist", and that these are reference diagrams rather than a governance artifact — so nobody later treats an un-updated diagram as authoritative

#### Technical Context

- **Slice 1 is what the BL-133 trial directive actually surfaced.** The operator's question — are credentials and expiry managed inside the platform or manually outside it? — has a precise answer today: entirely in-platform, as one JSON blob per client in KV, with the _decision_ to provision being the only manual part. What does not exist is any expiry primitive (`M2mClientRecord` has no `expiresAt`) or any way to query the keyspace. That is the gap this slice sizes
- **Slice 2 has no dependency on Slice 1** beyond the Layer-2 sequencing note. They ship as one stanza because they were asked for together, not because either blocks the other
- **Related items**: [BL-133](#bl-133-payments-platform--automated-mcp-access-checkout-on-cloudflare) (its parked trial directive raised Slice 1), [BL-048](#bl-048-mcp-server--wrangler-secret-sync-extracted-from-bl-037-phase-d) (the same shape of decision — deferred pending a substrate choice), [ADR-0014](../adr/0014-deactivate-audit-pipeline.md), [BL-145](#bl-145-design-partner-program--set-the-price-from-evidence-not-from-a-guess) (its usage-evidence need is one of Slice 1's adjacent questions)
...
- [x] `/.well-known/mcp` + `server.json` registry metadata published on the Worker — shipped 2026-09-14 under [BL-152](#bl-152-marketing-blitz--promote-the-released-gst-mcp-across-online-channels) Slice 2 (`mcp-server/src/registry-metadata.ts`, [ADR-0033](../adr/0033-registry-metadata-served-by-the-worker.md)); the hard gate below is untouched
