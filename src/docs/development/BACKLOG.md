...
**And there is a cheaper fix candidate than any of the above, found by asking what the model could have read to reach its conclusion.** Three surfaces tell it that Section-09 frameworks belong in `defaultFiredFrameworks`, and **none of the three carves out the case where a Section-09 framework is also a named trigger** — which is exactly `EU_AI_ACT` on any EU-exposed target:

- `irl-ingestion.ts:714` — _"Section-09-enumerated frameworks (GDPR, UK GDPR, PIPEDA, POPIA, Australia Privacy Act, etc.) are NOT conditional triggers — they go in `defaultFiredFrameworks`."_
- `irl-ingestion.ts:1342` (the interactive copy) — _"do NOT list Section-09 frameworks here."_
- `schemas/compose-dossier-envelope.ts:313` — _"Section-09 enumerated regulatory frameworks the partner is subject to (GDPR, UK GDPR, PIPEDA, POPIA, etc.)."_

Every example in all three is a non-trigger, so none is literally wrong — and the model's reported rationale, _"the more accurate representation when Section 09 names it explicitly,"_ is a syntactically valid reading of them.

**The sentence that disambiguates exists, and lives nowhere the model reads.** `CONTRACT.md:187`: _"The partition is by **how the framework was reached**, not by whether Section 09 happens to name it."_ That is the whole rule, and it is in the maintained doc only — absent from both prompt bodies and from the schema `.describe()`. Porting that one clause into the three surfaces above is narrower than server-side predicate evaluation, and unlike more prompt prose it is not a restatement: it is the missing exception, not the fourth copy of the rule. **No evidence establishes that the model read `:714` rather than something else**, so this is a candidate to test first, not a diagnosis.
