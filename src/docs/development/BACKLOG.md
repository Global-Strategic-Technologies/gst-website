...
- [ ] **Two items deliberately deferred out of the 2026-07-29 sweep** (recorded so they don't evaporate):
  - The `.brand-*` replicas in `brand.astro` omit their originals' `@media (max-width: 768px)` overrides — e.g. `PortfolioGrid`'s `.metric-value` shrinks at mobile, the replica does not. Only matters if the specimens are ever viewport-demoed directly; the Responsive Behavior iframes already cover that job for the components they frame.
