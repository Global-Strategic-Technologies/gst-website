...
- [ ] **Sentry event capture for handled-error paths** — `auth.failed` and `inoreader-rate-limit` (Inoreader 429) handlers call `Sentry.captureMessage` / `captureException` so [SENTRY_MANUAL_SETUP.md](SENTRY_MANUAL_SETUP.md) Alerts #2 + #3 fire on real incidents. Today these handlers early-return / catch-and-convert without notifying Sentry, so the configured alert rules sit dormant. Closes the "configured-but-dormant" gap documented in [SENTRY_MANUAL_SETUP.md § "Why some alerts are silent today"](SENTRY_MANUAL_SETUP.md#alert-rules) and exercises the unused [`captureException` export](../../../mcp-server/src/observability/sentry.ts#L88). Cross-ref: [worker.ts:111-118](../../../mcp-server/src/worker.ts#L111-L118) (auth.failed early return), [radar-live.ts:117](../../../mcp-server/src/tools/radar-live.ts#L117) (Inoreader-429 catch + circuit-open path). Verification: T.E.11 + T.E.12 in [MCP_SERVER_REMOTE_BL-032_TESTING.md § Section E](MCP_SERVER_REMOTE_BL-032_TESTING.md). NOTE: `auth.failed` capture must rate-limit per-fingerprint (Sentry's group-by-issue handles this) so a public-Worker probing burst doesn't flood the project quota; `inoreader-rate-limit` capture has no such concern (low-volume by definition — once per 6h breaker-open). **Dual-tracking note (2026-05-12)**: this same AC is mirrored in [BL-032.75 § K-section evidence-driven mitigations → Sentry captureMessage wiring](#bl-03275-mcp-server--production-observability-maturity). Closing either bullet must close the other to avoid drift — the underlying code change ships once. Both home for this work is BL-032.75 (engineering capacity lives there); the line here is the BL-032 AC contract that BL-032.75 inherits.
...
- [ ] **§ 2 — T.A.4 empty-bearer error message** — `Authorization: Bearer ` (empty token) returns 401 with message `"Authorization header must use Bearer scheme"` instead of an empty-token-specific message. Cosmetic error-disambiguation gap. **Severity: P1**. See [MCP_SERVER_REMOTE_BL-032_25.md § 2](./MCP_SERVER_REMOTE_BL-032_25.md#§-2--ta4-empty-bearer-error-message-p1) and [TESTING_FINDINGS § T.A.4](./BL-032_TESTING_FINDINGS.md#ta4--empty-bearer-schema).
- [ ] **§ 3 — T.K.2.b.3 local stdio `generate_diligence_agenda` timeout** — call hung 4 min on Claude Desktop's local stdio connector while staging completed normally. Tool-specific (other local-stdio tools worked in same session). Suspect: large JSON response overflowing stdio buffer, engine slow-path, or stdio child-process deadlock. **Severity: P1**. See [MCP_SERVER_REMOTE_BL-032_25.md § 3](./MCP_SERVER_REMOTE_BL-032_25.md#§-3--tk2b3-local-stdio-diligence-timeout-p1) and [TESTING_FINDINGS § T.K.2.b.3](./BL-032_TESTING_FINDINGS.md#tk2b3--generate_diligence_agenda).
- [ ] **§ 4 — T.X.1 secondary playbook polish** — two follow-ups from the T.X.1 placeholder-trap fix: (a) bash equivalent in [`DEPLOY.md` § B.3](../../../mcp-server/src/docs/operations/DEPLOY.md) likely has the same hazard — review and convert to `read -s MCP_KEY` if so; (b) `Invoke-McpRequest` helper's SSE-only parser silently returns the raw HTTP response on non-2xx, leaving operators with `$null` and no obvious cause. Either raise a clearer error OR document the diagnostic incantation prominently in the playbook. **Severity: P1**. See [MCP_SERVER_REMOTE_BL-032_25.md § 4](./MCP_SERVER_REMOTE_BL-032_25.md#§-4--tx1-secondary-playbook-polish-p1) and [TESTING_FINDINGS § T.X.1](./BL-032_TESTING_FINDINGS.md#tx1--setup-snippet-placeholder-is-a-copy-paste-trap).
- [ ] **§ 5 — T.X.4 credential-prompt `-AsSecureString` sweep** — three Upstash tokens leaked to chat transcripts during soak (T.C.7 recovery × 2, T.B.9.f preflight × 1) because Claude-authored preflight blocks used plain `Read-Host` rather than `-AsSecureString`. The T.X.1 fix migrated only `Invoke-McpRequest.ps1`; ad-hoc Section D / T.B.9.f / T.B.10.f-style snippets were not migrated. **Severity: P0** — token-rotation work blocks production deploy until process-hygiene closes too. See [MCP_SERVER_REMOTE_BL-032_25.md § 5](./MCP_SERVER_REMOTE_BL-032_25.md#§-5--tx4-credential-prompt--assecurestring-sweep-p0) and [TESTING_FINDINGS § T.X.4](./BL-032_TESTING_FINDINGS.md#tx4--third-upstash-standard-token-leaked-to-chat-during-tb9f-preflight).
