...
### BL-067: `citationSchema` em-dash literal — enrich rejection or pre-coerce hyphen→em-dash ⏳ DEFERRED-PENDING-RECURRENCE 2026-06-05

**Problem (observed once)**: 2026-06-05 post-BL-066 live exercise (claude-sonnet-4-6) hit `generate_diligence_agenda: 2/1` — attempt 1 was the expected 14-issue calibration consolidation (BL-065+BL-066 forcing-function framing working as designed: all 14 collapsed into a single retry), but attempt 2 surfaced a NEW floor: `errorClass: citation-format-invalid` with `recoveryAction: corrected-em-dash-and-citation-length`. The model used a plain hyphen `-` where `citationSchema` requires an em-dash `—` (regex: `^Section (\d{2}|--)[^—]*—.{20,}$` at `mcp-server/src/schemas/diligence-audit.ts`).

**Why deferred, not shipped**: this is one occurrence in one exercise on one model tier. `feedback_no_unfounded_risk_claims` + the BL-066 acceptance discipline say a single data point isn't a pattern. The next exercise (operator-driven, on a stronger model) will confirm whether the em-dash error is a recurring failure mode or a single-tier quirk of sonnet-4-6.

**If it recurs in the next 1–2 exercises, scope the fix as**:

1. **Enrich the citation regex rejection message** at the Zod schema definition site to include a literal `Fix: use em-dash (—, U+2014), not hyphen (-, U+002D). Section <NN> — <excerpt of ≥20 chars>.` This is the BL-065 forcing-function pattern applied to a structural Zod issue (the SDK's published Zod error text would otherwise just say "string does not match regex"). Same shape as the `Fix:` lines BL-065 added to cross-field refinements.
2. **OR pre-coerce** at the audit-payload layer: walk `_audit.<dim>.citation` strings and replace `-` (space-hyphen-space) with `—` (space-em-dash-space) before validation. Riskier — silently rewrites the model's text — but eliminates the retry entirely.

Option 1 is the audit-discipline answer (forcing function, no silent rewriting). Option 2 is the "make the wire format forgiving" answer. Pick at filing time based on whether other character-class issues co-occur (apostrophe variants, smart-quotes, en-dash vs em-dash).

**Acceptance trigger**: file as OPEN and ship when ≥2 of the next 3 live exercises show `citation-format-invalid` as the attempt-2 errorClass. If the next exercise on a stronger model produces 1/1 with no citation issue, close as NOT-A-PATTERN.

**Related**: BL-065 (forcing-function rejection enrichment), BL-066 (Rule-0 batch summary); the em-dash issue is structurally a "non-coded structural error" — the gap BL-065 originally tried to close at the handler layer before BL-066 reverted that approach. If em-dash recurs, it's evidence that the BL-066 trade-off ("structural Zod failures surface as raw SDK errors without Fix lines") has a real recurring cost and warrants a per-schema rejection enrichment pass (which doesn't require the BL-065 architectural compromise — just `.describe()` enrichment + custom message in `.regex(..., { message: ... })`).
