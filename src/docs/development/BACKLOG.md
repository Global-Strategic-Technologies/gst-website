...
- **A top inset, and it retires the `133px` magic number rather than inheriting it.** Found by the `code-reviewer` gate, which refused the push. Two **fixed** bars sit above the drawer — the site header (outside `main`, so it outranks everything inside it) and the sticky search/filter bar (`z-index: 9999` inside `main`, against the drawer's `1001`) — and `main { position: relative; z-index: 1 }` makes `main` a stacking context, so **no z-index the drawer can carry will lift it over either**. Subtracting a 12px gap from the cap put the sheet's top at y=12 when scrolled to the footer, sliding the FILTERS header and its close button underneath the site header: `elementFromPoint` on the close button returned `HEADER.site-header`, so the only in-sheet way to dismiss the drawer was **not tappable**. The `133px` is now `--drawer-top-inset`, used by both `top` and the phone cap, so the sheet is clear of the chrome in every scroll position. That also makes the cap's middle term always the binding one, which is what the footer listener's `scrollTop` coupling assumes.

**Measured on closure** (400×700, `/ma-portfolio/`): footer 106px, clearance 122px, natural drawer content 763px, `--drawer-top-inset` 133px. At rest the sheet renders 567px tall with its bottom at the viewport bottom and its top at the inset; scrolled to the page bottom it renders 445px, top still at the inset, with the 16px footer gap intact. Both outer clamp terms are guard rails that do not bind at realistic viewports — the 30% floor needs 478px of clearance, and the 85% ceiling needs a viewport over ~890px tall, where the drawer's own content is already shorter than the ceiling. Neither is asserted, because contriving a viewport to reach one would test the arithmetic rather than the behaviour.

**The lesson worth keeping.** Three of this change's own tests asserted geometry — `top >= 12`, header rect inside the viewport — and all three passed while the close button was completely buried. Rect assertions cannot see stacking. The suite now asserts hittability via `elementFromPoint` at both phone and tablet widths, and that assertion fails on the pre-fix CSS.
...
- **`--drawer-top-inset` is load-bearing, not cosmetic.** It is the only lever that clears the two fixed bars above the drawer, because `main`'s stacking context puts them permanently out of z-index reach. Changing it moves the sheet's top edge on both the phone and tablet branches; the hittability tests are what will catch a wrong value. **It is one flat value and not a perfect clear at the tablet band** — the chrome ends at 130 at 400×700 but 138 at 600×800, so ~5px of the sheet's top border sits under the bar there. Cosmetic, the same overlap the 350px desktop panel has always had at its top-left corner, and the controls the inset exists to protect are reachable at both bands. A band-specific second value would buy 5px of border for another magic number.
- **One thing left behind for whoever needs it**: `--filter-drawer-bg` (`variables.css:203`) is defined and documented but referenced by nothing. The mobile surface fix used `--surface-overlay-bg` instead, since that is the variant global.css documents for drawers over content.
