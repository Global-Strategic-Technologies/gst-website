...
- [ ] **TechPar `exitMultiple` UX gap** (discovered during BL-031.5 V2 verification trial 1; **cited file path stale as of 2026-05-26 sweep — needs re-investigation before closure**): the wizard's exit-multiple input was conditionally rendered — only visible when stage is `pe` or `enterprise`. **2026-05-26 sweep finding**: the originally-cited [`techpar-ui.ts:65-67`](../../../src/utils/techpar-ui.ts#L65-L67) no longer contains any `exitMultiple` references. The exit-multiple wiring now lives in [`src/utils/techpar/dom.ts:266,566,710`](../../../src/utils/techpar/dom.ts) and [`src/utils/techpar-engine.ts`](../../../src/utils/techpar-engine.ts) (8 references); whether the conditional-visibility + persistent-state behavior still exists hasn't been confirmed. Re-investigate before closing — the bullet's underlying concern (silent state persistence across stage changes affecting `gap.exitValue`) may still apply, may have been silently fixed, or may have been refactored away with the move to `techpar/dom.ts`. Original options: (a) reset `exitMultiple` to its default when stage drops below PE, (b) show the field at all stages with stage-appropriate guidance, (c) add a "current state" inspection panel, or (d) document the behavior as intentional
...
- [ ] **ICG `answeredCount` accounting bug** (filed from BL-034 sweep 2026-05-26): [`src/utils/icg-engine.ts:131`](../../utils/icg-engine.ts#L131) sets `answeredCount = Object.keys(state.answers).length` — but `state.answers` also contains entries with value `-1` (skipped questions), and line 132 immediately computes `skippedCount` filtering for exactly those `-1` values. **Result**: a skipped question is counted as BOTH answered AND skipped; the "Questions answered: N of T" display overcounts. **Fix**: `answeredCount = Object.values(state.answers).filter(v => v !== -1).length`. Small change (one line) but downstream consumers (Hub wizard results panel + MCP `assess_infrastructure_cost_governance` envelope) need a unit-test sweep before merge — touches result-schema shape semantics even though the field type doesn't change. Estimated ~20-30 min including consumer audit + test. **Confirmed still-present** during the 2026-05-26 audit sweep — not a stale-fixed bullet. Adjacent concern: the "Wizard / API symmetry follow-up" bullet above lists "add an `answeredCount`-based result-confidence indicator" as one resolution path; that proposal assumed `answeredCount` is correct. Fix this bullet first, then re-evaluate the symmetry proposal
