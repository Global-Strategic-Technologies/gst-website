...
> **Renumbered from BL-137 on 2026-08-23.** This item was filed as BL-137 on `feat/mcp-website-marketing` on 2026-08-18. Independently, and three days later, master filed a different BL-137 (workers-types global shadowing) — which then shipped, closed, and became [ADR-0020](../adr/0020-workers-types-global-shadowing-immunity.md). Merging master into this branch put two BL-137s in one file, so the unmerged one moved. **Commits on this branch dated 2026-08-18 to 2026-08-22 say "BL-137" and mean this stanza**; commit messages are immutable, so that mismatch is permanent and this note is the only thing that explains it. The wave-list entry in the header referring to "BL-137, workers-types global shadowing" is the other one, correctly.
...
>
> **The rest of BL-093 stays deferred and the premise-decision instruction stands.** Docs site, request-access form, payments, and the directory listing were NOT reopened by this. The marketing page therefore ships against surfaces that exist — the status page and the operator inbox — and deliberately links no docs subdomain. `tests/integration/mcp-marketing-parity.test.ts` binds the page's published ceilings, tool catalog, and hostnames to `mcp-server` source and fails the build if a future edit adds an SLA claim, an uptime figure, or a `docs.mcp.globalstrategic.tech` link.
...
- [ ] **FAQ answers embedding anchors flow into the FAQPage JSON-LD verbatim** — `SEO.astro` passes each `answer` string straight into `acceptedAnswer.text`, so the two MCP entries added on this branch (`services.astro`, `hub/index.astro`) put `<a href="/hub/mcp/">` markup, with a RELATIVE URL, into the structured data. Google's guidance allows limited HTML in answers, so this is a correctness question rather than a known break: decide between a tag-stripped copy for the JSON-LD and absolutizing the href. First anchors in the repo's FAQ data, so whatever is chosen becomes the pattern — [`JSON_LD_SCHEMA.md`](../seo/JSON_LD_SCHEMA.md) § FAQPage now records the behaviour
...
**Standing caution**: an unreproduced single-test failure in the mcp suite, now seen **three times**. The name was captured on the third (see below), which is the first hard evidence about which test it is. It fits the documented workerd cold-start flake but that remains an explanation, not evidence.
...
The six optimised outputs (three mp4 + three posters, `948 KiB`) are **untracked, and exist only on the operator's machine** — a fresh clone has none of them. The two raw captures now live in a gitignored `media-raw/` at the repo root, moved there so nothing under `public/` could sweep them into a deploy or a `git add -A`.
...
Measured basis at the time of the decision: pack `16.79 MiB`; largest tracked file `package-lock.json` at `0.62 MiB`; the keepers totalled `1.285 MiB` across nine files. Dropping the WebM twins took that to `948 KiB` across six, so the argument for git only got stronger — the decision did not depend on the reduction. Media-in-git is a real problem for large, frequently re-rendered binaries; these are write-once onboarding captures that change only when Claude Desktop's UI changes.
...
- **WebM twins were produced and then removed 2026-08-19 — do not re-add them.** The recipe was `libvpx-vp9 -crf 38 -b:v 0 -row-mt 1` at `fps=15`, kept here only so the rejection is checkable rather than repeated. Measured against the mp4s: connector-enabled `50.0` vs `48.3 KB` (+3.5%), prompts-resources `182.5` vs `184.6 KB` (−1.1%, noise), regulations-query `135.4` vs `93.9 KB` (**+44%**) — `367.9 KB` of WebM against `326.8 KB` of mp4, i.e. **12.6% larger in total**. The mp4+webm pairing buys reach only when one codec alone leaves a browser gap; H.264/MP4 is the universally supported baseline and WebM/VP9 support is _narrower_ (Safari 14+ only), so the twins reached no browser the mp4 misses while costing bytes. If a future encode makes VP9 or AV1 genuinely smaller **and** legibility holds, re-measure before re-adding — do not re-add on convention.
...
#### The poster set is provisional — and one thing must not be lost with it

**Operator note, 2026-08-19: one or more of the three posters will likely be dropped, depending on how the design implements the media blocks.** A design that shares a single still across the set, or that renders the reduced-motion state some other way, does not need three. Do not treat the current three as a fixed requirement.

**But each poster is currently doing two jobs, and only one of them is obvious.** It fills the `poster` attribute so a reader sees a meaningful frame instead of an empty box before playback — that job is cosmetic and genuinely optional. The second is not: **the poster is the `prefers-reduced-motion: reduce` fallback**. With three autoplaying loops on one page, a reduced-motion visitor must get static images instead, and the poster is what renders. Dropping a clip's poster therefore removes that clip's reduced-motion answer.

So the rule is: a poster may go **only** once that clip's reduced-motion state is satisfied some other way (a shared still, a first-frame extraction, a static alternative block). Deleting one to save bytes without replacing the fallback silently breaks an accessibility requirement that nothing in CI checks — there is no automated guard for this, which is why it is written down here.

If the posters survive as PNGs, they are the heaviest part of the set at `621 KB` of the `948 KiB`. Converting to WebP at quality 82 measured `110 KB` — an 82% reduction — and was checked at full size against the smallest text in the set (the regulations-query tool labels) with no visible loss. `poster` accepts WebP in every browser that matters.
...
- [ ] The media files committed, with a message naming `/hub/mcp/get-started/` — they have no consumer until the page lands, so an unused-asset sweep would otherwise remove them. **Six as of 2026-08-19** (three mp4 + three posters, `948 KiB`), down from nine when the WebM twins were dropped. Do not treat six as fixed — see the poster note below
- [ ] **ADR-0020** written from [`adr/TEMPLATE.md`](../adr/TEMPLATE.md) with `**Source initiative**: BL-138`, recording the git-vs-object-storage decision, the rejected alternatives, and the revisit threshold — plus its index row added to [`adr/README.md`](../adr/README.md). **Re-check the next free number before writing it**: this AC said 0019 when filed on 2026-08-19 and was stale within two days — master took 0019 for [the IRL extract record](../adr/0019-irl-extract-record-subject-indexing.md). 0001–0019 exist with no gaps as of 2026-08-21
- [ ] **The recipes migrated to a durable home.** This file removes completed items (see the note at the top), recoverable only via `git log` — but the recipes and the three page constraints are exactly what is needed _after_ closure, since the whole hosting rationale is that these clips get re-rendered when Claude Desktop's UI changes. A committed build script under `scripts/` is the elegant form (executable documentation that survives pruning); the Consequences section of this initiative's own ADR is the fallback. It must live in a **tracked** directory — beside the media in `media-raw/` it would be gitignored along with the files it documents, reproducing the exact problem this criterion exists to solve
