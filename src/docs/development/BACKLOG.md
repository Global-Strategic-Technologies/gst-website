...
### BL-100: FyiItem nests a link inside `<summary>` — `nested-interactive`

**Source**: surfaced 2026-08-02 the moment `/hub/radar/` joined the axe sweep and was run against real content (`npm run radar:stub`) | **Effort**: Small to change, but gated on a product call | **Status**: Open — needs the interaction ruling first

**As a** screen-reader or keyboard user, **I want** the Radar's FYI headlines to expose one unambiguous control **so that** focus order and announcements are predictable.

**What it is.** `src/components/radar/FyiItem.astro` puts the headline `<a>` inside the `<details>`'s `<summary>`. `<summary>` is itself an interactive control, so this is axe's `nested-interactive` (serious). The existing `onclick="arguments[0].stopPropagation();"` on that link is the original author working around the same collision — it stops a headline click from toggling the disclosure, which is the symptom, not the cause.

**Why it is not fixed inline.** The two affordances genuinely conflict: today a headline click **navigates** to the source and the chevron/row **expands**. Resolving it means picking one — move the source link into the expanded body (headline becomes toggle-only), or split the toggle into its own button beside a plain link. Both change how every FYI item behaves, which is a product decision, not a test fix. Same reasoning that routed the `/brand` contrast findings to BL-096 rather than deciding them inside a touch-target change.

**Note on detectability**: CI cannot see this. Without `MCP_KEY_WEBSITE_RADAR` the feed renders empty and no FYI item exists, so the baseline entry in `accessibility.test.ts` exists to keep the suite runnable against real content — it is not a ratchet that can meaningfully decrease. Reproduce with `npm run radar:stub`.

#### Acceptance Criteria

- [ ] Ruling on the interaction: does a headline click navigate, expand, or both-via-separate-controls?
- [ ] `nested-interactive` is zero on `/hub/radar/` when scanned against real feed content
- [ ] The `stopPropagation` workaround is gone rather than retained alongside a fix
- [ ] The `KNOWN_SERIOUS` entry for `nested-interactive` is removed, not lowered
