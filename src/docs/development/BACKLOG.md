...
- [ ] **Chase the two `503`s the dashboard is now showing.** The 2026-09-09 probe run (BL-159) read `trial_signup` as `minted: 1`, `bad_request: 1`, `unavailable: 2`, with the `unavailable` pair carrying `status_code: 503`. So **two of the three non-client-error signup attempts since go-live faulted**, which is exactly the failure mode the `trial_signup` event was added to make visible — and it worked. What it does **not** do is say which dependency broke: an unbound secret, Upstash, and Turnstile all surface as 503 here, so this needs the Worker logs (or a `wrangler tail` during a repro), not another panel. Worth doing before the alert rule above, since the rule's threshold should not be set against a baseline that includes a live fault
...
- **`trial_signup` shows `unavailable: 2` with `status_code: 503`, against `minted: 1`.** That is the panel doing exactly the job it was built for — the failure mode that is otherwise invisible without a live `wrangler tail`. Two attempts returned 503. A 503 alone does **not** say which dependency faulted — an unbound secret, Upstash, and Turnstile all land here — so closing it needs the logs, not this panel. The fourth event was a client-side `bad_request`, which is a different thing again. Filed against BL-155 below, whose surface it is.
