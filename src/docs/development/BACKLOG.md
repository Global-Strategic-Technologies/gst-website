...
**Source**: operator directive 2026-09-04 — add Spanish (`es`) and Brazilian Portuguese (`pt-BR`) to the website; the UX is being designed in Claude Design and will be supplied separately; this item owns the software architecture (routing, translation storage, extensibility, SEO, preference persistence, detection) and the decision record. Dialects and regions are a design constraint on this item, **not** a deliverable of it | **Effort**: ~2–3 weeks engineering for the architecture plus Tier A content; translation effort separate and per tier (see Slice 3) | **Status**: Open · Blocked on the Claude Design UX for the switcher and the suggestion banner only — every other slice can start
