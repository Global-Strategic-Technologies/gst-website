...
If the posters survive as PNGs, they are the heaviest part of the set at `621 KB` of the `948 KB`. Converting to WebP at quality 82 measured `110 KB` — an 82% reduction — and was checked at full size against the smallest text in the set (the regulations-query tool labels) with no visible loss. `poster` accepts WebP in every browser that matters.
...
- [ ] The media files committed, with a message naming `/hub/mcp/get-started/` — they have no consumer until the page lands, so an unused-asset sweep would otherwise remove them. **Six as of 2026-08-19** (three mp4 + three posters, `948 KB`), down from nine when the WebM twins were dropped. Do not treat six as fixed — see the poster note below
