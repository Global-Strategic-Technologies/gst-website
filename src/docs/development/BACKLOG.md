...
- [x] **UAT-07.6 / UAT-09.9** — ✅ Pass on `0.49.0`, the last case in the suite to run. `irlSource: partner-paste-verbatim-prepop` with `hashBindResult: pass-bound` and 37/37 claims verified; no `map-absent` and no `provenance-gap` in (J). Settles the cycle-3 question: `-prepop` is the strongest provenance form, not a loophole — the server hashes and caches the operator bytes at render time, and its own gap logic concurs by not auto-appending a reconstruction entry
