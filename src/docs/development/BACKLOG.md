...
**The tidiest statement of the whole finding is a `.describe()` pair.** `:285` (routing — correct, carves the both-case) and `:313` (Section-09 framing — no carve-out) sit in the same file, ship in the tool schema on every run and to every client that renders schema descriptions — the assumption the `.describe()` forcing-function pattern already rests on — and only one of them handles a framework that is named in Section 09 _and_ is a named trigger. That is where the clause belongs.
