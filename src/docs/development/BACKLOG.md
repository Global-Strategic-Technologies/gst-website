...
- [ ] **The two trial timing params are registered as GA4 custom metrics** (BL-164, shipped in the client 2026-09-17): `duration_ms` and `mint_ms` on `mcp_trial_signup` / `mcp_trial_refused`, scope Event, unit Milliseconds — Admin → Data display → Custom definitions. **Operator step, and it is order-dependent**: registration is NOT retroactive, so anything collected before it is unreportable, and the params only start arriving once the instrumentation is deployed to production. Verify in DebugView from one real signup before the campaign drives traffic. Worth doing `locale` as a custom dimension in the same visit — outstanding since BL-153. Until this is done, BL-164's "re-measure `LONG_VERIFY_MS` at n ≥ 10 mints" trigger **cannot fire**
- [ ] **Abandonment stays invisible, and the conversion count understates demand because of it** (carried forward from the closed BL-164 stanza). `mcp_trial_signup` counts signups that settle and `mcp_trial_refused` counts refusals the Worker issued; a visitor who starts the flow and closes the tab mid-verification is indistinguishable in the funnel from one who never clicked. So `mcp_trial_signup` is a **lower bound** on trial demand, by however many people gave up — and the BL-164 measurement makes that concrete rather than theoretical, since ~12 s of a 15 s signup is spent in the Turnstile solve. Read campaign lift with that in mind; closing the gap would need a start-of-flow event, which is deliberately not scoped here
- [ ] **A two-week pre-blitz baseline is captured** (sessions, `/hub/mcp/*` views, the conversion set) so "lift" has a denominator. Zero inbound is itself the baseline — record it as a number, not an adjective
