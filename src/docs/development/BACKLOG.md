...
- [x] `assess_infrastructure_cost_governance` — wraps the ICG engine; input includes the `answers` map and an optional `companyStage`; output is `{ overallScore, maturityLevel, domainScores[], showFoundationalFlag, recommendations[], answeredCount, totalQuestions, skippedCount }`. Field names are canonical to the engine; full reference in [`mcp-server/src/docs/tools/icg/CONTRACT.md`](../../../mcp-server/src/docs/tools/icg/CONTRACT.md)
- [x] `compute_techpar` — wraps the TechPar engine; input is `TechParInputs` (14 fields); output is `TechParResult`. Full reference in [`mcp-server/src/docs/tools/techpar/CONTRACT.md`](../../../mcp-server/src/docs/tools/techpar/CONTRACT.md)
- [x] `estimate_tech_debt_cost` — wraps the Tech Debt engine; **input MUST be raw values** (team size, salary, maintenance burden, deploy frequency, etc.) — slider-position helpers stay on the website side. Full reference in [`mcp-server/src/docs/tools/tech-debt/CONTRACT.md`](../../../mcp-server/src/docs/tools/tech-debt/CONTRACT.md)
- [x] `search_regulations` — facet/search across the regulatory-map JSON files; input `{ jurisdiction?, category?, query?, limit? }`; output includes the resource `uri` for each matched framework. Full reference in [`mcp-server/src/docs/tools/regulatory-map/CONTRACT.md`](../../../mcp-server/src/docs/tools/regulatory-map/CONTRACT.md)
...
- Diligence Machine input contract authored at `mcp-server/src/docs/tools/diligence/CONTRACT.md` — 13 fields, valid enums, downstream-effect summaries, hidden-semantics callouts (multi-region auto-sync, ordinal bracket comparison)
- Contracts registry at `mcp-server/src/docs/tools/README.md` — what-is-an-input-contract narrative, table of all known Hub-tool contracts (diligence today; ICG / TechPar / Tech Debt / Regulatory Map / Portfolio listed as `⏳ BL-031.5`), ~10-line IRL forward-look
- Cross-references wired from `mcp-server/src/docs/tools/diligence/USAGE.md`, `mcp-server/README.md` Tool Inventory, `src/schemas/diligence.ts` top-of-file comment, `src/docs/README.md` Quick Navigation
...
- [x] `mcp-server/src/docs/tools/diligence/CONTRACT.md` — full contract for `generate_diligence_agenda`. Each of the 13 fields has: identifier, display label (from `wizard-config.ts`), subtitle, valid-values table (id / label / description), 1-3 line downstream-effect summary, cardinality / hidden semantics where relevant
...
- [x] `mcp-server/src/docs/tools/README.md` exists with three sections: "What an input contract is", "Why the contract is its own artifact", "The contracts registry table"
...
- [x] `mcp-server/src/docs/tools/diligence/USAGE.md` — schema-mapping table linked to the new `CONTRACT.md`
...
- [x] `src/schemas/diligence.ts` — top-of-file comment block (4-6 lines) pointing to `mcp-server/src/docs/tools/diligence/CONTRACT.md` as the human-readable reference. No schema changes. **Bonus:** the four BL-031.5 schemas (`icg.ts`, `techpar.ts`, `tech-debt.ts`, `regulatory-map.ts`) gained the same top-of-file comment pointer as part of their respective BL-031.5 commits.
- [x] `src/docs/README.md` — Quick Navigation row "Understand a Hub tool's input contract" → `mcp-server/src/docs/tools/README.md`
...
- [x] **Cross-tool concept glossary** added to `mcp-server/src/docs/tools/README.md` as transitional artifact — flagged the funding-stage variance between ICG (`pre-series-b` / `series-bc` / `pe-backed` / `enterprise`) and TechPar (`seed` / `series_a` / `series_bc` / `pe` / `enterprise`); carried a "Will be superseded by [BL-031.87](#bl-03187-mcp-server--stage-taxonomy-adapter-layer)" note. Glossary subsequently retired by BL-031.87 closure (May 2, 2026)
...
- Glossary in `mcp-server/src/docs/tools/README.md` updated to reflect the canonical layer; "Will be superseded by BL-031.87" transitional note retired
...
- [x] `mcp-server/src/docs/tools/README.md` glossary section updated to reflect the canonical layer; "Will be superseded by BL-031.87" note retired
...
7. ✅ **Soak findings triaged via [BL-032.25](#bl-03225-mcp-revisions-prior-to-go-live--closed-2026-07-14-bucket-empty)** — all P0 (blocks-Go-Live) items closed with verification evidence; P1 items recorded for post-launch follow-up. New step added 2026-05-06 to formalize the soak → triage → deploy gate. (✅ 2026-05-12 — zero P0 items in BL-032.25; § 5 closed risk-accepted, § 1-4 P1-deferred per established convention; bucket fully closed 2026-07-14)
8. ⏳ `wrangler deploy --env production` — gated on soak completion + step 7 closure (~2026-05-13)

---

### BL-032.25: MCP Revisions prior to Go-Live ✅ CLOSED 2026-07-14 (bucket empty)

**Source**: BL-032.25 — bucket for soak-week findings discovered during BL-032 § B.5 (staging soak window 2026-05-06 → 2026-05-12). Items here are triaged as **P0** (blocks Phase 6 § B.6 production deploy) or **P1** (filed for post-launch follow-up). | **Architecture & plan**: [MCP_SERVER_REMOTE_BL-032_25.md](MCP_SERVER_REMOTE_BL-032_25.md) | **Effort**: variable per-item; close-out summary in the sibling doc's "Implementation order and execution plan" section | **Status**: ✅ **CLOSED 2026-07-14 — bucket empty.** Substrate shipped 2026-05-12; zero P0 ever open post-ship. § 1 closed rejected 2026-07-14 (deferral lifted same day by BL-032.75 Phase 2 close; 2-4h benchmark-audit spike executed → finding A, collapses by-design, adapter stays); 2026-05-13 close-out wave: § 2 closed `e97650d`, § 3 closed inconclusive, § 4 closed `170f1d0`; § 5 closed risk-accepted 2026-05-12. The one-week post-deploy review window (2026-05-12 → ~2026-05-19) surfaced no new findings | **Depends on**: BL-032 (substrate now shipped in production)
...
- [x] **§ 1 — Schema normalization across Hub Tools (CLOSED REJECTED 2026-07-14)** — investigated retiring [BL-031.87](#bl-03187-mcp-server--stage-taxonomy-adapter-layer)'s adapter pattern by normalizing the underlying schemas. **Severity: P1**. Investigation 2026-05-06; revised same day after operator clarification that URL backward-compat is NOT a business requirement; **2026-05-13 post-Go-Live re-confirmation** verified the BL-031.87 adapter is shipped and operationally stable. **2026-07-14: the 2-4h benchmark-audit spike executed** the same day its deferral gate lifted (BL-032.75 Phase 2 baselining closed via PR #309) — operator judged a Claude-prepared dossier covering all three collapse points (ICG `pre-series-b` ← seed+series-a, ICG `series-bc` and TechPar `series_bc` ← series-b+series-c). **Verdict: finding A — all three collapses are by-design.** Both benchmark datasets are expert-curated bands with no finer-grained underlying sample, and the public benchmark corpora they derive from (SaaS Capital, KeyBanc, OpenView/High Alpha, Bessemer) segment by ARR band / funding type, never by Series B vs C — normalization would fabricate benchmark boundaries the data doesn't support. Adapter stays; BL-031.87 remains Closed (not Superseded); no normalization initiative filed. Full closure stanza: [MCP_SERVER_REMOTE_BL-032_25.md § 1](./MCP_SERVER_REMOTE_BL-032_25.md#closure-stanza-2026-07-14)
...
**Source**: BL-034 — rolling cleanup catch-all for the MCP server doc surface | **Effort**: 0.5-1 day (executed at the end of the BL-031.x / BL-032.x / BL-033 sequence) | **Status**: ✅ Substantially complete (2026-07-02) — doc restructure, transitional-scaffolding delete, ADR audit, enumParity 7/7, and accumulated-bullet resolution all shipped. One open thread: the TechPar `exitMultiple` fix rides a dedicated PR. BL-033 (external pilot) remains open upstream but does not gate this cleanup. | **Depends on**: BL-031.75, BL-032, BL-032.5, BL-032.75, BL-033
...
- Transitional sections in `mcp-server/src/docs/tools/README.md` removed (precedence rule + AC-authoring convention)
...
- [x] **DONE (2026-07-02)**: `mcp-server/src/docs/tools/README.md` § "Transitional notes (remove when BL-034 closes)" — the precedence rule and the AC-authoring convention sections — DELETED in full. The convention has matured; the registry doc now ends at "How to add a new per-tool contract"
- [x] **DONE (2026-07-02)**: Verified no other doc references the deleted "Transitional notes" section by anchor (grep + repo-wide link checker, zero references)

**ADR-vs-living-doc audit**

- [x] **DONE (2026-07-02)**: Every per-tool `CONTRACT.md` cross-referenced against its schema. Drift is now **automated for all 7 contracts** via `enumParity` frontmatter (previously 3/7) — diligence (13 enums), portfolio, regulatory-map, and techpar opted in; contract-parity Vitest is 36/36 green. regulatory-map's category enum was refactored to a `REGULATION_CATEGORY_VALUES` const so it could be parity-checked; techpar + portfolio gained template-compliant valid-values tables
- [x] **DONE (2026-07-02)**: Architectural-decision docs under `src/docs/development/MCP_SERVER_*.md` audited (impartial agent sweep). Finding: **essentially clean** — every ADR reads as a properly date-stamped point-in-time record with explicit closure/status markers; no stale present-tense claims about current code. Path references to the moved docs were mechanically updated so links stay alive without rewriting frozen prose

**MCP server doc structure**

- [x] **DONE (2026-07-02)**: Restructured `mcp-server/src/docs/` into `tools/` `resources/` `prompts/`. Moved **all 7** per-tool docs (`diligence/`, `icg/`, `portfolio/`, `radar/`, `regulatory-map/`, `tech-debt/`, `techpar/` — the AC named 5; portfolio + radar were added since it was written) into `tools/<tool>/` via `git mv` (history preserved). Renamed `contracts/README.md` → `tools/README.md`. Authored `resources/README.md` (URI taxonomy documenting the real Library / Regulations / Radar families) and top-level `mcp-server/src/docs/README.md` navigator. Updated all ~150 tool-doc relative links + ~170 path-strings across `mcp-server/README.md`, `src/docs/README.md`, `src/schemas/<tool>.ts` comments, the high-risk `mcp-server/src/schemas.ts` `.describe()` string, and planning artifacts. `contract-parity.test.ts` `CONTRACTS_DIR` re-pointed at `tools/`. Zero new 404s (repo-wide link checker); pure restructure, no behavior change. `library/`, `operations/`, `testing/` left in place (not tool/resource/prompt docs)

**Items accumulated during prior initiatives** (append as they emerge)

- [ ] Library content-source convergence — if/when an Astro content-collection migration happens, replace the parallel-canonical `.md` digests with the unified source. **Re-verified still-deferred (2026-07-02)**: no Astro content-collection migration has happened; `library-loader.ts` still loads codegen'd `.md` digests. Remains contingent on that migration — no action. See [BL-031.5 deviation](MCP_SERVER_HUB_SURFACE_BL-031_5.md#deviation--library-content-source-bl-0315)
- [x] **DONE — formally dropped (2026-07-02)**: Radar per-item URIs — `gst://radar/item/<id>` were deferred in BL-031.5. Decision: **not** a Resource family. Cached item IDs churn on every `npm run radar:seed`; the `search_radar_offline` tool returns items directly. Documented in [`mcp-server/src/resources/radar.ts`](../../../mcp-server/src/resources/radar.ts) and the new [`resources/README.md`](../../../mcp-server/src/docs/resources/README.md) § "Not a Resource"
- [x] **DONE (silently resolved; verified 2026-05-26)**: Portfolio per-tool `CONTRACT.md` exists at [`mcp-server/src/docs/tools/portfolio/CONTRACT.md`](../../../mcp-server/src/docs/tools/portfolio/CONTRACT.md) (authored 2026-05-03 per the doc's `lastAuthored` frontmatter); the README link at `mcp-server/README.md:57` resolves. The "broken link" framing was stale at filing time. `USAGE.md` is not currently per-convention required for any tool — closing the bullet retroactively. If a future need emerges, file separately
- [x] **DONE 2026-05-02 (BL-031.95 Phase 3.B)**: `search_radar_cache` `CONTRACT.md` authored at `mcp-server/src/docs/tools/radar/CONTRACT.md` as part of BL-031.95 Phase 3 closure. Mirrors the website's single-filter surface (`category` only); documents the capability-mirror invariant explicitly so the future live `search_radar` (BL-032) inherits the same discipline. Earlier framing — "planned alongside live BL-032" — was wrong: the cached tool earns its own contract because it has its own user-facing semantics, and the live tool will get its own contract that compares/contrasts with this one
...
- [x] **DONE — graduated (2026-07-02)**: **IRL generator scoping spike**. The spike graduated into a full initiative and **shipped**: `generate_information_request_list_xlsx` + `prepare_irl_body` + `validate_irl_provenance` (BL-043) and the fillable-form generator (BL-044), registered in `mcp-server/src/server.ts`, with the `gst_information_request_list` prompt and `gst://library/information-request-list` Resource. Closing the placeholder bullet — the "strategic destination of the contracts pattern" is live
...
- [x] **DONE — decided: keep as-is (2026-07-02)**: **Wizard / API symmetry follow-up**. Decision at end-of-sequence: **option (a) — keep the intentional asymmetry and rely on the doc**. The sparse-`answers` acceptance is deliberate (different surfaces, different completion semantics), already documented in [`icg/CONTRACT.md`](../../../mcp-server/src/docs/tools/icg/CONTRACT.md) hidden-semantics and pinned by the `answeredCount` regression test; `compute_techpar`'s `null`-on-zero return is likewise documented. No code change — the API's flexibility is a feature for programmatic/offline callers (e.g. the IRL flow), and forcing wizard-style completeness would remove that. Re-open only if a concrete consumer is misled in practice
- [x] **DONE 2026-07-09 (PR #287, commit `32add65c` — "fix(techpar): reset exit-multiple when stage drops below PE/Enterprise"; ticked in the 2026-07-14 truth-pass)**: **TechPar `exitMultiple` UX gap** (discovered during BL-031.5 V2 verification trial 1; confirmed still present, re-verified 2026-05-26): the wizard's exit-multiple input is conditionally rendered — only visible when stage is `pe` or `enterprise`. At [`src/utils/techpar-ui.ts:67-68`](../../../src/utils/techpar-ui.ts#L67-L68) (line numbers shifted from the original `:65-67` citation): `const showExit = tp.stageKey === 'pe' || tp.stageKey === 'enterprise'; g('exit-field')?.classList.toggle('tp-exit-field--vis', showExit);`. The visibility toggle is CSS-class-only — the underlying state `tp.exitMultiple` and the DOM input value persist when the stage drops below PE. The stage-change handler at [`techpar-ui.ts:200-217`](../../../src/utils/techpar-ui.ts#L200-L217) sets `tp.stageKey` and re-renders but does NOT reset `tp.exitMultiple`. **Result**: a user who sets `exitMult=15` while on Enterprise and then switches to Series B-C silently carries 15× into their results, URL state, and any downstream calculations, with no UI to inspect or modify. Note: in scenarios where `gap.cumulative36 = 0`, the value has no observable output impact; in scenarios where cumulative excess is non-zero, the silent persistence directly affects `gap.exitValue`. Original options: (a) reset `exitMultiple` to its default when stage drops below PE, (b) show the field at all stages with stage-appropriate guidance, (c) add a "current state" inspection panel that exposes hidden values, or (d) document the behavior as intentional. **Decision (2026-07-02): option (a)** — reset `exitMultiple` to its default on stage change below PE. Fixed in a **dedicated PR** (separate from the BL-034 doc-cleanup PR so a behavioral change doesn't ride inside a docs restructure), with a unit test asserting the reset.
...
#### The discipline (as specified — shipped as `ENVELOPE_PRECHECK_DIRECTIVE`)
...
#### Closure (2026-07-10 truth-pass)

Shipped **2026-06-04** — same day it was filed — at mcp-server **0.14.0 / prompt 0.6.0** (PR #220, `c59b042e`): `ENVELOPE_PRECHECK_DIRECTIVE` landed in both verbose body shapes exactly per the spec above, plus a stopping cap the spec lacked (converge to ≥90% coverage OR 4 precheck iterations, whichever first). Arg-shape fix `{filledIrl, claims}` → `{filledIrl, citations}` + schema-prompt consistency guard followed the same day at 0.15.2 / prompt 0.6.2 (PR #224, `f91d6294`).

The directive became load-bearing infrastructure for the rest of the family: BL-056/BL-058 added the `precheck` observability block (iterations / attemptsTotal / outcome / errorsEncountered) keyed to this stopping rule; BL-071 made its counts server-arithmetic (`precheck.iterations MUST equal serverToolCallCounts.validate_irl_provenance.succeeded`); BL-079 Part A made each precheck iteration body-by-hash (`{irlBodyHash, citations}` — the ~5KB-per-iteration estimate above dropped to a 16-hex hash + citations). `selfCorrectionCalls: 0` with `totalEnvelopeCalls: 1` is the documented healthy target state in the current prompt.

### BL-052: `gst_irl_ingestion` — BL-045-VERIFY Block Schema Clarity (Cumulative vs Final-Response Counts) ✅ CLOSED 2026-06-04 (shipped at mcp-server 0.15.1 / prompt 0.6.1; remainder superseded by BL-071 + BL-062; stanza truth-passed 2026-07-10)

**Source**: BL-049 v12 StoreForce live exercise (2026-06-04) — the rendered BL-045-VERIFY block reported `selfCorrectionCalls: 0` and `totalEnvelopeCalls: 1`, but the operator-observed reality was 3+ envelope-call attempts across the session (the model iterated, hit perceived throughput hangs, the operator restarted Claude Desktop twice, and the model finally shipped on the third successful call). The block's count fields are honest only for the SHIPPING-RESPONSE envelope call, not for the cumulative workflow. The verification-protocol intent in BL-049 § Verification Protocol is cumulative ("selfCorrectionCalls > 0 means the workflow needed self-correction"); the prompt-body directive doesn't disambiguate clearly enough so the model rendered final-response counts. Secondary observation from the same trace: `conditionalTriggersFired: []` was reported empty even though the IRL Section 09 explicitly named EU AI Act applicability (and earlier in-progress envelope calls reported `EU_AI_ACT` correctly). Both are reporting-discipline issues, not architectural failures, but they erode the verify block's value as an audit artifact. | **Effort**: ~20 min (prompt body directive tightening + body-hash rebaseline) | **Status**: ✅ **CLOSED 2026-06-04** — fix #3 shipped same-day at mcp-server 0.15.1 / prompt 0.6.1 (`d3ca4b87`); fixes #1/#2 were later solved structurally by BL-071 + BL-062 (see Closure below); stanza sat at "Open" until the 2026-07-10 truth-pass. | **Depends on**: BL-049 merged (v0.13.0).
...
Shipped **2026-06-04** at mcp-server **0.15.1 / prompt 0.6.1** (`d3ca4b87`): fix #3's `meaningfulRecallsHaveDifferentInputs: <bool|null>` field landed in the BL_045_VERIFY_DIRECTIVE + interactive Step 5 block, with the post-BL-051 healthy-target framing (`0 → null`; `>0 + true` → healthy in-flight refinement; `>0 + false` → transport thrash — the operator diagnostic this stanza asked for).

Fixes #1 and #2 (directive-tightening for cumulative counts and trigger preservation) were **superseded by structurally stronger solutions**: BL-071 made the counts server-arithmetic — `serverToolCallCounts` is a server-side snapshot returned by `compose_dossier_envelope`, and the prompt's derivation identities (`precheck.iterations MUST equal ...succeeded`) replace model working-memory bookkeeping entirely, which is what fix #1's directive prose was trying to approximate. BL-062 disambiguated the conditional-trigger reporting (`considered` vs default-fired) that fix #2 targeted.

### BL-053: `compose_dossier_envelope` — Citation Array Form (Multi-Bullet Citation Support) ✅ CLOSED 2026-06-04 (shipped at mcp-server 0.15.0; follow-up coaching at 0.16.0 / prompt 0.7.0; stanza truth-passed 2026-07-10)

**Source**: BL-049 v12 StoreForce live exercise (2026-06-04) — the model's natural citation style for derived claims (TechPar verdicts, comparable engagements, Tech Debt syntheses) is to cite MULTIPLE supporting IRL bullets joined into a single citation excerpt with semicolons or "and". The verifier substring-matches against the canonical body, so a multi-bullet-summary citation NEVER substring-matches (it's a synthetic concatenation, not text that appears anywhere in the body). The model's only structurally-valid options are: (a) re-cite using ONE verbatim bullet substring (losing the multi-source attribution), (b) demote to tier-2 with `Section --` partner-supplied sentinel (loses the IRL grounding signal), or (c) accept the unverified flag (transparent but inflates the auto-gap count). All three lose information vs. the model's genuine intent of "this claim is supported by bullets X, Y, AND Z." The schema-level fix is to let `citation` be `string | string[]`; the verifier check passes if all elements of the array verify individually as substrings of the body. | **Effort**: ~2 hours (schema delta on `compose-dossier-envelope.ts` claimSchema; deriveTier wrapper; verifier loop; test cases for the array form; tools list update) | **Status**: ✅ **CLOSED 2026-06-04** — shipped same-day at mcp-server 0.15.0 (PR #221, `c0dae854`) with follow-up coaching at 0.16.0 / prompt 0.7.0 (PR #225, `0bfb5c6b`); stanza sat at "Open" until the 2026-07-10 truth-pass (see Closure below). | **Depends on**: BL-049 merged (v0.13.0); composes with BL-051 (the precheck-iteration workflow benefits from this schema flexibility).
...
Shipped **2026-06-04** at mcp-server **0.15.0** (PR #221, `c0dae854`) exactly per the schema/verifier deltas above, on BOTH tools — `compose_dossier_envelope.claims[].citation` AND `validate_irl_provenance.citations[].citation` take the `string | string[]` union (the spec only named the envelope; extending the fast verifier keeps the BL-051 precheck loop consistent), with an added 1–8 element cap on the array form and strict aggregation (any element unverified → aggregate unverified; all verbatim → verified; mixed verified/fuzzy → verified-fuzzy; all partner-supplied → partner-supplied). The shared `citationFieldSchema` now lives in `src/schemas/validate-irl-provenance.ts` and is imported by the envelope schema.

Same-day follow-up at **0.16.0 / prompt 0.7.0** (PR #225, `0bfb5c6b`) added array-form coaching to the prompt body, tier-discipline message branching, and a transport-roundtrip test. BREAKING_CHANGES stanzas exist for both releases.
