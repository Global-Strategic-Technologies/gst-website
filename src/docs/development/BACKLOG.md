...
And the coupling is tighter than "somewhere in the body". Both notes live **inside `RUN_AUDIT_DIRECTIVE` itself** (`:625`), the same block that instructs the model to transcribe the counts — rendered at `:1024` in the full body (gated on `showRunAudit`, so `debug` only) and at `:1135` in extract-only, with the interactive builder carrying its own copy at `:1338`. **A model that has counts to report has necessarily just read the two notes explaining them**, because the notes and the transcription instruction are the same list.

So there is no missing clause to add. **The contract states both facts, adjacent to the instruction that surfaces them, and the model reports them as bugs anyway** — which makes this a placement and salience problem rather than a coverage one, and a genuinely harder one. The honest options are to move these two notes to where the counts are actually transcribed, or to decide the cost is acceptable and stop treating each report as new. Either way the dedupe is what makes the decision implementable once instead of twice.
