...
**DROPPED 2026-08-15 — won't-fix by operator decision, and this paragraph scopes ONLY the `engCost` disclosure gap above; the `infraHostingAnnual` finding that follows stays open.** _"No one will notice it."_ The residual is ~4% on `rdOpEx` with the zone verdict stable, and the disclosure work it would take is not worth that. Recorded so a met trigger stops reading like scheduled work — including the "the shape of the fix is already in the file" line above, which describes a fix that will not be built.
