...
So there is no missing clause to add. **The contract states both facts, in the body the model is reading, and the model reports them as bugs anyway** — which makes this a placement and salience problem rather than a coverage one, and a genuinely harder one. The honest options are to move these two notes to where the counts are actually transcribed, or to decide the cost is acceptable and stop treating each report as new. Either way the dedupe is what makes the decision implementable once instead of twice.
