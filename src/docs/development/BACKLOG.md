...
**That trigger is now met, and the two post-fix sweeps of 2026-08-15 isolate it to a single number.** Both runs took `estimated-from-headcount` for `engCost` and both applied `ENG_COST_DEDUP_RULE` correctly — 42 total engineering less the 9-person Infra/DevOps/DBA group. Both results decompose exactly: **$3,630,000 = 33 × $110,000** and **$3,795,000 = 33 × $115,000**. Identical headcount basis, a rate that moved by $5,000, and **the branch has nowhere to declare either term** — `estimated-from-headcount` requires `annualizationSource` and a citation, and the module JSDoc documents it as _"derived from team × salary"_ while asking for neither factor. IRL Section 07 states base-salary bands and says in terms _"base salary only (not fully-loaded)"_ against a schema field that is a fully-loaded cost, so a markup was applied twice and named neither time.

**A second, cleaner instance on `infraHostingAnnual`, where the divergence is the method itself.** Run 1 took `ytd-annualized-with-period` (`ytdMonths: 3`, `ytdMathCheck` anchoring $292,000/mo against $850,000 YTD) and derived $850,000 ÷ 3 × 12 = **$3,400,000**. Run 2 took `monthly-x12` on the same monthly anchor: $292,000 × 12 = **$3,504,000**. Both are arithmetically exact, both cite real IRL figures, and the enum offers both paths with nothing stating which applies when the IRL supplies a monthly figure _and_ a YTD figure. **Note what this proves about audit strength**: run 1 used the strongest branch in the schema and passed its handler-verified cross-check (292,000 × 3 = 876,000 against 850,000 reported, 3.1% — inside the 10% tolerance). The check confirmed the arithmetic and was structurally unable to say the method was the wrong one to pick. **A cross-check validates a derivation; it cannot adjudicate between two valid derivations.** Any fix here needs a selection rule, not more validation.

**Also observed**: run 2 supplied `_audit.rdCapEx: { annualizationSource: "irl-annualized-stated" }` for a zero value under `capexView: "gaap"`, where the engine excludes the field from `total` entirely. `TECHPAR_MODE_RULE` sanctions the placeholder escape for `rdOpEx` and calls it _"the one sanctioned exception"_; the model generalized it to a second field unprompted. Evidence that a sanctioned exception to a rule the enum makes unsatisfiable does not stay contained — which is an argument for the absence value, not for tighter wording.
