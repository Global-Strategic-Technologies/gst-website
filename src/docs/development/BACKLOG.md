...
- [ ] `search_radar_cache` `CONTRACT.md` — the README marks its contract as "planned alongside live BL-032 `search_radar`". Authoring belongs to BL-032 (MCP Server — Internal Remote, Phase 2) when the live tool ships, not BL-034; tracked here only so the gap is not lost between initiatives
