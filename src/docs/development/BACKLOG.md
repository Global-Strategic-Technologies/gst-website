...
- [ ] conventions.md teaches the hub-tool chrome (`.tool-action-bar` family, wizard stepper, tabs, methodology, bench-note) and ToolShellSpecimen shows them in place
- [ ] Every block named in the class table lists its required BEM children — `option-card`, `project-card`, `search`, `breadcrumb`, `bench-table`, `panel`, `field` (full set) join the table
- [ ] The absent reusable families are added (`brutal-tab`, `brutal-slider`, `brutal-teaser-card`, `brutal-faq`, `brutal-shadow`, `brutal-filter-drawer`, `toc`, `brutal-stat`/`brutal-cta`, `--copied`, `.no-print`, `.skip-nav`), each with a specimen row or a copyable snippet; regulatory-map one-offs explicitly excluded with a sentence saying so
- [ ] Token table gains the semantic hub colors, the dataviz scales, the dark-side literals and the omitted z/accent tokens; the implicit rules are written down — radius 0, the focus-ring recipe, the letter-spacing scale, `.container` 1600/3rem, the 1024px tier, and the reduced-motion posture (whichever way that ruling goes)
- [ ] The header stays under the converter's 32,000-char inline-README truncation with headroom (it cuts the tail; conventions.md is ~12 KB today) — if the vocabulary outgrows it, the overflow moves to a shipped guideline doc rather than being dropped

**Slice 3 — publish the chrome by extraction, not by hand (plan first)**

- [ ] A build step renders `/brand` (`astro build` → `dist/brand/index.html`), slices it per specimen section (`h3[id]` anchors — the page's TOC already keys on them), and emits each slice as a static `@dsCard` HTML card carrying the rendered production markup plus the scoped CSS it needs — mechanism 1 by construction: nothing hand-written, nothing to drift
- [ ] Header / Hero / Footer / StatsBar / CTABox / Breadcrumb / the section-card families are browsable in the Design project and named in conventions.md as "copy the rendered markup" patterns
- [ ] Because the card scaffold is now ours, a **dark-mode card** exists for at least the chrome set (the converter's `body{background:#fff}` limit no longer applies to cards we author)
- [ ] The extraction is wired so it re-runs on re-sync and fails loudly if a section anchor disappears; CLAUDE_DESIGN_SYNC.md and NOTES.md describe the second card path alongside the React-specimen path
- [ ] **Decision recorded, not assumed:** whether any scoped chrome should be _promoted_ into the published system (Hero headline recipe, `.site-header` rules, the `.trust-card`/`.brutal-trust-card` and `.breadcrumb__*`/`.brutal-breadcrumb` duplications). That is a website design-system change with its own benefit and its own regression risk, not a sync change — if taken, it is filed as its own item and cross-linked here; if declined, this stanza says why
