...
**Source**: operator directive 2026-07-15 — `src/docs/development/` had accumulated 39 flat files (~2.4 MB) where 32 closed-initiative narratives drowned the 7 living reference docs, while load-bearing design rationale existed only inside those frozen narratives (cited by ~100 path-bearing references from live code/config/tests/docs) | **Architecture & plan**: [MCP_DOCS_DISTILLATION_BL-088.md](MCP_DOCS_DISTILLATION_BL-088.md) — disposition table, link policies, PR ledger | **Effort**: ~7-9 days across 5 PR waves | **Status**: **In progress** — PR 1 (archive wave: `_archive/` scaffolding + 16 archive-only docs moved + ~30 living refs repointed + lifecycle convention codified) landed 2026-07-15; PR 2 (`mcp-server/src/docs/ARCHITECTURE.md` + 42 refs), PR 3a (`src/docs/adr/` + ADR-0001/2/3 + 19 refs), and PR 3b (ADR-0004–0007 + 27 refs) landed 2026-07-17; PRs 4-5 (maintained-doc folds + `prompts/irl-ingestion.md`, closure sweep) pending
