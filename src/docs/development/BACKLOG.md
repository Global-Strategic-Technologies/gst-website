...
- [ ] **Diagrams are mermaid in fenced blocks**, and `README.md` states this as the convention **for this directory** — explicitly not a repo-wide ruling, and explicitly not a call to convert anything. The repo is not a blank slate: mermaid, SVG, `.mmd`, `.drawio` and `.puml` are all absent, but **ASCII box-drawing diagrams already exist in 34+ markdown files** (`ARCHITECTURE.md`, `DEVELOPER_TOOLING.md`, `JSON_LD_SCHEMA.md`, `TEST_STRATEGY.md` among them). This slice adds a second medium in one directory, not a first medium to the repo
