...
- [ ] **§ 2 — T.A.4 empty-bearer error message** — `Authorization: Bearer ` (empty token) returns 401 with message `"Authorization header must use Bearer scheme"` instead of an empty-token-specific message. Cosmetic error-disambiguation gap. **Severity: P1**. **2026-05-13 re-investigation** corrected the original root-cause analysis: the empty-token check already exists in `bearer.ts:75-76`, but **HTTP runtimes normalize trailing whitespace on header values**, so `Bearer ` arrives as `"Bearer"` (no trailing space) and trips the scheme check before the empty-token check is ever reached. The existing integration test (`auth.test.ts:73-89`) already documents this as correct-by-design. **Two mitigation options**: (A) close as resolved-by-design (10 min doc update; recommended); (B) ship a 5-line surgical fix that detects bare-`Bearer` and routes it to the empty-token branch (30 min). See [MCP_SERVER_REMOTE_BL-032_25.md § 2](./MCP_SERVER_REMOTE_BL-032_25.md#§-2--ta4-empty-bearer-error-message-p1) and [TESTING_FINDINGS § T.A.4](./BL-032_TESTING_FINDINGS.md#ta4--empty-bearer-schema).
- [ ] **§ 3 — T.K.2.b.3 local stdio `generate_diligence_agenda` timeout** — call hung 4 min on Claude Desktop's local stdio connector while staging completed normally. Tool-specific (other local-stdio tools worked in same session). Suspect: large JSON response overflowing stdio buffer, engine slow-path, or stdio child-process deadlock. **Severity: P1**. **2026-05-13 re-investigation** confirms no post-soak code changes in the call path (handler is byte-identical to 2026-05-10), and test coverage gap (no test exercises worst-case all-13-fields input or measures response size). Response-size estimate refined to ~15–20 KB after noting handler returns both `content[0].text` AND `structuredContent` (duplicates the wire payload). **Next action**: build a reproduction script at `mcp-server/scripts/repro-k2b3.ts` and instrument three checkpoints (engine return / Worker write / consumer finish) — recommend bundling with BL-032.75 Phase 2a's metric emitters which will surface recurrence cheaply. See [MCP_SERVER_REMOTE_BL-032_25.md § 3](./MCP_SERVER_REMOTE_BL-032_25.md#§-3--tk2b3-local-stdio-diligence-timeout-p1) and [TESTING_FINDINGS § T.K.2.b.3](./BL-032_TESTING_FINDINGS.md#tk2b3--generate_diligence_agenda).
- [ ] **§ 4 — T.X.1 secondary playbook polish** — two follow-ups from the T.X.1 placeholder-trap fix, both **confirmed with exact line refs 2026-05-13**: (a) bash equivalent at [`DEPLOY.md` § B.3 lines 435–438](../../../mcp-server/src/docs/operations/DEPLOY.md) has the literal placeholder `<your-MCP_KEY_RP-token-value>` AND still defaults `MCP_URL` to staging — bundle both fixes; (b) `Invoke-McpRequest` SSE-parser at [`Invoke-McpRequest.ps1:92-97`](../../../mcp-server/scripts/Invoke-McpRequest.ps1) silently returns the raw HTTP response on non-SSE bodies (401 JSON, 5xx JSON) — recommend Option B.2 (return a structured envelope on the non-SSE path) + B.3 (document the diagnostic incantation). Adjacent doc sweep clean (no other stale production-URL refs; the six `YOUR_TOKEN_HERE` markers in REMOTE_CLIENT_SETUP.md are intentional). **Severity: P1**. Bundled ~30-min commit. See [MCP_SERVER_REMOTE_BL-032_25.md § 4](./MCP_SERVER_REMOTE_BL-032_25.md#§-4--tx1-secondary-playbook-polish-p1) and [TESTING_FINDINGS § T.X.1](./BL-032_TESTING_FINDINGS.md#tx1--setup-snippet-placeholder-is-a-copy-paste-trap).
