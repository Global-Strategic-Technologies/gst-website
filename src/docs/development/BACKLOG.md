...
- [x] Publish the `/.well-known/mcp` + `server.json` registry metadata — shipped 2026-09-14: `GET /server.json` and alias `GET /.well-known/mcp` on the Worker (`mcp-server/src/registry-metadata.ts`, official-registry schema 2025-12-11, version from `env.VERSION`, description mirrored from the JSON-LD and test-bound both ways; [ADR-0033](../adr/0033-registry-metadata-served-by-the-worker.md)). **Operator step still open**: after the staging auto-deploy, probe both paths with the `ClaudeBot` / `Claude-User` user agents (the zone's AI-bot block 403s `/authorize`, [SEO_IMPLEMENTATION.md § Crawler access](../seo/SEO_IMPLEMENTATION.md)); if blocked, add a WAF skip rule for `/.well-known/*` and `/server.json` before any registry submission, and record the probe result in that doc either way. The other directories (MCPMarket, Cursor's catalog, the official registry) submit from the same artefact
