...
- [ ] **T2 redirect-URI provisioning** (operator-side, blocking): register `https://mcp.globalstrategic.tech/admin/inoreader/reauth/callback` against the Inoreader app in the developer dashboard. **Single registered URI per Inoreader-tier constraint** — production-only; staging shares the production Inoreader app/tokens but cannot exercise the live OAuth flow. Bind `INOREADER_REDIRECT_URI` and `MCP_ADMIN_KEY` Worker secrets via `wrangler secret put --env production`
- [ ] **T2 validation** (operator-side, blocking): after redirect-URI registration + secret binding, operator runs full re-auth from a mobile browser; verify `admin-reauth-callback-success` Sentry event appears ~1 min after Authorize tap; total elapsed time documented
