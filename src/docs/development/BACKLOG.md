...
**But where those notes render does not match where the numbers go, and that mismatch is the actual defect.** Both live inside `RUN_AUDIT_DIRECTIVE` (`:625`), which renders **only at `auditLevel: debug`** in the full body (`:1024`, gated on `showRunAudit` = `auditLevel === 'debug'`, `:847`) and in interactive (`:1283`, same gate at `:1171`). Extract-only renders it unconditionally (`:1135`) — the one mode that never calls the tool, which is the second instance above.

The numbers ship at **every** level regardless: `serverToolCallCounts` and `countersScope` are attached with no audit-level condition (`mcp-server/src/tools/compose-dossier-envelope.ts:155-157`), and the full body deliberately names both fields at every level with only the transcription rules `debug`-scoped (`:595-601`). The file states the mismatch about itself at `:606-609` — the directive "ships at every audit level, where most runs emit no block for those rules to govern."

**So it is two problems stacked, not one.** At `standard` and `enhanced` in `mode: full`, the model receives `{attempted: 5, succeeded: 4}` and `countersScope: run` with **neither note anywhere in its body** — a plain coverage gap. At `debug` the notes sit in the same list as the transcription instruction, so a model that misreads them was not short of proximity — a salience problem, and the harder of the two.

**Three options, and the strongest is not a prompt edit.** Put the explanation where the numbers are: `emitInstructions` in the tool response (`compose-dossier-envelope.ts:1370`) reaches every audit level, every body shape and every client, which no prompt-side change does. Second, rename the wire label the misread turns on — `run` reads as "session" where `irl-body` would encode the derivation; it is a client-visible enum, so it costs a migration. Third and explicitly rejected: changing the self-count semantic, which `CONTRACT.md:217` argues against directly ("consumers must not correct this value"). Deduping is what lets any of these land once instead of twice.
