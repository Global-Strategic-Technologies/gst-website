...
- [~] Specimens render the real component wherever it is renderable in isolation (cards, buttons, form controls, chips, tool shells, breadcrumbs, tiles, tables) — **slice 1 shipped 2026-08-02**: `Breadcrumb` (gained `crumbs?` + `ariaLabel?` props), `StatsBar` and `WireItem` now render the real component; `TableOfContents` and the logo components already did. **Slice 2 shipped 2026-08-08** (the AC-2 extraction): the logo lockup, header nav links, theme toggle and footer links specimens render `HeaderLogo` / `HeaderNavLinks` / `ThemeToggleButton` / `FooterLinks` — presentational inners extracted from `Header.astro` / `ThemeToggle.astro` / `Footer.astro`, whose wrappers keep the singleton ids, scripts and landmarks. All demo props follow one rule: passing the override keeps links in-page AND drops the analytics onclick, so a specimen click never emits `navigation_click` (`HeaderNavLinks`' production branch stays literal markup because the GA4 payload is not derivable from a link model — 'M&A Portfolio' label vs 'M&A' text — and no test pins it). The four parity guards died with their replicas per slice-1 precedent; the footer-fit negative margin moved to `Footer.astro` (`.footer-top :global(.theme-toggle)`), the context that needs it. The rest are deferred with reasons below — AC-1 is **not** met for them, and the sync comments added alongside are harm reduction, not AC-2 being satisfied
  ...
  - **Deferred, singleton DOM id** (cannot render twice; these keep replicas + parity guards + sync comments): `CTASection` (`id="contact"`, `CTASection.astro:24` — note its inline `trackCTA` onclick is a real click-time side effect but is _not_ the disqualifier), `ProjectModal` (19 ids), `PortfolioHeader` (22 `getElementById` lookups, owns the filter runtime), `StickyControls`, `FilterDrawer` (6 ids). `Header` / `ThemeToggle` left this list in slice 2 via the presentational-inner extraction — the same remedy applies to `CTASection` whenever its slice is cut (its extraction must also solve the `trackCTA` suppression, which slice 2's demo-prop rule now models)
...
- [x] For components that genuinely cannot be rendered twice on a page — `Header.astro` and `ThemeToggle.astro` both carry singleton `id`s (`#themeToggle`) and a bound script — either extract a presentational inner component the page and the specimen both use, or keep the replica **with** the parity E2E guard added 2026-07-28 in `brand-page.test.ts` (which compares the specimen's size, colour and gap against the live component on the same page) and a comment naming the file to keep it in sync with. **Done 2026-08-08 via the extraction branch** (slice 2, see AC-1): `HeaderLogo` / `HeaderNavLinks` extracted from `Header`, `ThemeToggleButton` from `ThemeToggle`, `FooterLinks` from `Footer` (whose replica was only ever hostage to `ThemeToggle`'s singleton). Wrappers keep id + script + landmark + sticky shell; the pattern is recorded in STYLES_GUIDE § How a specimen relates to what ships
