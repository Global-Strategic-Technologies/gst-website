...
>
>   **Design sync (BL-147's last criterion):** the local [CLAUDE_DESIGN_SYNC.md](CLAUDE_DESIGN_SYNC.md) steps passed, with 19/19 chrome cards rendering and the styling bundle regenerated. The upload to claude.ai/design needs the operator's authenticated session and was **handed to the operator** and is **done** — operator-synced (recorded 2026-09-21).
>
...
> - **2026-09-19**: 1 stanza (BL-154, five-layer architecture reference + storage-store review) closed and pruned the day it shipped. Last pre-prune revision: `893d564f`. Both slices in one PR:
>   - **Slice 1** became [ADR-0036](../adr/0036-client-records-stay-in-kv.md): the M2M client record stays in KV, the indexes it lacks live in Upstash beside it, four revisit triggers. The stanza's "no expiry primitive exists" was stale — BL-155 had added `expiresAt`, enforced at mint and reaped at +30d — and the ADR opens by answering the BL-133 question that raised it. The mint-path read was **measured against staging**, not reasoned: three new ad-hoc probe surfaces (`--surfaces`, never scheduled, unauthenticated) put an edge-cached KV read at ~10 ms and an origin miss at ~98 ms client-observed; D1 could not be measured without a binding and the ADR says so as a finding. The local shape check found that `listM2mClients` ignored `list_complete`/`cursor` and would have silently truncated a roster past one page — fixed with a paged-KV test.
...
- **Related items**: [BL-133](#bl-133-payments-platform--automated-mcp-access-checkout-on-cloudflare) (the payments rail this deliberately does not use), BL-156 (closed 2026-09-14; now [`/hub/mcp/from-code/`](../../page-templates/HubMcpFromCodePage.astro)), [ADR-0036](../adr/0036-client-records-stay-in-kv.md) (the KV-vs-relational question this forced — answered: the record stays in KV; closed BL-154 Slice 1), [BL-004](#bl-004-email-capture-system) (deliberately avoided — Turnstile is the identity gate, not email)
