...
> **Completed and closed items** are removed from this file once done — recover any stanza's full acceptance criteria and technical context via `git log -- src/docs/development/BACKLOG.md`, or consult the per-initiative design docs in [`_archive/`](_archive/README.md) (they are no longer kept in this directory — see the [initiative-doc lifecycle](README.md)). Seven cleanup waves so far:
>
...
**Source**: operator directive 2026-09-04 — the GST MCP server is officially released (the `/hub/mcp/` family plus the three onboarding guides and their six recorded clips shipped 2026-08-27 → 09-04); promote it across LinkedIn, the Claude connector directory, and Google Ads, with Google Analytics verified first so lift is measurable. Google has offered spend incentives, which puts a clock on the Ads slice | **Effort**: ~1 day engineering (measurement + privacy copy), then ~2–3 weeks of operator time in parallel with [BL-145](#bl-145-design-partner-program--set-the-price-from-evidence-not-from-a-guess) | **Status**: Open
...
- [ ] **Annotation audit**: every registered tool carries a correct `readOnlyHint` / `destructiveHint` (measured 2026-09-05: two registered tools carry **no** `annotations` block at all — `compose-dossier-envelope.ts` and `validate-irl-provenance.ts` — so this is a fix, not a precaution; verify the rest per tool, including the xlsx `fill` tool, whose write is to the response not to state). Add a contract-parity test so a future tool cannot register without one
- [ ] **Privacy policy covers the MCP server.** `src/pages/privacy.astro` currently does not mention the MCP server, the OAuth consent scopes, Upstash, Cloudflare, or Sentry as processors of connector traffic. A missing or incomplete policy is an immediate rejection; fix this before the form, not after
- [ ] **The reviewer's path is a real path.** Access is operator-gated (pre-registered clients, `MCP_KEY_*` consent flow — [ADR-0008](../adr/0008-mcp-oauth-embedded-authorization-server.md)); a reviewer and, after listing, any Claude user who toggles it on must land somewhere sensible. Decide what an unprovisioned directory user gets (the free-pilot request path? a read-only tool subset? a clear "request access" error?) and make the reviewer's test credential a **non-production** client with `--allow-radar` **off**
...
- [ ] Publish the `/.well-known/mcp` + `server.json` registry metadata that BL-093's sub-block already lists as missing — same slice, same PR, so the other directories (MCPMarket, Cursor's catalog, the official registry) can be submitted from the same artefacts
- [ ] Post-listing: `utm_source=claude-directory` is unavailable (the directory does not pass query strings), so attribution for this channel is **new OAuth consents per week** from the Worker's own telemetry, not GA4. Record where that number is read from
...
### BL-149: Diagnose the 5000ms first-use suite flake — twenty-six instances, eight named files, both workspaces, reproducible in isolation at ~37%

**Source**: seventeen single-test failures in the `@gst/mcp-server` suite between 2026-08-04 and 2026-09-08, recorded in [`CLAUDE.md` § Testing Standards](../../../.claude/CLAUDE.md) and carried as a standing caution on [BL-106](#bl-106-mcp-server--2026-07-28-spec-alignment--closed-2026-08-04) until 2026-09-02 — plus instances 18–22 on 2026-09-09, two of them in the WEBSITE suite and one on `master` itself, and instances 23–26 on 2026-09-10/11, two of them under Vitest 5 and one failing seven of the eleven Worker-booting files at once | **Effort**: Medium — the work is reproduction and instrumentation, not a fix; the fix may be small once the cause is known | **Status**: Open
...
#### Instances 23–26 (2026-09-10/11) — it survives a test-runner major, which rules out Vitest 4
...
**Instance 26 persists across the runner major on `mcp-server` too.** It happened again later on 2026-09-11, on `chore/bl-160-mcp-vitest-5`, the first `mcp-server` runs on **Vitest 5.0.0**. The default config and a `clearMocks: false` comparison config both failed files drawn from that same seven: six with the default and all seven with the override. The other four booting files passed in both. A matched isolated baseline shows it isn't the upgrade. `worker-roundtrip.test.ts` alone failed 6/6 on `master` under Vitest 4.1.11, with the first `it` taking 5294–6271ms. It failed 3/3 on the branch under 5.0.0, at 5421–7176ms. The distributions are the same, with first-`it` fetch latency straddling the 5000ms default on this machine. The first run after each `npm ci` also showed a ~4.5s `import` phase on **both** runners, from a cold cache. One earlier 1/1 `master` pass was a sample just under the threshold, not a difference. Linux CI is the verifier.
...
- **A missing teardown.** All **eleven** files that actually boot a worker call `await worker?.stop()` in `afterAll`, awaited in every case (checked file by file). Three further files mention `unstable_dev` only in comments stating they deliberately do NOT boot one — `audit-emission`, `radar-resources-worker`, `radar-snapshot-endpoint` — so the population is 11, not the 14 a grep for the symbol returns (recounted 2026-09-11; it was 9 of 12 when this bullet was first written).
