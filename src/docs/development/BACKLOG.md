...
- [ ] **T1 monitor-the-monitor — verification**: first synthetic firing observed end-to-end (Worker Logs → Sentry Issue → Slack `[SYNTHETIC]` message); first-firing date filled into [`SENTRY_ALERT_RULES.md`](../../../mcp-server/src/docs/operations/SENTRY_ALERT_RULES.md) § 4. Depends on first production deploy post-PR + first Monday 14:00 UTC tick OR operator running the § 4 force-fire procedure
