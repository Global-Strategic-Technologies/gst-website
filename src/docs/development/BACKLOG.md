...
**Source**: operator directive 2026-07-15 — `src/docs/development/` had accumulated 39 flat files (~2.4 MB) where 32 closed-initiative narratives drowned the 7 living reference docs, while load-bearing design rationale existed only inside those frozen narratives (cited by ~100 path-bearing references from live code/config/tests/docs) | **Architecture & plan**: [MCP_DOCS_DISTILLATION_BL-088.md](MCP_DOCS_DISTILLATION_BL-088.md) — disposition table, link policies, PR ledger | **Effort**: ~7-9 days across 5 PR waves | **Status**: **In progress** — PR 1 (archive wave: `_archive/` scaffolding + 16 archive-only docs moved + ~30 living refs repointed + lifecycle convention codified) landed 2026-07-15; PR 2 (`mcp-server/src/docs/ARCHITECTURE.md` distilled from the 5 BL-031/032.x docs + 42 refs repointed) landed 2026-07-17; PRs 3-5 (7 ADRs, maintained-doc folds, closure sweep) pending
