...
**Superseded twice — do not act on that last clause.** _2026-08-21_: the wrangler chain was cleared by pinning `wrangler` at an exact `4.121.0` (its miniflare resolves `undici@7.29.0`, outside the vulnerable range), **not** by merging the Dependabot dev-dependencies PR. _2026-08-22_: [BL-137](#bl-137-mcp-server-compiles-worker-and-node-code-as-one-program-so-worker-globals-type-shadow-node-ones) then lifted that pin — wrangler floats on `^4.125.0`, the undici chain was re-measured and is still clear, and the instruction to close wrangler Dependabot PRs no longer applies. See [DEVELOPER_TOOLING.md § npm audit policy](DEVELOPER_TOOLING.md) and [ADR-0020](../adr/0020-workers-types-global-shadowing-immunity.md). Through both changes, the detection gap this item is actually about remains untouched.
...
**Source**: Dependabot batch resolution, 2026-08-21 | **Effort**: Medium | **Status**: **Closed 2026-08-22** (server `0.57.0`) — the dependence was removed and the pins lifted. Durable record: [ADR-0020](../adr/0020-workers-types-global-shadowing-immunity.md)
...
**Why the pin, and not a code fix.** Patching the three call sites would have made `tsc` green while leaving the whole workspace typed against `any` for two of the most-used Node globals — trading a loud failure for a silent one. The bump was held instead: `@cloudflare/workers-types` pinned to `5.20260804.1` and `wrangler` to `4.121.0` (its peer floor forces them to move together). **Both pins were lifted 2026-08-22** — see the resolution below.

**This is not confined to the stdio path — do not dismiss it on that basis.** The natural reading of "Worker globals shadow Node globals" is that only Node-side code suffers, and since **nobody runs the stdio transport** (the team works against the remote Worker), that would make this cosmetic. It is not. `Buffer` is used in **production Worker code**: `src/lib/upstash-cache-store.ts` and `src/cache/irl-body-cache.ts` both call `Buffer.byteLength(...)` for payload sizing, and both are reached from `worker.ts` through `server.ts`; `src/config.ts` reads `process.env`. Those expressions are typed `any` under the newer workers-types, in the code path that actually serves clients. The three `tsc` errors happened to land in `src/index.ts` and two test files, which is what makes the blast radius easy to under-read from the error list alone.
...
**How it was resolved (2026-08-22).** Not by the tsconfig split this stanza previously proposed. That split was never attempted and is **not** established as impossible — it was simply more machinery than the problem needed once the actual dependency graph was measured. What shipped instead, in four parts:

1. **`Env` moved to `mcp-server/src/env.ts`.** `index.d.ts` has exactly one inbound edge — the reference directive in `worker.ts` — and the website's root program reached it through a single `import type { Env } from '../worker'` chain. Severing that took the root program from 119 `mcp-server/src` files plus the global script down to 25 files and none.
2. **`Buffer` and `process` removed from `mcp-server/src`.** Six `Buffer.byteLength` sites became `utf8ByteLength()` (`src/lib/utf8-bytes.ts`, hoisting a `TextEncoder` helper `metrics/guard.ts` had already written privately); the three `process` sites import from `node:process` **and annotate `NodeJS.Process`**.
3. **Two ESLint rules** over `mcp-server/src` and `mcp-server/tests`, covering value and type positions separately — `no-restricted-globals` skips type positions by design, so it alone reported nothing on one of the two originally-broken sites.
4. **A declaration-name allowlist test** keyed on the full top-level `declare` set of the installed `index.d.ts`, which is what replaces the pin. It failed on the very first real bump and named all eleven new declarations.

**What is proven, and what is not.** Proven: the reference directive **cannot** be removed — it is what makes `@types/node` stand down so `Request`/`Response` resolve to Workers shapes, and the `cloudflare:*` ambient modules exist only in that file. Also proven, and contrary to the obvious assumption: importing from `node:process`/`node:buffer` does **not** by itself restore `@types/node` typing, because those modules export the same global binding that got redeclared. Not established either way: whether a project-referenced split would work. Nobody tried it.

**Residual, accepted and documented**: `Buffer` value imports inside mcp-server **tests** remain `any` under the current workers-types. `mcp-server/src` touches neither global at all, which is the property the ESLint pair keeps true. Details and the revisit trigger: [ADR-0020](../adr/0020-workers-types-global-shadowing-immunity.md) § Consequences.
