...
**Source**: two consecutive production deploy failures, on unrelated commits | **Shipped**: six commits, CI only (no `@gst/mcp-server` version change)
...
5. **D3's shape reappeared inside D3's own fix — a control documented as existing while being inert.** The stub matrix was described in this stanza as the script's pre-merge guard while living only in a scratch directory; code review caught it. And the label step, added to stop `gh issue create --label` erroring, was written inline above that command under `bash -e` — so a transient label failure aborted the step _before_ the Issue was attempted, muting the notification in exactly the case it exists to report. Both now degrade instead: label creation warns and continues, and a labeled `gh issue create` falls back to an unlabeled one. **Filing beats routing.** `prettier-drift-check.yml` — the precedent D3's fix was copied _from_ — carried the same defect in its two-step form and was fixed here too (Directive 6), along with its `gh issue list --label` lookup, which errors on a missing label for the same reason.
6. **The fix for a conflation introduced a new one, in the opposite direction.** Replacing the cap's aggregate flags with per-attempt state closed the dangerous path (a stale count reporting "no run ever appeared", the one code that authorises `workflow_dispatch`) but made a late API blip outrank a run already observed in flight — reporting exit 5, "do not re-run, fix the credential", when the remedy was exactly to re-run. Resolved by remembering one fact across attempts and nothing else: that a run was seen. The asymmetry is the point — a remembered sighting can only route to 3, where being wrong costs a re-run; exit 4 still demands a live final observation.
