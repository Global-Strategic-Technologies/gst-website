...
- [ ] **Annotation audit**: every registered tool carries a correct `readOnlyHint` / `destructiveHint` (all 16 tool modules under `mcp-server/src/tools/` reference `readOnlyHint` today — verify per tool, including the xlsx `fill` tool, whose write is to the response not to state). Add a contract-parity test so a future tool cannot register without one
...
- [ ] E2E: **do not duplicate the 117 English text assertions per locale.** Add one per-locale smoke spec (chrome renders in-locale, switcher round-trips a deep route, `<html lang>` and hreflang are right, banner shows/dismisses/persists) and extend `tests/e2e/accessibility.test.ts` with a sampled Tier A route per locale. Directive 11 applies to every string this item moves into a catalog: `grep tests/` for the old literal
- [ ] Docs: new [`src/docs/development/LOCALIZATION.md`](../development/README.md) (locale model, adding a locale, adding a string, the translation workflow, the tier table) linked from the master index and from `.claude/CLAUDE.md`; the `gst-page-content` skill gains a hand-off line for translation register; [STYLES_GUIDE.md](../styles/STYLES_GUIDE.md) gains the text-expansion and `hyphens` notes
