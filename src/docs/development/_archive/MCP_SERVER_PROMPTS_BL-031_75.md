...
**`gst_target_quick_look` ICG answer-population workflow** (the architecture doc references "ICG (light variant)" in the surface table; this is the resolved body contract). The full ICG tool requires answers to all 20 questions (the website wizard does not allow skipping — see [BL-031.5 V1 evidence](../../../mcp-server/README.md) and the ICG hidden-semantics callout in [`mcp-server/src/docs/tools/icg/CONTRACT.md`](../../../mcp-server/src/docs/tools/icg/CONTRACT.md)). The prompt body therefore instructs the model to produce a complete 20-answer payload by:
