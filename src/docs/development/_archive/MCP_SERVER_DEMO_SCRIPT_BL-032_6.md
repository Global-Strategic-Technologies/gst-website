...
```
Research the following three dimensions, in this exact order:

1. Use the GST MCP server to research comparable engagements (have we done this before?): call list_portfolio_facets first, then search_portfolio for healthcare/RCM/EU precedents.

2. Use the GST MCP server to research regulatory exposure (what's the regulatory risk?): call list_regulation_facets first, then search_regulations for Germany BDSG, France CNIL, EU GDPR — one call per jurisdiction.

3. Use the GST MCP server to research market signal (what's the market saying about this space?): call search_radar for European healthcare IT and RCM.

Afterwards synthesize: go / no-go / conditional, then one bullet per dimension
...
📋 **PASTE** as the first message into Claude Desktop (also available verbatim in Appendix A.8 + the fixture file [`mcp-server/tests/fixtures/medsig-health-filled-irl.md`](../../../mcp-server/tests/fixtures/medsig-health-filled-irl.md)):
...
- Applicable regulatory frameworks: HIPAA (US-statutory, BAAs in place with all customers), GDPR (EU-statutory, DPO appointed 2025), Germany BDSG, France CNIL guidance, Netherlands UAVG (the Dutch GDPR implementation that replaced Wbp in 2018), Spain LOPDGDD; SOC 2 Type II audit (renewed); HITRUST in progress; no PCI DSS exposure (not in cardholder flow); CCPA applicable for California customers
...
- **Partial deeplink coverage — only Diligence agenda (B) and Portfolio (G) show Open-in-Hub links; TechPar (C) / ICG (D) / Tech Debt (E) / Regulatory (F) / Radar (H) sections are missing them.** This is the **known v0.0.2 directive-strength regression** diagnosed during demo prep; v0.0.3 (queued post-demo per [`mcp-server/tests/examples/diligence-sweep.golden.md`](../../../mcp-server/tests/examples/diligence-sweep.golden.md) § v0.0.3 candidate patches) replaces "Capture" with "Surface" in the relevant Steps. **Demo-day recovery line**: _"You'll see Open-in-Hub links on the agenda and the portfolio sections — those work today. The other five sections each have a wizard, and the deeplink URL is already coming back from the tool response — we just shipped v0.0.2 and the prompt-body directive that surfaces them in the section close gets sharpened in v0.0.3 next week. Same data; the bridge to the Hub for those five sections lands in the next patch."_
