...
| **Call 1 (fired)**              | Blank canonical IRL template (hash `a99c42edd24aff64`)                    | Verifier returned 0 verified / 30 unverified / 30 tier-mismatches — **correct rejections** of a wrong-body envelope call. The cited SanFran-specific text is genuinely not in the blank template. |
...
```
- 0-01 — SanFran Solutions Inc. legal entity and brand are the same; HQ Toronto ON Canada; founded 2010
...
A property-based test in `mcp-server/tests/unit/schemas/extract-irl-from-xlsx.test.ts` should fuzz this: take the SanFran fixture, randomly perturb column widths / styles / non-Response cell contents, assert the canonical output is unchanged.
...
4. **The meta fence emits `"fixtureFillRatio"` matching the value the tool computed** (98.4% for SanFran). v11 showed the model independently computing this; with BL-049 the tool returns it and the model copies it through.
...
- **Unit**: `mcp-server/tests/unit/schemas/extract-irl-from-xlsx.test.ts` — engine round-trip against the SanFran + MedSig + Helios-Grid fixture xlsx files; property-based determinism test (perturbations that should not affect canonical output); error-path test for non-canonical xlsx shapes.
...
**Risk 2: Canonical-form spec drift.** If the canonical-form rules change (e.g., a new whitespace convention), every previously-computed `irlBodyHash` becomes stale. **Mitigation**: a snapshot test pinning the canonical output for the SanFran fixture. Changes require deliberate update + BREAKING_CHANGES entry.
...
- Unit tests: 6-10 cases covering happy path (SanFran fixture), error path (non-canonical xlsx), property-based determinism (SanFran fixture with random column-width / cell-style perturbations).
...
- Re-run SanFran IRL via the xlsx-attached workflow. Model attaches xlsx + invokes `/gst_irl_ingestion` interactive in a single touch.
