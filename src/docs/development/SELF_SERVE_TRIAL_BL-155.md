...
> **Status**: designed and reviewed 2026-09-06, then **rescoped to the connector flow only** (operator, 2026-09-06) — see § Scope. **Every slice is built** (Slices 1, 2b on 2026-09-06; 2, 3, 3a, 4 on 2026-09-07; branch `feat/bl-155-self-serve-mcp-trial`). What remains is the operator's production go-live (secrets + approval, BACKLOG § BL-155 Slice 4) and the closure distillation per the initiative-doc lifecycle.
...
- ~~**`trial` stays undocumented** on the public tier surface. It exists in `ASSIGNABLE_TIERS` but is not published in marketing copy or the tier table, so `mcp-marketing-parity.test.ts` and `hub-mcp-page.test.ts` keep enumerating three tiers unchanged.~~ **REVERSED 2026-09-08 by operator decision** — see § Amendment below.
...
**✅ Implemented 2026-09-07** (plan-reviewed: one REVISE round, then APPROVE). `src/trial/signup.ts` + `src/trial/turnstile.ts`, `lib/hmac.ts`, `rotateM2mSecret` in `m2m-clients.ts`, `TRIAL_SCOPES` in `auth/scopes.ts`, `TRIAL_TTL_SECONDS` / `TRIAL_IDENTITY_TTL_SECONDS` in `ratelimit/tiers.ts`, the staging-only CORS extra-origin list in `auth/cors.ts`. Operator account: `operations/AUTH.md § Self-serve trial mint`. **Wire contract**: `POST /trial/signup` with `{ "turnstileToken" }` → `200 { credential: "<clientId>:<secret>", clientId, expiresAt, reissued }`; `400 bad-request`, `400 challenge-failed { retryable }`, `403 trial-expired`, `409 in-progress { retryAfterSeconds }`, `429 rate-limited` (+ `Retry-After`), `503 unavailable`; all `{ error, message, … }`, all `no-store`. Decisions the build settled: identity key TTL is **30 days** (the record reaps at ≈33d — the privacy copy quotes 30 for the IP-derived key only); IP limiter 10/hour; the tier-scoped radar deny and `rateLimitSubject` split below had already shipped in Slice 2b; **CORS for previews/localhost** is a staging-only `TRIAL_EXTRA_ORIGINS` list, never a bare `*.vercel.app`. **Production gate**: its two secrets are not set, and the `mcp-production` approval is withheld, until Slice 3 lands the privacy disclosure and the public-copy corrections — the endpoint is 503 there by construction until then. Bullets below are the design as reviewed; the built shape is authoritative where they differ.
...
- **CORS**: `https://globalstrategic.tech` and `www.` are already allowed (`auth/cors.ts`). Vercel preview origins and localhost are not — **decided (Slice 2 build)**: a staging-only `TRIAL_EXTRA_ORIGINS` `[vars]` list of exact origins plus at most one one-label suffix `https://*-<team>.vercel.app` (team slug read off a real preview URL when Slice 3 first runs against staging), honoured only by the preflight and trial branches; a bare `*.vercel.app` is rejected at parse. Production's set is untouched.
...
**The design system is already synced** to the Claude Design project (`https://claude.ai/design/p/660c7df6-e99f-4f47-b9f7-b1ab32e52969`) — tokens, `html.dark-theme`, every `html.palette-N` block, the `.brutal-*` vocabulary, ten specimen galleries and 19 chrome cards sliced from real production output. See [CLAUDE_DESIGN_SYNC.md](CLAUDE_DESIGN_SYNC.md). The agent writes its own JSX and styles it with GST classes; it **cannot** import `.astro` components, and we never hand-write React versions of them.
...
- Must work in light AND dark theme AND every palette.
...
**✅ Built 2026-09-07** (plan-reviewed: REVISE, REVISE, APPROVE). One Tier A page, [`HubMcpTrialPage.astro`](../../page-templates/HubMcpTrialPage.astro) at `/hub/mcp/trial/`, serving both flows from the [hand-off](MCP_TRIAL_SIGNUP_HANDOFF_BL-155.md): the connector flow (this item) and the "From code" presentation (BL-156's UX). Every state block is server-rendered from `hub-mcp-trial.json` and toggled by `src/scripts/trial-signup.ts`; the pure helpers are in `src/utils/trial-signup-core.ts`. **Three hand-off values did not survive contact with source** and the page follows source: the ceilings (hand-off 30/300 = `free-pilot`; page 15/100 = `TIER_LIMITS.trial`, pinned by `tests/integration/mcp-trial-parity.test.ts`), the request-access anchor (`#request-access`), and the token snippet's `token_type` (`bearer`). Two Worker additions were needed for the copy: `issuedAt` on the re-issue 200 and the 403 (the revoked-credential date), and `retryAfterSeconds` in the 429 body (`Retry-After` is not CORS-exposed). The prototype bundle is not committed (Claude Design is its source; a local copy was moved out of `src/`, where its CSS tripped the repo's lint and token sweeps). Bullets below are the design as written; the built shape is authoritative where they differ.
...
_Retained for the M2M initiative:_ **This is critical path, not a follow-on.** The trial mints an M2M `client_credentials` pair, and **nothing on the website documents how to use one.** [`/hub/mcp/get-started/`](../../pages/hub/mcp/get-started/index.astro) covers only the custom-connector flow, which a trial credential cannot drive (see § Scope). So without this page the signup page's success state has nowhere to point, and the failure lands _after_ the visitor has already spent their one-time secret. Ship it with Slice 3 or the trial does not work.
...
- **Do not describe a config-file or custom-header integration.** There are no `X-GST-Client-*` headers, and a trial credential pasted into `claude_desktop_config.json` cannot work. An early Claude Design mockup invented both — see § Scope for why, so nobody re-derives it.
...
7. E2E for the signup page with the always-passes test sitekey; light/dark and every palette; axe on the new route.
8. **A real end-to-end exercise against staging — driven only by what the signup page tells a stranger to do** (rewritten 2026-09-07 for the connector flow; the earlier text described the M2M exercise and warned against Claude Desktop, which is now backwards). Once staging carries its two trial secrets (the always-pass Turnstile test pair): sign up on the staging page, receive the one credential string, then follow the page's own three steps — add the connector in Claude Desktop, paste the string at the consent page, approve — and confirm the GST tools are listed, a `search_radar` call is refused with `-32002`, and a `search_portfolio` call succeeds. Do not substitute knowledge you have and the visitor does not — the page failing to say something is exactly the defect this exercise exists to catch. Then sign up again from the same network and confirm the previous credential is refused at consent while the new one works and the shown expiry is unchanged. For expiry, **the primary method is minting a record with a past `expiresAt` via the admin API and proving both doors refuse it** (consent 401; `/token` `invalid_client`) — a 72h wall-clock wait is not a test anyone will repeat. Assert the residuals as expected behaviour, not bugs: a `mcp_m2m_*` token minted before expiry lives ≤1h, and a consent grant runs to its captured `expiresAt`. Directive 5: not done until proven.
