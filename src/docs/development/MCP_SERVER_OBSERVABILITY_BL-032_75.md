...
| ---------------------------- | ----------------- | ------------------------------------------------------------------------ | -------------------------------------------------------------- | ------------------------------------------------ |
| `tool_invocation`            | Counter+Histogram | `tool_name`, `key_prefix`, `success`, `duration_ms`                      | Partial — safeLog has all dims; no aggregation surface         | Volume + latency by tool and outcome             |
...
| `prompt_invocation`          | Counter           | `prompt_name`, `key_prefix`                                              | No                                                             | Prompt usage by name                             |
...
| `rate_limit_decision`        | Counter           | `key_prefix`, `decision` (allow/throttle/deny)                           | Partial — limiter logs allow/throttle; no aggregation          | Rate-limit pressure by key                       |
...
#### Analytics Engine column map (must be decided before any emitter is written)

Cloudflare Analytics Engine is **positional-columnar**, not Prometheus-style named-series. One dataset, fixed columns:
...
| `blob3`   | `key_prefix`     | First 6 chars of bearer-token hash; null when not applicable                                                                                                          |
...
| `index1`  | `key_prefix`     | AE's sampling-key column. Mirrored from `blob3` so AE samples by tenant when scaling                                                                                  |

**Why one dataset, not one-per-event-family**: AE's 25 events/sec/binding write ceiling is per-DATASET, not per-binding. Splitting into 8 datasets needs 8 bindings (one per family) and risks dimension-explosion via duplicated `key_prefix` columns. Single dataset trades query complexity (every query has `WHERE blob1 = '<event>'`) for cleaner write-path and uniform schema discipline.

This column map lives in `mcp-server/src/metrics/_schema.ts` as the **single source of truth** for emitters, runtime cardinality guards, vitest fixtures, Grafana SQL queries, and status-page queries. Changing it is a breaking change to every downstream consumer.
...
│   └── routes/
│       └── status.ts               # NEW — Worker-rendered status page (server-side AE query, server-side cache)
...
- **3 BL-032.5-deferred ACs**: `prompt_invocations_total` folds into Step 2's emitter set; **`notifications/message` breaking-change push is a category error to fold here** — it's an MCP-protocol surface, not a metric; punt to BL-033; **`GET /prompts/<name>/scopes`** introspection also belongs in BL-033.
...
2. **Runtime cardinality guard** (~0.5 day): `guard.ts` enforces the schema at runtime — rejects unknown event-type dims, truncates strings >256 chars (configurable per field), hashes any field flagged as "tenant-bounded only" (e.g. `key_prefix`). Pairs with a CI test that enumerates registered dims against the schema. Static enumeration catches the obvious cases; runtime enforces it holds against conditional emissions.
...
| Cardinality explosion on metric dimensions                                   | Two-layer defense: CI test enumerates registered dims against `_schema.ts`; runtime `guard.ts` rejects unknown dims + truncates strings >256 chars + samples `key_prefix` via `index1` |
