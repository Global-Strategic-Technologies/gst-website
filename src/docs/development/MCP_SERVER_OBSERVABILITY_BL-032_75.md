...
- Empirical confirmation (2026-05-27): Cloudflare cron-events dashboard now reports `Success` on natural firings; [`_archive/BL-032_76_VERIFICATION.md`](../../../mcp-server/src/docs/operations/_archive/BL-032_76_VERIFICATION.md) operator runbook covers the 5-point verification surface (Cloudflare dashboard, Sentry Crons, Sentry Issues, `/health`, `wrangler tail`).
...
### Phase 1 — instrumentation (✅ SHIPPED 2026-05-31)

**Closure note 2026-05-31**: Phase 1 shipped incrementally across the BL-031.x → BL-038 work; an audit during the BL-037 wrap-up surfaced that the BACKLOG status and design-doc closure had drifted from code reality (same documentation-vs-code drift pattern BL-038 closed). This stanza records the truth state with file pointers for every step.

| Step                                           | Status                | Evidence                                                                                                                                                                                                                                                                                                                                          |
| ---------------------------------------------- | --------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 1. AE schema + sink interface                  | ✅                    | [`mcp-server/src/metrics/_schema.ts`](../../../mcp-server/src/metrics/_schema.ts), [`sinks/_interface.ts`](../../../mcp-server/src/metrics/sinks/_interface.ts), [`sinks/analytics-engine.ts`](../../../mcp-server/src/metrics/sinks/analytics-engine.ts), [`sinks/in-memory.ts`](../../../mcp-server/src/metrics/sinks/in-memory.ts)             |
| 2. Runtime cardinality guard                   | ✅                    | [`mcp-server/src/metrics/guard.ts`](../../../mcp-server/src/metrics/guard.ts) with [`guard.test.ts`](../../../mcp-server/tests/unit/metrics/guard.test.ts) — enforces event-type enum, blob/double slot allowlist, string truncation, AE payload-size limit                                                                                       |
| 3. `withMetrics` HOF                           | ✅                    | [`mcp-server/src/metrics/with-metrics.ts`](../../../mcp-server/src/metrics/with-metrics.ts) — exports `withToolMetrics`, `withResourceMetrics`, `withPromptMetrics` over a shared `withMetricsCore` generic                                                                                                                                       |
| 4. Per-event emitters wired into registrations | ✅                    | **10 tools** (diligence, generate-irl-xlsx, icg, portfolio×2, radar-live×2, regulations×2, tech-debt, techpar) + **5 resources** (library, radar×3, regulations) + **all prompts via `prompts/_registry.ts`**. All threaded with `MetricsContext` from `createServer(env, ctx)` per [`server.ts:104-134`](../../../mcp-server/src/server.ts#L104) |
| 5. AE binding in wrangler.toml                 | ✅                    | [`wrangler.toml:51-52`](../../../mcp-server/wrangler.toml#L51) (dev: `mcp_events_dev`), [`:144-145`](../../../mcp-server/wrangler.toml#L144) (staging: `mcp_events_staging`), [`:192-193`](../../../mcp-server/wrangler.toml#L192) (production: `mcp_events`)                                                                                     |
| 6. `inoreader_call` emitter                    | ✅ shipped 2026-05-29 | [`inoreader-egress.ts`](../../../mcp-server/src/lib/inoreader-egress.ts) emit at every counter increment + parallel emit on `singleFetch` network-error catch (status_code='0' for timeouts/aborts). Schema gained `blob7=zone1` filter + `NAME_VALUES` allowlist in `guard.ts`                                                                   |
| 7. Vitest coverage                             | ✅                    | 76/76 passing across `tests/unit/metrics/{guard,schema,with-metrics,prompt-span,sdk-integration}.test.ts` + `sinks/{analytics-engine,in-memory}.test.ts`                                                                                                                                                                                          |

**Account-level Analytics Engine enable**: shipped 2026-05-28 — the Cloudflare dashboard required a one-time blank-dataset creation to flip the account-level AE switch before `writeDataPoint` calls would land. Documented in [`mcp-server/src/docs/operations/DEPLOY.md`](../../../mcp-server/src/docs/operations/DEPLOY.md).

**Operator read-only token**: `gst-mcp-ae-read` Cloudflare API token (Account → Account Analytics → Read) bound for SQL-API queries via [`mcp-server/scripts/Verify-AeEmission.ps1`](../../../mcp-server/scripts/Verify-AeEmission.ps1). Activated by this closure pass; pending baseline window for empirical event-volume verification (Phase 2).

**Phase 2 baselining clock**: started 2026-05-31 with this closure. First baselining-window data-pull deadline 2026-06-07. Tracking artifact at [`mcp-server/observability/slo-baselines.md`](../../../mcp-server/observability/slo-baselines.md).

---

(Original Phase 1 implementation plan, retained as reference for the closure trail):
...
5. **Cloudflare Analytics Engine binding in `wrangler.toml`** (~0.25 day):
   ...
   dataset = "mcp_events"
   ```
6. **`inoreader_call` emitter** (~0.5 day, **gated on Phase 0 soak passing**): wraps the `recordInoreaderEgress` call in `inoreader-egress.ts` so every counter increment also writes an AE event. Single source of truth — the `/health.inoreaderSpend` numbers and the AE events derive from the same chokepoint. **✅ Shipped 2026-05-29** — emit lives inside `recordInoreaderEgress` (one event per outbound call), plus a parallel emit on the `singleFetch` network-error catch branch (`status_code='0'`, `outcome='error'`) so timeouts / aborts don't undercount. Schema gained `blob7=zone1` (`'1'`/`'0'`) so dashboard SQL filters Zone-1 spend without consulting docs (`WHERE blob7='1'`). Schema also gained `NAME_VALUES` allowlist enforced in `guard.ts` — catches drift between the wrapper's `CATEGORIES` enum and the schema's pinned category set. `keyOwner` threaded from MCP tool HOFs and `/radar/snapshot` handler all the way through to `authenticatedFetch`; cron/oauth paths intentionally land in `__none__`. **Drift breadcrumb** (Sentry `inoreader.spend.drift`) stays Sentry-only for Phase 1; revisit if Phase 3 dashboards need an AE projection.
