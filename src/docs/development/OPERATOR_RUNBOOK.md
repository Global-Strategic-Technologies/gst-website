...
> Why the prompt ARGUMENT for client-ready: only the `filledIrl` argument produces `pass-bound` hash-bind authority (the server hashes the partner's exact bytes itself, with no model emission in the path). Every other route is `pass-internal`, including a chat paste and an attached `.md` — the hash went through the model even when the bytes did not change. The xlsx-reconstruction path is `pass-internal` too, and there the model controls both the body and the hash — and auto-appends a `provenance-gap:` disclosure (BL-072). Fine for drafts, not defensible for a client/regulator.
>
...
| ------------------------------------------ | -------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `filledIrl.source`                         | model, server-CAPPED | `partner-paste-verbatim` / `partner-paste-verbatim-prepop` = strong; `model-reconstruction-*` = draft-grade. **Server-capped**: an over-claimed `-prepop` is downgraded and disclosed in (J), so this value is checked, not merely reported. **The cap only ever fires on `-prepop`**: an asserted `partner-paste-verbatim` is returned untouched, so on a chat-delivered body (pasted or attached `.md`) that grade is the model's own classification and no server check contradicts it. See [ADR-0018](../adr/0018-body-integrity-and-capped-provenance.md) — Amendment. |
