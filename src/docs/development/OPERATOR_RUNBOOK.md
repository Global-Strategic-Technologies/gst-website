...
> ⚠️ **Check the paste kept its line breaks.** Claude Desktop's argument fields are single-line inputs, and on some client versions a multi-line paste arrives with every newline collapsed to a space. A real 2026-08-13 run lost all 141 line breaks in a 79KB IRL for a **one-byte** change in size, and produced a dossier citing structure that no longer existed. The server now halts on it rather than proceeding — if that happens you did nothing wrong; attach the `.md` and invoke without `filledIrl` (interactive mode), or use a client with a multi-line input. The collapse cannot be repaired. See [IRL_PARTNER_PASTE_RUNBOOK.md](./IRL_PARTNER_PASTE_RUNBOOK.md) § Step 3.
