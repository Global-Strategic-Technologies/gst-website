...
> **Source**: BL-032.25 — bucket for soak-findings discovered during BL-032 § B.5 (staging soak window 2026-05-06 → 2026-05-12). **As of 2026-05-12 BL-032 has shipped to production** at `mcp.globalstrategic.tech`; this initiative ran in **post-Go-Live close-out mode**, tracking the remaining P1 follow-ups + any new findings surfaced during the one-week post-deploy review window (2026-05-12 → ~2026-05-19). **The bucket closed 2026-07-14** — this doc is now the historical record.
>
> **BACKLOG entry**: [BL-032.25 in BACKLOG.md](./BACKLOG.md#bl-03225-mcp-revisions-prior-to-go-live--closed-2026-07-14-bucket-empty)
>
> **Status**: ✅ **CLOSED 2026-07-14 — bucket empty.** Zero P0 items (substrate shipped). § 1 closed rejected 2026-07-14 (benchmark-audit spike executed same day its BL-032.75 Phase 2 deferral gate lifted; finding A — collapses by-design; see § 1 closure stanza); § 2 closed 2026-05-13 (commit `e97650d`); § 3 closed inconclusive 2026-05-13 (reproduction tool + instrumentation retained); § 4 closed 2026-05-13 (commit `170f1d0`); § 5 closed risk-accepted 2026-05-12. No new findings surfaced during the post-deploy review window. § 1 authored at initiative-creation time as the anchor finding (schema normalization → adapter retirement question); §§ 2–4 filed retroactively 2026-05-12 from the soak.
>
...
## § 1 — Schema normalization across Hub Tools (Investigation — P1, ✅ closed rejected 2026-07-14)

### Status

- **Authored**: 2026-05-06 (anchor item at initiative creation)
- **Severity**: **P1** — post-launch follow-up; did NOT block B.6 production deploy
...
- **Closure stanza**: ✅ see [Closure stanza (2026-07-14)](#closure-stanza-2026-07-14) below — benchmark-audit spike executed, finding A (collapses by-design), item closed rejected
...
- [x] **Benchmark audit completed** — answers whether the existing collapses are by-design or lazy. **This is the new dominant question.** Cheap to answer (2-4 hours); expensive to skip. ✅ Executed 2026-07-14 — **finding A** (all three collapses by-design; see closure stanza)
...
**Status**: investigation conclusions hold. The benchmark-audit spike (2–4 hours, original ICG / TechPar benchmark authors or a senior consultant) remains the next concrete action. Spike was **not yet scheduled** as of 2026-05-13; executed 2026-07-14 (see closure stanza below).

### Closure stanza (2026-07-14)

**✅ CLOSED — REJECTED (finding A: collapses are by-design).** The 2–4h benchmark-audit spike executed 2026-07-14, the same day its deferral gate lifted (BL-032.75 Phase 2 baselining closed 2026-07-14 via PR #309). Judged by the operator in-session, on a Claude-prepared audit dossier covering **all three collapse points** — one more than the finding-table above enumerated: [`stage-adapters.ts:48-53`](../../data/common/stage-adapters.ts) shows ICG carries TWO collapses (`pre-series-b` ← seed + series-a AND `series-bc` ← series-b + series-c) alongside TechPar's `series_bc`.

**Per-collapse findings** (each judged independently):

1. **ICG `pre-series-b` (seed + series-a) — by-design.** `BENCHMARK_RANGES` ([`icg-engine.ts:406-411`](../../utils/icg-engine.ts)) is a set of four expert-authored maturity-score priors introduced 2026-03-21 (`25f0a655`) as a contextualization feature — there is **no underlying per-company sample** that could be re-cut into finer cohorts. The instrument itself (20 questions, 0–3 scale) lacks the resolution to distinguish seed from series-a governance maturity, which shifts with scale, not with the A round. A split would fabricate a boundary.
2. **ICG `series-bc` (series-b + series-c) — by-design.** Same authored dataset, same instrument-resolution limit. The 30–55 band already overlaps both neighbors; splitting it into two bands with no data behind the boundary would _reduce_ the feature's honesty (the collapsed label currently tells the user they're in a coarse bucket).
3. **TechPar `series_bc` (series-b + series-c) — by-design.** `STAGES` ([`stages.ts:11-92`](../../data/techpar/stages.ts)) bands are curated from public SaaS benchmark data ([`industry-notes.ts:5`](../../data/techpar/industry-notes.ts): "Benchmarks remain SaaS-derived"). **Source check (2026-07-14 web audit)**: the major published spend-benchmark corpora — SaaS Capital spending benchmarks, KeyBanc, OpenView/High Alpha, Bessemer — segment by **ARR band and funding type** (bootstrapped vs equity-backed), not round letter; none publish tech-spend ratios for Series B and Series C as separate cohorts. The blended B–C band mirrors the source data's own granularity — there is nothing finer to split _to_. Secondarily, the collapse is conceptually load-bearing: B and C share the `convergence` trajectory frame; the regime break is at PE/enterprise.

**Disposition** (per the recommendation table above): finding A + no architectural trigger fired (no third stage-cohort tool planned through BL-033; BL-033 pilot not started) → **"leave the adapter pattern alone; it's encoding a real data limitation and renaming where the limitation lives gains nothing."** No normalization initiative filed. [BL-031.87](./MCP_SERVER_STAGE_ADAPTER_BL-031_87.md) remains **Closed** (not Superseded); its adapter layer and the `stageContext { native, canonical }` output — which already discloses the collapses to consumers — stay as-shipped. Revisit only if a trigger from the decision-criteria checklist fires (a third stage-cohort tool with finer-grained data, or BL-033 pilot feedback flagging the adapter as confusing).

**With § 1 closed, the BL-032.25 bucket is empty — the initiative is closed.**
...
| § 1  | 2–4 hrs benchmark-audit spike; then 0 days OR 2–3 days engineering depending on audit outcome | Schedule / data | Docs-only closure (audit outcome A → 0 days) | **✅ Closed rejected 2026-07-14** — spike executed same day the Phase 2 gate lifted; finding A (collapses by-design)            |
...
3. **§ 1 deferred** until BL-032.75 Phase 2 closes — re-surface as a Phase 2 close-out agenda item; no action this cycle. _(Resolved: Phase 2 closed 2026-07-14; spike executed the same day → finding A → § 1 closed rejected. See § 1 closure stanza.)_
...
- **§ 1 — when the audit eventually runs**: if it returns finding B (lazy modeling), BL-031.87 is no longer the canonical answer for stage taxonomy. Update BL-031.87's status from Closed to Superseded and add a redirect note. Do NOT delete BL-031.87's content — it's the historical record of why the adapter pattern was chosen. _(Resolved 2026-07-14: audit returned finding A — BL-031.87 stays Closed, no supersession.)_
...
_Last updated: 2026-07-14 — § 1 closed rejected (benchmark-audit spike executed the day its BL-032.75 Phase 2 deferral gate lifted; finding A — all three stage collapses are by-design; adapter pattern stays). Bucket empty; **BL-032.25 is closed**. Prior wave 2026-05-13: § 2 (commit `e97650d`) and § 4 (commit `170f1d0`) closed via fixes; § 3 closed inconclusive (H1+H2 eliminated by 10-run reproduction; H3 upstream-only — reproduction tool + handler instrumentation retained for recurrence detection)._
