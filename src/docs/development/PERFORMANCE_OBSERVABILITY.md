...
| MCP server SLOs + alerting       | Cloudflare Analytics Engine + Worker cron + Sentry email + `/status` | [`mcp-server/src/docs/ARCHITECTURE.md` § Observability](../../../mcp-server/src/docs/ARCHITECTURE.md#observability) (shipped at mcp-server 0.39.0; the Grafana dashboard artifact shipped 2026-09-08 — [`operations/GRAFANA.md`](../../../mcp-server/src/docs/operations/GRAFANA.md) — with datasource configuration and import outstanding as operator tasks) |
