...
- `fast-uri: 3.1.7` — patched `fast-uri` for the ajv subtree (transitive via `@modelcontextprotocol/sdk → ajv@8.20.0`, which declares `^3.0.1`), closing the host-confusion advisories `GHSA-v2hh-gcrm-f6hx` + `GHSA-4c8g-83qw-93j6`, the 2026-08 backslash-authority-introducer advisory (vulnerable `3.0.0–3.1.4`), and the four 2026-09 URI-normalization advisories listed in the history above (vulnerable `3.0.0–3.1.5`, which was this pin's previous value). Stays on the 3.x line — deliberately NOT 4.x, which would violate ajv's `^3.0.1`. Re-evaluate when ajv widens its range to `^4`.
- `qs: 6.16.0` (**flat**, added 2026-09-04) — closes `GHSA-x5fp-wj9c-mxmx` (array-limit bypass via bracket-key comma parsing) and `GHSA-4mjr-xmp4-gh2g` (DoS via attacker-controlled `isBuffer`), vulnerable `2.2.5–6.15.3`, and with them the `body-parser@1.20.x` / `express@4.22.2` findings that only depended on the vulnerable `qs`. Chain: `agents → @modelcontextprotocol/sdk → express` / `express-rate-limit → express → body-parser → qs`; the tree holds a single `node_modules/qs` node (consumers declare `^6.14.0` and `^6.15.2`), so the flat form is the right shape per the hono lesson above. Drop when the SDK's express and body-parser resolve a patched `qs` on their own: check with `npm ls qs --all`, delete the override, then `npm audit --omit=dev`.
