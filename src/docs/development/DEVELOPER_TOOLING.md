...
- **2026-09-04 (#447)** — follow-on to the entry below, merged after it: the same override fix had been prepared on a second branch, and the review of that branch asked for something the required suite would catch, so it added the overrides guard (`tests/integration/overrides-honoured.test.ts`). The guard's first run retired three entries the tree had already resolved past (`hono`, `@hono/node-server`, the SDK-scoped `express-rate-limit`); the audit stayed at zero and no installed version changed. It also normalised eighteen wrangler-subtree `devOptional` flags that a plain `npm install` had been rewriting since #438.
