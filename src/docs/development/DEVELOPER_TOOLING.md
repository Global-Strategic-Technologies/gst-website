...
- ~~`esbuild: 0.28.1`~~ — **removed 2026-08-21**, on the exit condition this entry itself named ("remove when the tree's esbuild consumers resolve to `>=0.28.1` naturally"). That condition is now met: every declared range in the tree floors at or above the patched release — `astro`/`@astrojs/vercel` at `^0.28.0` resolving to `0.28.1`, `vite` at `^0.27.0 || ^0.28.0`, `agents` at `^0.28.1`, `wrangler` at an exact `0.28.1`, and `mcp-server` itself at `^0.28.2`. The override had become actively harmful rather than merely redundant: because it is flat, it forced `mcp-server`'s `^0.28.2` **down** to `0.28.1`, so the manifest asked for one version and the tree installed another. Verified after removal with `npm ls esbuild --all` (no `invalid`, nothing below `0.28.1`) and `npm audit --audit-level=moderate --omit=dev` → 0 vulnerabilities.
...
- `@cloudflare/workers-types: 5.20260804.1` (exact, added 2026-08-21) — **`5.20260807.2` and later add `declare const Buffer: any;` and `declare const process: any;` at global scope** (in the package's `index.d.ts` globals block, alongside `declare const origin` / `declare const navigator`). [mcp-server/src/worker.ts](../../../mcp-server/src/worker.ts) line 1 carries `/// <reference types="@cloudflare/workers-types" />`, and a type-library reference applies program-wide — so those two `any` globals shadow the `@types/node` declarations across **all** of `mcp-server`, Worker and stdio and tests alike. The effect is a silent loss of Node type fidelity, not a contained one; it surfaced as three `tsc --noEmit` errors that are only the visible tip:
  ...
  Bisected against a verified-green master baseline: `5.20260804.1` clean, `5.20260807.2` broken; the bump alone reproduces all three errors with every other dependency held at master. **Patching the three call sites was rejected** — it would paper over a program-wide `any` regression on every `process.*` and `Buffer.*` use in the workspace while leaving the underlying shadowing in place. Remove the pin when Cloudflare narrows those globals (re-check with `npm view @cloudflare/workers-types@latest` then `grep 'declare const Buffer' node_modules/@cloudflare/workers-types/index.d.ts`); the durable fix, if Cloudflare does not, is splitting `mcp-server` into separate Worker and Node TypeScript programs so the Worker globals stop reaching Node code — see BL-137.
