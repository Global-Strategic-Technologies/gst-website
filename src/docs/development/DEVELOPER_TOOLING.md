...
**Current production state** (measured 2026-08-21): **zero vulnerabilities** on the enforced gate (`--omit=dev`).
...
The `wrangler → miniflare → undici` chain this section tracked from 2026-08-04 is **cleared as of 2026-08-21** and its bullet is retired: the Dependabot batch moved wrangler to an exact `4.121.0` (see § Held version pins below), whose miniflare resolves `undici@7.29.0` — outside the vulnerable range. `@sentry/cli`'s separate `undici@6.28.0` node, cleared by hand on 2026-08-17, remains clean. Verified with `npm audit --json` (the `nodes` array carries one entry per chain, and the six above are all `node_modules/@lhci/*`-rooted).

> **Do not merge [#419](https://github.com/Global-Strategic-Technologies/gst-website/pull/419)** (or any Dependabot PR raising `wrangler` past `4.121.0`). The bullet retired above previously routed that clearance through #419 at `^4.123.0`; that is now the wrong instruction. `4.123.0` is outside the exact pin, and taking it drags `@cloudflare/workers-types` into the range § Held version pins exists to exclude. The undici clearance it was wanted for has already happened.

_(This paragraph read "3 dev-only advisories … all one chain" from 2026-08-04 until 2026-08-17, when it was measured again and found to be describing a third of the tree; it then read "9 … in two chains" until 2026-08-21, when the wrangler chain cleared. Dev-only advisories fail nothing, so this paragraph only ever matches reality when someone re-measures it — do that before citing it.)_
...
- `fast-uri: 3.1.5` — patched `fast-uri` for the ajv subtree (transitive via `@modelcontextprotocol/sdk → ajv@8.20.0`, which declares `^3.0.1`), closing the host-confusion advisories `GHSA-v2hh-gcrm-f6hx` + `GHSA-4c8g-83qw-93j6` and, from 2026-08, the backslash-authority-introducer advisory (vulnerable `3.0.0–3.1.4`). Stays on the 3.x line — deliberately NOT 4.x, which would violate ajv's `^3.0.1`. Re-evaluate when ajv widens its range to `^4`.
- `hono: 4.12.34` (**top-level**) — closes the hono/jsx cross-request-disclosure + header advisories and the CORS-middleware ReDoS (`GHSA-…`, vulnerable `<4.12.34`). **BL-106 reversed this entry's form**: it was previously scoped under `@modelcontextprotocol/sdk` and `@hono/node-server`, and this doc claimed the flat form "is NOT honored by npm 11 in this repo". That is no longer true and the reason is instructive — **path-scoped override keys stop resolving when the parent moves in the tree.** Once BL-106 dropped `@modelcontextprotocol/sdk` as a direct dependency (it is now only an `agents` peer), both scoped keys silently became dead: npm reported `invalid: "4.12.34" from …` while leaving `4.12.32` installed. Prefer the flat form for packages that appear once in the tree; reserve scoped keys for genuine subtree-only pins (e.g. `@lhci/cli` below).
- `@modelcontextprotocol/sdk → { express-rate-limit: 8.5.1 }` and `@hono/node-server → { ".": 2.0.11 }` (scoped) — `@hono/node-server@2.0.11` closes its own Windows serve-static path-traversal (`GHSA-frvp-7c67-39w9`, `<2.0.5`). **The `.` self-key forces a 1.x→2.x major OUTSIDE the SDK's declared `^1.19.9`** — accepted because the Worker uses the Web-standard `createMcpHandler` transport, not the `@hono/node-server` Node adapter (SDK-internal, unreached), and `npm run test:mcp` proves no module-load break. `express-rate-limit: 8.5.1` closes the depends-on-vulnerable-`ip-address` advisory. Re-evaluate when the SDK declares `@hono/node-server ^2` (drop then).
...
**Held version pins** — distinct from `overrides`: these are exact versions on **direct** dependencies in [mcp-server/package.json](../../../mcp-server/package.json), pinned to hold a package *below* its latest release. An exact pin (not a caret range) is required in both cases, because a floating range re-resolves to the broken version on any lockfile regeneration — which is precisely how Dependabot proposes updates.

- `@cloudflare/workers-types: 5.20260804.1` (exact, added 2026-08-21) — **`5.20260807.2` and later add `declare const Buffer: any;` and `declare const process: any;` at global scope** (in the package's `index.d.ts` globals block, alongside `declare const origin` / `declare const navigator`). [mcp-server/src/worker.ts](../../../mcp-server/src/worker.ts) line 1 carries `/// <reference types="@cloudflare/workers-types" />`, and a type-library reference applies program-wide — so those two `any` globals shadow the `@types/node` declarations across **all** of `mcp-server`, Worker and stdio and tests alike. The effect is a silent loss of Node type fidelity, not a contained one; it surfaced as three `tsc --noEmit` errors that are only the visible tip:
  - `src/index.ts` — `process.exit(1)` stops returning `never`, so the `serveStdio` factory's `catch` no longer terminates the control flow and the factory widens to `McpServer | undefined`, failing `McpServerFactory`.
  - `tests/integration/oauth-flow.test.ts`, `tests/unit/lib/generate-irl-xlsx.test.ts` — `Buffer` loses its Node overloads, so `buf.toString('base64url')` / `.toString('utf8')` fail with `TS2554: Expected 0 arguments, but got 1`.

  Bisected against a verified-green master baseline: `5.20260804.1` clean, `5.20260807.2` broken; the bump alone reproduces all three errors with every other dependency held at master. **Patching the three call sites was rejected** — it would paper over a program-wide `any` regression on every `process.*` and `Buffer.*` use in the workspace while leaving the underlying shadowing in place. The condition is **still unmet at the current latest** (`5.20260822.1` carries both declarations), so this is a standing hold, not a one-release blip. Remove the pin when Cloudflare narrows those globals — check without mutating the tree by reading the candidate straight out of its tarball: `npm pack @cloudflare/workers-types@<version>` then `grep 'declare const Buffer' package/index.d.ts` on the extracted copy. The durable fix, if Cloudflare does not narrow them, is splitting `mcp-server` into separate Worker and Node TypeScript programs so the Worker globals stop reaching Node code — see BL-137.
- `wrangler: 4.121.0` (exact, added 2026-08-21) — **coupled to the pin above, not independently held.** Wrangler declares `@cloudflare/workers-types` as a peer and advances that floor almost every release: `4.121.0` peers `^5.20260804.1` (the newest wrangler that our pinned types version satisfies), while `4.122.0` demands `^5.20260811.1` and `4.125.0` demands `^5.20260820.1` — all inside the broken range, and npm fails the install with `ERESOLVE` rather than warning. This is still a bump from master's `^4.118.0`; it is simply the ceiling. Raise it in lockstep whenever the types pin is raised or removed.

**Automated dependency updates** — [Dependabot](../../../.github/dependabot.yml) opens PRs weekly for npm and GitHub Actions version bumps. When reviewing Dependabot PRs that update `@astrojs/vercel` or `@vercel/routing-utils`, check whether the `path-to-regexp` override can be removed by running `npm audit --omit=dev` after deleting the `overrides` block. `@cloudflare/workers-types` and `wrangler` carry `ignore` entries in that config so Dependabot stops re-proposing them above the held pins documented immediately above — the hold is enforced by configuration rather than by a reviewer remembering to close a weekly PR. **Delete both `ignore` entries together when the pins are lifted**; leaving one ignored while the other floats re-creates the `ERESOLVE` the pins exist to prevent.
