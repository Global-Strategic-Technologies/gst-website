...
The `wrangler → miniflare → undici` chain this section tracked from 2026-08-04 is **cleared as of 2026-08-21** and its bullet is retired: the Dependabot batch moved wrangler to an exact `4.121.0` (see § Held version pins below), whose miniflare resolves `undici@7.29.0` — outside the vulnerable range. `@sentry/cli`'s separate `undici@6.28.0` node, cleared by hand on 2026-08-17, remains clean. Verified with `npm audit --json`. **Check the `via` field, not the node paths** — only two of the six entries are `@lhci/*`-rooted (`node_modules/@lhci/cli`, `node_modules/@lhci/utils`); the other four sit at `node_modules/lighthouse`, `node_modules/puppeteer-core`, `node_modules/@puppeteer/browsers` and `node_modules/extract-zip`, hoisted to the root where their paths no longer name the chain that reached them. All six trace through the single direct dependency `@lhci/cli` to one `extract-zip` advisory, and `via` is what shows that.
