...
> **Merge-commits-only is enforced at the repository level (set 2026-09-21).** `allow_squash_merge` and `allow_rebase_merge` are **false**; only `allow_merge_commit` is true. [`.claude/CLAUDE.md`](../../../.claude/CLAUDE.md) § Git Workflow has always said "Merge commits, never squash", but nothing enforced it and the squash button stayed available. **What a squash costs**: it collapses a branch's commits into one new commit with a different SHA, so any branch still holding the originals is no longer an ancestor of `master`. Git then sees the same content arriving twice and reports **conflicts that have no real disagreement in them** — reconciling them means merging changes against identical copies of themselves. Observed on PR #504, which conflicted against `master` purely because PR #503 had squash-merged the same branch as `9a0dac30`; the remedy was to cut a fresh branch from `origin/master` and cherry-pick the one genuinely-missing commit, opened as PR #505 — not to resolve the conflicts. If a future PR conflicts and the diff looks like content already on `master`, check for a squash before resolving anything.
...
`playwright.config.ts` boots **one** dev server, on 4321 (reused locally if you already have one running), started as `npx astro dev --port 4321` rather than `npm run dev`: on Windows the `npm run` shell sat between Playwright and node, so the server outlived every run, was reused by the next, and twice died mid-suite. `tests/e2e/localization.test.ts` runs against it like every other spec; its first test fails by name if fewer than two locales are `live` in `src/i18n/locales.ts`.

The config also sets a **baseline `storageState`** for every context (`tests/e2e/helpers/storage-baseline.ts`): palette 0 and light theme, both stamped with today's date, and ambient motion switched off. The site's default look follows the date ([ADR-0040](../adr/0040-daily-look-rotation.md)) and motion is on by default ([ADR-0039](../adr/0039-ambient-motion-is-a-per-browser-design-setting.md)); without the baseline, specs would pass or fail by weekday. It seeds both the 4321 and 4325 origins, and `tests/e2e/storage-baseline.test.ts` fails if it didn't land. See [TEST_BEST_PRACTICES § 29](../testing/TEST_BEST_PRACTICES.md#29--toggling-htmldark-theme-to-measure-dark-colours).

BL-153 briefly shipped a **second** server (`astro dev --port 4326 --ignore-lock`, `PUBLIC_I18N_LIVE_LOCALES=es,pt-BR`) so that spec could run against a forced-live set. It was removed the same day: in CI both servers boot cold and share `node_modules/.vite`, and one server's dependency re-optimization invalidated hashes the other had already handed out — 4321 answered TechPar's `import('chart.js')` with `504 Outdated Optimize Dep`, the trajectory legend rendered empty, and the suite went red in two consecutive runs (trace-confirmed). **Never run two Vite dev servers against the same checkout in one Playwright run.** If a forced-live server is ever needed again, it needs its own Vite `cacheDir` — and the simpler answer is the one taken: keep the registry the single source of liveness and let the spec fail loudly when it is not met. Details: [LOCALIZATION.md § Testing](LOCALIZATION.md#testing).
