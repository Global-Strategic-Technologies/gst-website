...
- **2026-09-03** — red on master again with **4 production advisories** (1 high, 3 moderate), all transitive under `agents → @modelcontextprotocol/sdk` and none introduced by a PR: `fast-uri@3.1.5` (four new SSRF / host-confusion advisories, `GHSA-5jgf-p345-68v8`, `GHSA-f65p-4m7j-42xc`, `GHSA-fph4-wmhf-6fwf`, `GHSA-jqff-g426-hqxp`; vulnerable `3.0.0–3.1.5`) and `qs@6.15.3` (`GHSA-x5fp-wj9c-mxmx` array-limit bypass, `GHSA-4mjr-xmp4-gh2g` DoS; vulnerable `<=6.15.3`), the latter counted three times through `body-parser` and `express@4` inside `express-rate-limit`. Cleared by advancing the existing `fast-uri` pin to `3.1.7` and adding a flat `qs: 6.16.0` override. Found during a Dependabot merge sweep, the same way as 2026-08-17: the audit job is not a required check, so it had been red without anyone being told.
...
- `qs: 6.16.0` (**top-level**, added 2026-09-03) — closes `GHSA-x5fp-wj9c-mxmx` + `GHSA-4mjr-xmp4-gh2g` (vulnerable `<=6.15.3`). Reached only through `express@4` and `body-parser@1` under `express-rate-limit` inside the SDK, both declaring `~6.15.1`; the override lifts them one minor. Remove when express 4 / body-parser 1 ship declaring `qs >=6.16.0`, or when the SDK drops `express-rate-limit` (check with `npm ls qs --all`).
