...
- `brace-expansion: 5.0.9` (**top-level**, added BL-106) — closes the DoS-via-unbounded-intermediate-arrays advisory that bypassed the CVE-2026-14257 mitigation (vulnerable `4.0.0–5.0.8`). Reaches the tree via `@astrojs/vercel → @vercel/nft → glob@13 → minimatch@10`. **This entry supersedes a "not an override" note**: the 2026-07 fix was a plain lockfile bump to `5.0.8`, on the reasoning that a flat override would also hit the unrelated dev `minimatch@3.1.5` (which needs `brace-expansion ^1.1.7`) and force a cross-major break. That risk did not materialise — the flat `5.0.9` override resolves cleanly and `npm ls` reports no invalid or missing packages, because the dev `minimatch@3` chain resolves its own `brace-expansion@1.x` in a nested position. Verified with the full website + mcp suites green.
