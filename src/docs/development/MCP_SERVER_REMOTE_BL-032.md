...
| Q13 | Upstash project sharing (NEW)                   | **Pivoted to two-database architecture (Path 2, 2026-05-05)** after Upstash ACL was discovered Prod-Pack-gated during Phase 6 § A.3 walkthrough. Worker now reads `inoreader:*` from the website's existing DB via its **Read-Only token** (free, ships alongside Standard) and reads/writes `mcp:*` against a dedicated free **MCP DB** with a separate Standard token. Achieves rotation isolation AND storage-layer Q4 enforcement on free tier. See full revision history in [Q13 § Resolved revision (2026-05-05)](#q13-upstash-project-sharing-new) | Phase 1 (`wrangler.toml` secret name); Phase 4 (single-DB code shipped); **Phase 6 prerequisite**: dual-client refactor for `UPSTASH_INOREADER_REST_*` + `UPSTASH_MCP_REST_*` secret split                                                              |
...
- **Upstash project**: **revised 2026-05-05 to Path 2 (two-database architecture)** — Worker reads `inoreader:*` from the website's DB via a Read-Only token, and reads/writes `mcp:*` against a dedicated free MCP DB with its own Standard token. Replaces the original "same project, separate REST token" plan after Upstash ACL was discovered Prod-Pack-gated. See [Q13 § Resolved revision (2026-05-05)](#q13-upstash-project-sharing-new) for full rationale
...
**Phase 4b outcome (2026-05-04, commit `db4a5a5`)**: rename shipped. Tool name `search_radar_cache` → `search_radar_offline`; file `radar-cache.ts` → `radar-offline.ts` (git mv preserves blame); exports `handleRadarCacheTool`/`registerRadarCacheTool`/`SearchRadarCacheInputSchema` → `handleRadarOfflineTool`/`registerRadarOfflineTool`/`SearchRadarOfflineInputSchema`. Deprecated alias registered separately by `registerSearchRadarCacheAlias()` from `_local-only.ts`; tail-calls the same handler and emits `[gst-mcp] DEPRECATION: ...` to stderr. `mcp-server` version bumped 0.0.1 → 0.1.0; `mcp-server/BREAKING_CHANGES.md` authored. Sibling-doc closure (BL-031.5 line 144 prediction edit, BACKLOG/BL-031.x text sweeps) deferred to BL-032 Phase 6 closure pass per the documentation deliverables catalog. All 364 tests pass post-rename.
...
**Phase 4c outcome (2026-05-04)**: shipped exactly as planned. [`mcp-server/src/content/radar-live-store.ts`](../../../mcp-server/src/content/radar-live-store.ts) is the Worker-compatible adapter; reads `mcp:radar:cache:wire` / `mcp:radar:cache:fyi` from Upstash with 6h TTL, falls back to Inoreader on cache miss via `inoreader-worker.ts` (Phase 4a). The transform logic was extracted to a new file [`mcp-server/src/content/radar-transform.ts`](../../../mcp-server/src/content/radar-transform.ts) — `toSnapshotItem(InoreaderItem, tier)` is shared by both `radar-snapshot.ts` (offline) and `radar-live-store.ts` (live), preserving the "two adapters, zero duplication of logic" target. Tools at [`mcp-server/src/tools/radar-live.ts`](../../../mcp-server/src/tools/radar-live.ts): `search_radar` + `get_latest_insights`, both registered by `createServer(env)` (transport-portable). The env parameter on `createServer()` is the BL-032 Phase 4c addition that lets radar-live tools capture the request-scoped Worker env in their handler closures (the Worker rebuilds `createServer(env)` per fetch; stdio calls `createServer()` with empty env and tools return `config-missing` if Inoreader creds aren't present locally).
...
**Phase 5 outcome (2026-05-04)**: shipped via `@sentry/cloudflare@10.51.0`. **One deviation from the recommendation, recorded by the user during the Q1-Q11 review on 2026-05-03**: a **separate Sentry project** (not the website's reused) — see the resolutions summary at the top of this doc. The new project name and DSN provisioning are operator concerns; the wiring is in [`mcp-server/src/observability/sentry.ts`](../../../mcp-server/src/observability/sentry.ts). Used `withSentry(optionsCallback, handler)` from `@sentry/cloudflare` as the wrapping pattern — graceful skip when SENTRY_DSN isn't bound (returns `undefined` from optionsCallback); per-request scope set via `tagRequest(keyOwner, path)` immediately after bearer auth so per-request events carry attribution. `SENTRY_MANUAL_SETUP.md` gained the "MCP Worker (BL-032 Phase 5)" section per the plan.
...
**Phase 5 outcome (2026-05-04)**: shipped exactly as planned. [`mcp-server/src/observability/inoreader-status.ts`](../../../mcp-server/src/observability/inoreader-status.ts) exposes `recordInoreaderStatus(env, 'ok' | 'degraded', note?)` and `readInoreaderStatus(env)` against the `mcp:inoreader:last-status` Upstash key with a 5-minute TTL. The radar-live tools' content adapter ([`radar-live-store.ts`](../../../mcp-server/src/content/radar-live-store.ts)) calls `recordInoreaderStatus(env, 'ok', 'wire' | 'fyi')` on every successful Inoreader fetch and `recordInoreaderStatus(env, 'degraded', 'wire:<reason>' | 'fyi:<reason>')` on every failure. [`mcp-server/src/observability/health.ts`](../../../mcp-server/src/observability/health.ts) reads this state plus a single Upstash GET liveness probe (`mcp:health:probe` — key doesn't need to exist; we just verify the REST endpoint responds) to assemble the BACKLOG-shaped `{ ok, version, gitSha, redis, inoreader, inoreaderObservedAt, phase }` payload. **`inoreader: 'unknown'` is intentionally NOT a degraded signal** — Worker cold-starts begin in this state and ramp to `'ok'` after the first radar-live call; reporting unknown as degraded would create page-oncall noise on every cold start. 8 unit tests cover the response-builder behaviors (ok-when-both-fine, ok-when-redis-fine-and-inoreader-unknown, degraded-when-redis-throws, degraded-when-inoreader-degraded, graceful-skip-when-creds-absent, malformed-cache-entry-falls-through-to-unknown, gitSha env propagation).
...
**Resolved revision (2026-05-05) — pivoted to Path 2 (two-database architecture)** during Phase 6 deploy when the original "separate REST token within the same project" plan hit Upstash platform constraints:

1. **Discovery (Phase 6 § A.3 walkthrough)**: Upstash's console does not ship a "Create New Token" button — each database has only Standard + Read-Only default tokens. Additional named/scoped tokens require the redis-cli `ACL SETUSER` + `ACL RESTTOKEN` commands.
2. **First pivot — ACL path**: DEPLOY.md was rewritten to walk through `ACL SETUSER mcp-worker on >$PASSWORD ~mcp:* ~inoreader:* +@all` followed by `ACL RESTTOKEN mcp-worker $PASSWORD`. Operator hit `ERR Command 'ACL' is available only with Prod Pack` — ACL is gated behind Upstash's paid Prod Pack add-on (per-database upgrade; bundles 99.99% SLA, multi-zone HA, encryption at rest, SOC-2 reporting, etc., none of which BL-032 needs).
3. **Second pivot — Path 2 (chosen)**: instead of ACL-scoping one DB, use **two free Upstash databases**:
   - **Inoreader DB** (the website's existing DB, unchanged): Worker accesses it via the website-DB's **Read-Only** token (free, ships alongside the Standard token). This enforces the Q4 read-only-on-`inoreader:*` invariant **at the storage layer** rather than as a code-only contract — even if Worker code regressed and tried to `set` an `inoreader:*` key, Upstash would reject it
   - **MCP DB** (new, free): Worker holds the Standard token; full read+write on `mcp:*` (rate-limit counters, circuit-breaker flag, health probe, inoreader-status cache). Rotation isolation is real — rotating this token has zero website-side blast radius
4. **Outcome vs. original goal**: Path 2 satisfies BOTH original Q13 goals (rotation isolation) AND the Q4 invariant (read-only-on-`inoreader:*`), at the storage layer rather than just in code, on free tier. Compromise of the bearer key + Wrangler-secret extraction now yields write access only to `mcp:*` keys, not the website's data.

Operational consequence: Worker holds **four** Upstash-related secrets per env instead of two — `UPSTASH_INOREADER_REST_URL`, `UPSTASH_INOREADER_REST_TOKEN` (read-only), `UPSTASH_MCP_REST_URL`, `UPSTASH_MCP_REST_TOKEN` (Standard). Worker code holds two Redis clients: one threaded through `inoreader-worker.ts` (Inoreader-DB, read paths only), one through `limiter.ts` / `circuit-breaker.ts` / `health.ts` / `inoreader-status.ts` (MCP-DB, read+write).

**Implementation rationale + sequencing**: see [Phase 5.5 — Path 2 dual-client refactor](#phase-55--path-2-dual-client-refactor-phase-6-prerequisite-1-2-days--shipped-2026-05-05) for the helper-function injection design, three-commit sequencing, verification gates, and operator deploy prerequisites. The refactor shipped 2026-05-05; DEPLOY.md § A.3 is now executable as the operator runbook.
...
│   │   ├── upstash-cache-store.ts     # CacheStore interface impl backed by @upstash/redis (MCP DB)
│   │   └── upstash-clients.ts         # Path 2 (Phase 5.5) — createInoreaderClient + createMcpClient factories
...
- **Docs**: [`mcp-server/src/docs/tools/radar/USAGE_REMOTE.md`](../../../mcp-server/src/docs/tools/radar/USAGE_REMOTE.md) authored — three end-to-end scenarios (live search by query, time-windowed FYI digest, agent-driven daily brief); [`mcp-server/src/docs/tools/radar/CONTRACT.md`](../../../mcp-server/src/docs/tools/radar/CONTRACT.md) gains `## Live tool surface (BL-032)` section; [`mcp-server/src/docs/tools/README.md`](../../../mcp-server/src/docs/tools/README.md) table gains `search_radar` + `get_latest_insights` rows; [`mcp-server/README.md`](../../../mcp-server/README.md) "What's exposed" Tools table gains the transport column + the two new tool rows; [`src/docs/hub/RADAR.md`](../hub/RADAR.md) "Inoreader budget envelope" subsection updated; this doc gets a `Resolved Phase 4: search_radar coexistence = ...` stanza recording how [Q2](#q2-search_radar-vs-search_radar_cache--coexistence-replacement-or-capability-mirror-revisited) actually settled
...
#### Phase 5.5 — Path 2 dual-client refactor (Phase 6 prerequisite, 1-2 days) — SHIPPED 2026-05-05

**Goal**: split the Worker's single Upstash Redis client into two — one read-only for `inoreader:*`, one read+write for `mcp:*` — so the [Q13](#q13-upstash-project-sharing-new) rotation-isolation goal AND the [Q4](#q4-inoreader-client-refactor--fork-or-generalize) read-only invariant are both enforced at the storage layer rather than as code-only contracts.

**Why this exists between Phase 5 and Phase 6**: the original Phase 4-5 code wired all Upstash access through a single `UPSTASH_REDIS_REST_URL`/`TOKEN` pair, with Q13's "separate REST token" assumption that Upstash supported per-token scoping in its console UI. Phase 6 § A.3 walkthrough revealed the assumption was wrong — ACL is gated behind paid Prod Pack — so the architecture pivoted to **Path 2**: two free Upstash databases (the website's existing DB read-only via Read-Only token, plus a new dedicated MCP DB). Full pivot history in [Q13's Resolved revision (2026-05-05)](#q13-upstash-project-sharing-new) stanza.

**Scope**:

- **Helper-function injection** (rejected: threading two clients through `createServer(env, clients)` — bigger signature footprint, no real win on stateless REST). New module [`mcp-server/src/lib/upstash-clients.ts`](../../../mcp-server/src/lib/upstash-clients.ts) exports two factories:
  - `createInoreaderClient(env)` — Inoreader DB (Read-Only token, `inoreader:*` keys)
  - `createMcpClient(env)` — MCP DB (Standard token, `mcp:*` keys)
- **Env interface change** in [`worker.ts`](../../../mcp-server/src/worker.ts): drop `UPSTASH_REDIS_REST_URL`/`TOKEN`; add `UPSTASH_INOREADER_REST_URL`/`TOKEN` + `UPSTASH_MCP_REST_URL`/`TOKEN`. No backward-compat shim (single internal operator)
- **6 consumer call sites** updated to use the appropriate helper: `inoreader-worker.ts` → `createInoreaderClient`; `upstash-cache-store.ts` / `limiter.ts` / `circuit-breaker.ts` / `inoreader-status.ts` / `health.ts` → `createMcpClient`
- **`/health` payload shape**: single `redis: 'ok' | 'degraded'` field replaced with two fields — `upstashMcp` + `upstashInoreader` — allowing operators to disambiguate "MCP DB misconfigured" from "website's Upstash project is degraded" without log triage. Aggregate `ok` flag = `upstashMcp === 'ok' && upstashInoreader === 'ok' && inoreader !== 'degraded'`
- **Inoreader-DB probe key** = `inoreader:access_token` (always exists, exercises auth + read paths). Probe-result value DISCARDED — never logged (would leak the access token)

**Three-commit sequencing on `feature-mcp1`**:

1. `refactor(mcp): split Upstash into Inoreader + MCP clients (Path 2)` — 8 production-code files, ~100 LOC. Atomic typecheck boundary
2. `test(mcp): align Worker tests for Path 2 dual-client architecture` — 6 test files (5 updated + 1 new `upstash-clients.test.ts` for constructor-routing regression coverage), ~60 LOC
3. `docs(mcp): align wrangler.toml + ops docs + BL-032 design doc with Path 2 secret matrix` — wrangler.toml comment block, RATE_LIMITS.md cross-reference, DEPLOY.md status-callout removal, this doc's Phase 5.5 + Q13 cross-reference + footer, ~50 LOC

**Verification gates** (each commit must pass before the next):

- After Commit 1: `npm run typecheck` MUST pass (tests still reference old env names — expected)
- After Commit 2: `npm test` MUST pass (399 tests / 41 files — was 388, +11 from new constructor-routing file)
- After Commit 3: `npx wrangler deploy --dry-run --env staging` MUST succeed

**Deploy-side prerequisites for Phase 6** (operator-side, per DEPLOY.md A.3):

1. Set 4 new Wrangler secrets per env: `UPSTASH_INOREADER_REST_URL`/`TOKEN` (Read-Only token from website's DB) + `UPSTASH_MCP_REST_URL`/`TOKEN` (Standard token from new MCP DB)
2. Verify via `wrangler secret list` before `wrangler deploy`
3. After first radar call, hit `/health` and confirm `inoreader: 'ok'` (not `'unknown'`) — proves the MCP-DB write path works (would silently fail if operator pasted Read-Only token where Standard was expected)
...
| `mcp-server/src/docs/tools/radar/USAGE_REMOTE.md`                                                                                                                                                                                                                                                   | 4                                                      |
| `mcp-server/src/docs/tools/radar/CONTRACT.md` § "Live tool surface (BL-032)"                                                                                                                                                                                                                        | 4                                                      |
| `mcp-server/src/docs/tools/README.md` (table extension)                                                                                                                                                                                                                                             | 4                                                      |
...
| Per-tool USAGE/CONTRACT updates for `search_radar_cache` rename ([`radar/USAGE.md`](../../../mcp-server/src/docs/tools/radar/USAGE.md), [`radar/CONTRACT.md`](../../../mcp-server/src/docs/tools/radar/CONTRACT.md), [`portfolio/USAGE.md`](../../../mcp-server/src/docs/tools/portfolio/USAGE.md)) | 4                                                      |
...
| [`mcp-server/src/docs/tools/radar/CONTRACT.md`](../../../mcp-server/src/docs/tools/radar/CONTRACT.md) | 4 | New `## Live tool surface (BL-032)` section beside the existing snapshot contract; cross-references the parallel USAGE files; documents the offline rename + alias deprecation |
| [`mcp-server/src/docs/tools/README.md`](../../../mcp-server/src/docs/tools/README.md) | 4 | Contracts table gains rows for `search_radar` (live) and `get_latest_insights`; cross-link to `radar/CONTRACT.md`'s new live section |
...
| Path                                              | Phase | What changes                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 |
| ------------------------------------------------- | ----- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| [`src/docs/README.md`](../README.md)              | 6     | "Quick Navigation" table gains a single row: **"Find any MCP server documentation" → [`mcp-server/README.md`](../../../mcp-server/README.md)**. The existing two rows ("See the MCP server in a concrete scenario" → `mcp-server/src/docs/tools/diligence/USAGE.md`; "Understand a Hub tool's input contract" → `mcp-server/src/docs/tools/README.md`) stay — they're answer-specific deep-links, complementary to the catalog entry, not duplicates                                                                                         |
...
_Last updated: 2026-05-05 — **Phase 5.5 (Path 2 dual-client refactor) shipped** in three commits on `feature-mcp1`: production code (8 files, ~100 LOC), tests (6 files, +11 new constructor-routing tests; 399/399 passing), and docs (wrangler.toml + RATE_LIMITS.md + DEPLOY.md callout removal + this doc's Phase 5.5 + Q13 cross-reference). File layout updated: `mcp-server/src/lib/upstash-clients.ts` is the new foundational module exporting `createInoreaderClient` + `createMcpClient`. Path 2 architectural decisions captured in the Phase 5.5 sub-section above; operational consequences in Q13's Resolved-revision (2026-05-05) stanza. DEPLOY.md § A.3 is now executable as the operator runbook for Phase 6._

_2026-05-05 (earlier) — Q13 pivoted to **Path 2 (two-database architecture)** after Phase 6 § A.3 walkthrough hit Upstash ACL gated behind paid Prod Pack. Path 2 uses two free Upstash DBs: Worker reads `inoreader:*` from website-DB via Read-Only token (storage-layer Q4 enforcement), reads/writes `mcp:*` from a new dedicated MCP DB via Standard token (rotation isolation). Q13 Resolved-revision stanza records the journey + four-secret matrix._

_2026-05-03 — pre-Phase-1 resolutions appended (Q1 SDK choice locked to Cloudflare `agents` SDK + stateless `createMcpHandler`; Q2/Q3/Q4 user-confirmed; Q12/Q13 added to surface radar-tool transport binding + Upstash sharing as explicit decisions; operational confirmations recorded). Doc was implementation-ready as of this update._
