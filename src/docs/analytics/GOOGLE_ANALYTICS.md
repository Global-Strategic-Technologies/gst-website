...
**Both are registered as GA4 custom _metrics_** — "Trial signup duration" (`duration_ms`) and "Trial mint duration" (`mint_ms`), event scope, unit Milliseconds, created 2026-09-17. Registration is **not retroactive**: only events collected after that moment are reportable, so the first measurement below does not appear in any report built on these metrics. Originally recorded as: register both as custom metrics (scope: event, unit: milliseconds) — the same step `locale` needs as a custom dimension (§ above). Unregistered params are collected and visible in DebugView but cannot be reported on. Neither name is a GA4 reserved parameter name, and neither uses a disallowed prefix (`_`, `firebase_`, `ga_`, `google_`, `gtag.`) — [checked against Google's reserved-names reference, 2026-09-17](https://support.google.com/analytics/answer/13316687).

The landing page's request-access link ALSO keeps its `trackCTA('mcp-request-access', 'hub-mcp')` call, so `cta_click` (§ 5) is unbroken and `mcp_request_access` can be declared a key event on its own. Enforced by `tests/unit/tool-analytics.test.ts` (prefix, category, the seven names) and `tests/unit/mcp-analytics.test.ts`. `tests/e2e/analytics.test.ts` wires `mcp_guide_view` and `mcp_endpoint_copied` end-to-end on `/hub/mcp/get-started/`; the **two trial events are not covered there** — they need a live Turnstile challenge, so their coverage is the unit suites plus `tests/e2e/hub-mcp-trial.test.ts`, which exercises the page's states without asserting on GA.
