...
**Register both as GA4 custom _metrics_** (scope: event, unit: milliseconds) — the same step `locale` needs as a custom dimension (§ above). Unregistered params are collected and visible in DebugView but cannot be reported on. Neither name is a GA4 reserved parameter name, and neither uses a disallowed prefix (`_`, `firebase_`, `ga_`, `google_`, `gtag.`) — [checked against Google's reserved-names reference, 2026-09-17](https://support.google.com/analytics/answer/13316687).
