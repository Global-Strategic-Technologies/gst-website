...
**Both are registered as GA4 custom _metrics_** — "Trial signup duration" (`duration_ms`) and "Trial mint duration" (`mint_ms`), event scope, unit Milliseconds, created 2026-09-17. Registration is **not retroactive**: only events collected after that moment are reportable, so the first measurement below does not appear in any report built on these metrics. Unregistered params are collected and visible in DebugView but cannot be reported on, which is what these two were until that date. Neither name is a GA4 reserved parameter name, and neither uses a disallowed prefix (`_`, `firebase_`, `ga_`, `google_`, `gtag.`) — [checked against Google's reserved-names reference, 2026-09-17](https://support.google.com/analytics/answer/13316687).
