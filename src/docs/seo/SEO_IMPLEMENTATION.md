...
- **AI-bot blocking** is on: requests from AI crawler and AI assistant user agents (GPTBot, ClaudeBot, Claude-User, ChatGPT-User, PerplexityBot, and others) get a Cloudflare 403 page ("Sorry, you have been blocked") on the `docs.` and `status.` hosts. Googlebot and generic clients are unaffected. Observed 2026-09-02 by user-agent probe; the block is user-agent based on the Free plan.
  ...
  Re-probed 2026-09-14 on `mcp-staging.globalstrategic.tech` after BL-152 added the registry document: with the `ClaudeBot`, `Claude-User` and `GPTBot` user agents, `/.well-known/mcp` and `/.well-known/oauth-authorization-server` return 200 while `/server.json` and `/authorize` return the Cloudflare 403. So the block exempts the `/.well-known/` prefix and nothing else. Consequence: **submit registries with the `/.well-known/mcp` alias, not `/server.json`**, unless a WAF skip rule for `/server.json` is added in the zone dashboard; the two paths serve identical bytes (`mcp-server/src/registry-metadata.ts`).
