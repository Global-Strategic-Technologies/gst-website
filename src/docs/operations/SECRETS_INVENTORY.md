...
| `OAUTH_M2M_SIGNING_KEY`                                                                          | ❌                        | ✅ (2026-07-24)       | ⏳ before prod OAuth rollout | BL-033 Slice 2 — HS256 signing key for self-contained M2M access tokens (`mcp_m2m_*`). Rotation = immediate mass revocation of all M2M tokens (that's the emergency lever, see AUTH.md § Revoke). Generate 32B random base64url; distinct value per env                                                         |
