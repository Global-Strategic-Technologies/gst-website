...
| `TURNSTILE_SECRET_KEY`                                                                           | ❌                        | ✅ (2026-09-07, test)   | ⏳ pending (Slice 3 gate)  | BL-155 Slice 2 — Cloudflare Turnstile widget secret, used only for server-side siteverify from `POST /trial/signup`. **The endpoint fails closed (503, nothing minted) while unbound**, so set it BEFORE the deploy that carries the code is approved. Staging uses the documented always-pass TEST secret paired with the test sitekey on the page; production gets the real widget's secret. **Production stays unset until Slice 3 lands the privacy disclosure and public copy** — that is the merge gate, not an oversight. `wrangler secret put TURNSTILE_SECRET_KEY --env <env>` (stdin) |
| `TRIAL_IP_HMAC_SECRET`                                                                           | ❌                        | ✅ (2026-09-07)         | ⏳ pending (Slice 3 gate)  | BL-155 Slice 2 — keys the per-visitor identity (HMAC-SHA256 of the client IP) that enforces one trial per identity per 30 days. **Rotating it forgets every identity at once** — every visitor becomes eligible for a fresh trial — so rotate deliberately, never routinely. Same fail-closed + set-before-approve rule as `TURNSTILE_SECRET_KEY`. Generate 32B random base64url; distinct value per env                                                                                                                                                                                        |
| `TURNSTILE_SITE_KEY` (public, website)                                                           | ⏳ Slice 3                | n/a                     | n/a                        | BL-155 Slice 3 — the widget's PUBLIC site key, rendered into the signup page (`PUBLIC_TURNSTILE_SITE_KEY`, Vercel env). Not a secret; listed so the pair is inventoried together. Preview/dev use the documented always-pass test sitekey, which must be paired with the test secret on staging                                                                                                                                                                                                                                                                                                 |
| `TURNSTILE_EXPECTED_HOSTNAMES` / `TRIAL_EXTRA_ORIGINS` (`[vars]`, not secrets)                   | n/a                       | wrangler.toml           | wrangler.toml (first only) | BL-155 Slice 2 — non-sensitive config committed in `wrangler.toml` `[env.<name>.vars]`: the hostnames a siteverify response must name, and (staging only) extra browser origins the trial endpoint's CORS accepts. Listed for the operational inventory                                                                                                                                                                                                                                                                                                                                         |
...
| `SENTRY_AUTH_TOKEN`                                                                              | ✅ — Production (website) | ❌                      | ❌                         | 🔑 **Two different tokens, one name.** The website's build-time source-map upload runs on **Vercel**; the MCP Worker's runs in **GitHub Actions** (shipped BL-037 Phase A, 2026-05-31 — this row previously said "comes with BL-037" long after it had). Never a Worker runtime secret in either case — it is consumed at build time. GitHub copy is one of the three pending repo-level deletions; see [§ GitHub Actions](#github-actions-cicd-deploy-credentials--bl-111)                                                                                                                     |
| `SENTRY_ORG` / `SENTRY_PROJECT`                                                                  | ✅ — Production (website) | ❌                      | ❌                         | Companion to `SENTRY_AUTH_TOKEN` on the website side. **Not secrets on the MCP side and never will be** — [`deploy.mjs:53-54`](../../../mcp-server/scripts/deploy.mjs) hardcodes `gst-7o` / `gst-mcp-server` as defaults, so BL-037 needed no bindings for them                                                                                                                                                                                                                                                                                                                                 |
...
| **2026-05-31** _(stale row closed 2026-08-05)_             | ✅ Completed | BL-037 Phase A delivery                                                                                                                              | _Added_ `SENTRY_AUTH_TOKEN` as a GitHub Actions secret for the MCP Worker deploy (repo secret created `2026-05-31T16:56Z`). `SENTRY_ORG` / `SENTRY_PROJECT` were **not** needed — [`deploy.mjs:53-54`](../../../mcp-server/scripts/deploy.mjs) hardcodes `gst-7o` / `gst-mcp-server` as defaults. The row sat "TBD / Pending" for nine weeks after shipping; the Date column means _when the action happened_, not when the row was corrected.                                                                                                                     |
