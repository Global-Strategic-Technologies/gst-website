...
**One question left open, deliberately**: whether `getMcpAuthContext` — the other symbol `agents/mcp/server` exports — is worth adopting or is redundant against our own `AuthSuccess` threading. Decision 4 largely moots it (we keep `agents` as a thin adapter and resolve auth ourselves before the handler runs), so it was not worth holding the initiative open for. It becomes live again only if decision 4 is revisited.
