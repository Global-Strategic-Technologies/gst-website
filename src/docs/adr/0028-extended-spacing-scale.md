...
BL-094 defers the analogous off-scale _font-size_ sweep because snapping a size to the nearest token **changes rendered type**, and the repo has no visual-regression coverage to catch a mistake. `STYLES_GUIDE.md:1028` states the prohibition.
...
`STYLES_GUIDE.md:1028` prohibits **snapping**, not tokenizing. The distinction is the whole basis of this ADR.

## Accepted residuals

Four off-scale **rem** spacing values remain in the six swept files, each kept deliberately. (Those files also keep blessed _px_ micro-spacing — `cards.css:351,544` `padding: 2px var(--spacing-sm)`, `:445,465` `1px`, `:107,741` `outline-offset: 2px` — which is exactly the `STYLES_GUIDE.md:1063` form and is not in scope here.)

| Value      | Site              | Why it stays                                                                                                                                                                                                                                                                                         |
| ---------- | ----------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `0.125rem` | `cards.css:223`   | Substantively the `STYLES_GUIDE.md:1063` badge case. That exception is written in **px** (`1px`/`2px`); this declaration is rem, and converting would break value-identity under a non-16px root — so the unit diverges from the exception on purpose.                                               |
...
| `0.375rem` | `Footer.astro:62` | 6px sits between `xs` (4px) and `sm` (8px); moving it moves pixels. `STYLES_GUIDE.md:939` also names `0.375rem`, but that exception is about approximating text height and does not cover this one.                                                                                                  |
