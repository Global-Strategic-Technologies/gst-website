...
**Stated precisely: this bounds the retry budget, not total latency.** Two fetch latencies still land on the response path, so a _hung_ Upstash delays an IRL tool call rather than being invisible to it. A construction-time `AbortSignal.timeout` is the wrong instrument for a per-request client — the signal would start at `createServer` rather than at the write and would abort spuriously on any tool running longer than the budget, losing counts on exactly the slow calls most worth counting. Recorded as a bounded residual rather than met with a mechanism that trades a delay for a lost count.
