...
- **Authoring rule**: a new text colour from the brand/status family uses its `-ink` token; brand teal text uses `--color-primary`. Fills, borders, chart strokes and specimen swatches keep the base token. **Amended by [ADR-0040](0040-primary-has-a-bright-variant-for-dark-pairings.md):** a primary fill under dark ink, primary text on a constant-dark surface, and decorative glows use `--color-primary-bright`.
