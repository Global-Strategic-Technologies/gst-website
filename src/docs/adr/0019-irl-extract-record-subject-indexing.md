...
- **The excerpt is capped at 240 characters, with a hard floor of 20 and word-boundary truncation.** Settled by measurement, not preference. Record bytes ≈ rows × (≈207 B of ref/request/status/tier overhead + min(answerChars, cap)); a real 60–80 KB body with ~67 answered rows averages ~1,000 chars of answer per row, so an uncapped record lands near the body's own size on top of the worksheet fence and up to eight `payload:` fences, while this cap holds it near 30 KB and roughly flat as bodies grow. The floor exists because [`diligence-audit.ts`](../../../mcp-server/src/schemas/diligence-audit.ts) requires a post-em-dash excerpt of ≥ 20 characters and, for tier 1, requires it to contain the dimension's enum value as a whole-token literal (`BL-045-TIER-1-LITERAL-MISMATCH`). **A byte-only cap that severs that token silently demotes every tier-1 citation to tier 2** — defeating the reason the excerpt is kept — so truncation is on a word boundary and a truncated excerpt is flagged rather than passed off as complete.
  ...
  **There are TWO floors, and conflating them is a real defect rather than a wording nicety.** The 20 is the tier-1 citation floor above — never emit an excerpt shorter than it. The **back-up floor is half the cap (120)**: word-boundary truncation only backs up to the last space when that space falls above 120, and hard-cuts at the cap otherwise. It exists because the schema REJECTS an excerpt flagged `excerptTruncated` that is shorter than half the declared cap — a flag on a span that plainly was not cut tells a consumer to distrust a citation it could have used at tier 1. Backing up to a boundary at 30 characters satisfies the 20 floor and fails validation, which is exactly what the helper did until code review constructed the input. Half the cap is DERIVED rather than its own constant, so the three places that state it — [`capIrlExtractExcerpt`](../../../mcp-server/src/schemas/irl-extract-record.ts), the `excerptTruncated` refinement, and the model-facing directive — are held together by a lockstep test rather than by a shared symbol. Changing one means changing three.
