...
- The "224px of usable width at 320" in the `JobCard` reversal was measured against `.container`'s flat 96px gutter, which [ADR-0027](0027-container-query-thresholds-are-container-facts.md) retired. That container is now 288px — though note it is the _container_, not the id's usable width: `.mdoc-job__body` and `.mdoc-step` padding take ~61px off it. The reversal stands; the wire id still cannot hold one line below 768px.
