...
- **Source initiative**: BL-144 (design handoff at `src/design_handoff_bl144_mono_pin/`; the BACKLOG stanza was pruned on completion — recover it with `git log -- src/docs/development/BACKLOG.md`)
...
**One face, self-hosted, named once.** `--font-family-mono` resolves to `'GST Mono'` — Geist Mono Variable (OFL 1.1, `vercel/geist-font` v1.7.2), subset to 367 codepoints and 25,952 bytes at `public/fonts/gst-mono-var-latin.woff2`, weight axis 100–900 intact. Declared in `src/styles/fonts.css`, imported first from `global.css` so the faces precede every consumer including the print rules.
...
**`font-display: swap` behind metric-matched fallbacks.** Two `local()`-only faces — `GST Mono Fallback` (Menlo/DejaVu at `size-adjust: 99.7%`) and `GST Mono Fallback WD` (Consolas at `109.1%`) — occupy the pinned face's box before the swap, so the swap changes glyph shapes and not one line break. `ascent-override`/`descent-override` are read off the file (100.5% / 29.5%), not from published tables.
...
| `WorkflowCard.astro`                  | `minmax(0, 1fr)` at ≤768 (a bare `1fr` floored the track at the nowrap wire id's min-content and blew the grid past the viewport by 57px at 320); the longest id scrolls there rather than splitting or shrinking.                                                                                                                                   |
...
**Two `/brand` specimen surfaces changed:** `CompositeLogo.astro`'s SVG wordmark carried `font-family="monospace"` as a presentation attribute and now goes through the token (verified to resolve in all three engines), and `filter.css` carried two hardcoded copies of the old sans stack — both found by the new guard, not by review.

**Worker-served HTML resolves the fallbacks, never a generic.** `mcp-server/` is a separate origin and cannot fetch the site's font, so `html-shell.ts` exports `MONO_FALLBACK_FACES` / `MONO_STACK` and inlines the `local()` faces. The percentages are duplicated across the workspace boundary and nothing enforces that they agree — the guard checks the faces are present, not that the numbers match.
