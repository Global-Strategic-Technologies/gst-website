...
**Amendment (2026-09-01).** Three figures above are historical rather than current, and none of them changes this ADR's decision:

- The "224px of usable width at 320" in the `JobCard` reversal was measured against `.container`'s flat 96px gutter, which [ADR-0027](0027-container-query-thresholds-are-container-facts.md) retired. That container is now 288px — and note it is the _container_, not the id's usable width. At 320px `JobCard`'s own `@media (max-width: 768px)` block resets `.mdoc-job__body`'s left padding to `--spacing-sm`, so the deduction is 8 + 8 + 12 + 3 = **31px**, not the 61px the desktop base rules take, leaving **~257px**. The reversal stands either way — 257px is still under the 267px of id ink — but do not carry a desktop deduction to a phone width.
- The CTA label that "fitted its button with 0.4px to spare" now clears by ~59px at 320px (measured: 168px of ink in a 227px interior), so `buttons.css`'s `--spacing-md` is retained by choice rather than by fit.
- The sash's 253.2 / 252.7 / 253.2px is still the correct measurement of the 35-character single-node subtext this ADR was written against, but that band has since been restructured into a two-field list. The live pair is recorded in `Sash.astro`'s header block — read it there rather than from any copy.

The pin also had a widening consequence this ADR did not anticipate. Its stated risk was clipping; the opposite direction bit us too. Geist Mono's 0.6em advance against Consolas' 0.55em grew the footer link row by 16.2px and pushed it to two lines on a 430px phone — recorded in TYPOGRAPHY_REFERENCE § The pinned mono.
