...
- **18 rows had a Status claiming an answer with an empty Response. In 17 of them the answer was sitting in a discarded column.** Exactly one row was genuinely unanswered.
...
**2. Comments is an answer, not a caveat, and is not labelled as unconfirmed.** `<answer>` is column G and column E joined into **one contiguous span**, G first, separated by a single space, with a period inserted after G only when G ends in an alphanumeric or a closing `)` `]` `"` `'`. Status does not gate inclusion.
...
**Accepted residual — pre-change workbooks are ambiguous.** Workbooks filled under the old Instructions, which invited caveats into Comments, legitimately mix GST research with recipient remarks. Stated precisely: the separation is lost **in the extracted body**, not absolutely. The workbook still holds all seven columns and an operator can open it, and the Comments-sourced ref list names exactly which rows to check. `generate-xlsx.ts` now tells recipients that D/E/F are read into the deliverable and routes non-answers ("scheduled for Q3 refresh", "confidential, discuss in call") to **Notes**, so the ambiguity does not accumulate going forward.

**Cosmetic edge, not special-cased**: a Response ending `…this."` satisfies the closing-quote condition and gains a period after the quote. The normalizer flattens both, so nothing downstream sees it; detecting "quoted content that already terminates" needs a parser this does not want.
