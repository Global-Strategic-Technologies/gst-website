...
The four onboarding guides under `/hub/mcp/` — `get-started/`, `using/`, `advanced-operations/` and `from-code/` — share one family sheet (`src/styles/components/mcp-guide.css`), one client script (`src/utils/mcp-onboarding.ts`) and one `TechArticle` helper. The first three implement the 2026-08 design handoff for the GST MCP Server and carry recorded media; `from-code/` (BL-156, 2026-09-14) is the developer half of the self-serve trial, carries no media, and is localized (its body is `src/page-templates/HubMcpFromCodePage.astro`, see [LOCALIZATION.md § Content tiers](../development/LOCALIZATION.md) for why it alone is). This doc records what is not obvious from the pages themselves: the clip-player pattern (the site's first `<video>` usage), the per-clip constraints the media imposes, the reduced-motion rule nothing in CI checks, and the re-encode recipes for regenerating the media. The hosting decision (media in git, WebP posters, the rejected alternatives) is [ADR-0022](../adr/0022-mcp-onboarding-media-in-git.md).

**Drift guard**: published server facts on the three media guides (prompt count, sweep engine count, fill-ratio halt rule, every cited `gst_*`/tool name, hostnames, the em-dash prohibition) are pinned against `mcp-server` source by `tests/integration/mcp-onboarding-parity.test.ts`. The from-code guide has its own guard, `tests/integration/mcp-from-code-parity.test.ts`, because its vocabulary is OAuth rather than tool names: it pins the endpoints, the token lifetime, the exact expiry error string, the radar refusal code and the trial ceilings, and it refuses the two integrations that do not exist. Page shape and behavior for all four are covered by `tests/e2e/hub-mcp-onboarding.test.ts`; all four routes, plus the Spanish from-code route, are in the `accessibility.test.ts` scan.
