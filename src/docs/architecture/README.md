...
| [SECRETS_INVENTORY.md](../operations/SECRETS_INVENTORY.md)                                                                       | **Links.** No secret is named in this directory.                                                                                          |
...
Two hard rules follow from the table. **No tool, prompt or resource counts appear anywhere in this directory** — `tests/integration/mcp-published-tool-count.test.ts` pins those figures in four docs, and a fifth uncontrolled copy would drift. And **no secret names, hostnames beyond the public ones, or namespace ids** — the inventory owns those.
