...
> **Superseded 2026-07-31.** `RadarFeedSkeleton.astro` was deleted when the Radar feed stopped being a `server:defer` island — deferring the page's primary content left it unindexed. The `@keyframes pulse` animation moved to `src/styles/components/skeleton.css` (it was never at `global.css:137`; that line is an unrelated media query). The documented pattern survives — the canonical reference is now the `/brand` specimens. See [RADAR.md](../hub/RADAR.md) (the section named here was itself superseded — see the stanza below).
