...
**Status**: Complete for **rem** spacing as of 2026-09-02 and for **px** spacing as of 2026-09-15 (BL-151), and enforced in both units. The earlier (March 2026) "Complete" was wrong about scope. The March 23, 2026 pass did replace actionable **pixel** spacing, and the micro-spacing exception it describes is real (1-3px badge padding and optical alignment — `cards.css:351` `padding: 2px var(--spacing-sm)` is exactly it). What was false is the claim that those exceptions were all that remained: **rem** spacing literals were never swept, and **321 of them across 38 files** were still in the source when ADR-0028 was written (2026-09-02).
...
- **Off-scale px values, ruled and kept** (BL-151) — `5px`, `6px`, `14px`, the sticky header clearances and the sash/nav-reserve derived constants. The 46 on-scale px literals (`4px` ×22, `8px` ×12, `12px` ×7, `16px` ×3, `28px` ×1, `40px` ×1) were swept to tokens; the micro-spacing exception was ruled to end at `3px`, so `4px` is `--spacing-xs`. Negative px values have no token and stay literal. ADR-0029 § The px half carries the table.
...
**Enforcement** (the absence of which is why these accumulated): a rem or px spacing literal with an exact token now fails `lint:css` at error severity, and the vitest guard covers all of `src/` rather than six files. See [STYLES_GUIDE § 3](STYLES_GUIDE.md#3-hardcoded-spacing) for the reach of each.
...
**Micro-spacing exception**: `1px`, `2px` or `3px`, written in px, are acceptable for badge padding, border-radius fine-tuning, and optical alignment adjustments, since the spacing scale starts at 4px. `4px` is `--spacing-xs` and is not covered (BL-151). See STYLES_GUIDE.md for the documented exception.
...
> **Superseded 2026-07-31.** `RadarFeedSkeleton.astro` was deleted when the Radar feed stopped being a `server:defer` island — deferring the page's primary content left it unindexed. The `@keyframes pulse` animation moved to `src/styles/components/skeleton.css` (it was never at `global.css:137`; that line is an unrelated media query). The documented pattern survives — the canonical reference is now the `/brand` specimens. See [RADAR.md](../hub/RADAR.md); the section this originally pointed at ("Why the feed is not a server island") was itself superseded — see the stanza below.
...
2. **All 100 color violations fixed.** The large majority are same-value substitutions with no rendered change. **Nine are deliberate value changes**, each fixing a real defect where the literal was the _light-theme_ hex of a semantic color frozen into both themes — so the declaration silently ignored dark theme and every alternative palette:
   ...
   A tenth change does not fit the "frozen hex" framing but is listed for completeness: the brand page's `kbd` keycap moved from an opaque `light-dark(#ffffff, #1a1a1a)` to `--surface-tint-bg` (a 5% neutral tint). The original was already theme-switched; the change is an improvement — a white keycap on the `--bg-light-alt` row was nearly invisible in light theme.
