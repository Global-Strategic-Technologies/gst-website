...
- **One source, two surfaces — and a THIRD that is deliberately not it**. `src/data/library/information-request-list/article.md` is the single source of truth for (1) the Hub page `/hub/library/information-request-list/`, which imports the markdown rather than duplicating it, and (2) the MCP Resource `gst://library/information-request-list`, codegenned into `library-data.generated.ts` at prebuild (never hand-edited). The Resource form exists so agent contexts can scope "everything we need to know about a target" with a single `resources/read` call. **The prompt `gst_irl_create` is NOT a third surface of that article**: it calls `embedIrlGeneratorSource()`, which embeds the decoupled generator source `src/data/irl/information-request-list.md` under the inline label `gst://irl/source`. `src/prompts/embed.ts` states the reason in terms — the library article is free-form prose and may differ from the list. The two sources diverge today by exactly two Section 00 rows. This matters beyond bookkeeping: the generator source is the taxonomy the workbook is built from, so it is the taxonomy the answers come back against, and it is what the IRL extract record keys off. A note claiming the library article is the prompt's lineage would designate the wrong artifact as authoritative for every reference the record carries.
...
| ------------------------ | -------------------------------------------------------------------------------------------------------------------- |
| Target / client name     | `targetName` arg in `gst_diligence_kickoff`, `gst_target_quick_look`, `gst_diligence_handoff_memo`, `gst_irl_create` |
| Engagement context       | `transactionContext` arg in `gst_irl_create`; informs `gst_diligence_kickoff` framing                                |
...
| Bullet                            | Feeds                                                                                                                                                                                                                                                                         |
| --------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| One-paragraph product description | `gst_architecture_layer_review.targetSummary`; `gst_irl_create.productSummary` (light)                                                                                                                                                                                        |
...
| `gst_irl_create` prompt body                            | Embeds the canonical Resource as the second message in both modes; the model reproduces the bullets verbatim in the paste-ready text artifact. When called with args, also orchestrates the XLSX generator (additive behavior — interactive mode unchanged). | Body-mention invariant from `assertPromptInvariants` requires the Resource URI to appear literally in the rendered text; the per-mode `orchestrates` body-mention contract (URI in both modes; tool name in one-shot only) is asserted by [`mcp-server/tests/unit/prompts/irl-create.test.ts`](../../../tests/unit/prompts/irl-create.test.ts). |
...
3. If no match exists, the IRL has a coverage gap. Resolve in the same PR:
   ...
   - **Verify** the `gst_irl_create` golden file still captures representative output (re-record if the artifact's shape changed materially).
...
5. **Re-record the prompt golden** (`mcp-server/tests/examples/irl-create.golden.md`) if the article shape change is material enough to alter the deliverable's structure.
...
| -------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `rdOpEx`       | **No IRL source — synthesized.** Under `deepdive` the engine replaces it with the three components. Pass `rdOpEx: null` and omit `_audit.rdOpEx` (optional under `deepdive` since server 0.65.0); the field itself must still be present |
...
| `toolingCost`  | Section 02 — "Annual build and tooling cost". **A blank component bullet** (this, `prodCost`, or `engCost`'s inputs) is passed as `null` with `_audit` source `irl-absent` — never 0; the tool lists it in `extractionOnly`              |
...
### `gst_target_quick_look` — the fourth `compute_techpar` caller, and its tech-debt sources (2026-08-20)

`gst_target_quick_look` takes five form arguments and no IRL, so it was left out of the rows above. It is nonetheless a `compute_techpar` caller and an `estimate_tech_debt_cost` caller, and both of its calls were non-conformant: the TechPar call stated no `mode` at all, and the tech-debt call omitted the `_audit` sibling the schema requires — a call that fails validation as written. Both are now branch-conditional on whether canonical target evidence (an IRL extract record, a filled IRL, a supplied target document) is present in context.

| Quick-look call          | No evidence in context                                                                                                                                                                              | Evidence in context                                                                                             |
| ------------------------ | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------- |
| `compute_techpar.mode`   | `quick`. Nothing supplies the Section 02 components, so `deepdive` would synthesize R&D OpEx by summing three zeros                                                                                 | `deepdive`, per the rule above — the components exist in the record                                             |
| `compute_techpar._audit` | The `Section --` partner-supplied form, from `buildPartnerSuppliedTechParAudit()`. Under the derived-tier discipline that sentinel grades as `partner-supplied`, not as a verified IRL citation     | `Section NN — <the covering fact's excerpt>`, section derived from the fact's reference (`0-03` → `Section 00`) |
| `_audit.mttrSource`      | `irl-absent` with `mttrHours: null`. The enum has **no value meaning "synthesized from stage norms"**, and any non-`irl-stated` source forces the value to null — the tool rejects a non-null value | `irl-stated` with the Section 04 figure, P1 not P0, per the MTTR rule                                           |
| `_audit.incidentsSource` | `irl-absent` with `incidents: null`; the tool returns `extractionOnly` and the brief renders the field as extraction-only rather than quoting a carrying cost                                       | `irl-stated` with the most-recent-quarter count converted to monthly                                            |

**Why this belongs in this document rather than only in the prompt.** The reason the two calls were wrong is the reason every row here exists: an input with no mapping does not stay empty. `mode` had no row, so it was chosen unguided; `_audit` had no row for a non-IRL caller, so it was omitted entirely. The evidence branch is also what makes the quick-look call _auditable_ for the first time — before it, a real Section-00 ARR figure and a stage-norm guess arrived at identical provenance grade.
