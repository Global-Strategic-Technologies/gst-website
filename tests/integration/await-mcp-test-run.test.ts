...
function run(opts: RunOpts = {}): { code: number; output: string } {
  ...
  // A 1 s cap with 0.2 s polls: the cap arms resolve in ~1.2 s, and cases that expect the
  // loop to reach a verdict pass capS: '5' so a slow machine cannot cut them short. The
  // script's cap is wall-clock (BL-111), so these are real seconds.
...
const body = (json: string) => `printf '%s' '${json}'`;
...
describe('await-mcp-test-run.sh — the suite reached a verdict', () => {
  ...
  2) ${body(PENDING)} ;;
  *) ${body(SUCCESS)} ;;`);
    expect(run({ targetSha: SHA, repo: 'o/r', capS: '5' }).code).toBe(0);
  });
  ...
  *) ${body(CANCELLED_THEN_SUCCESS)} ;;`);
  ...
  it('exits 1 fast when every run is terminal and none succeeded', () => {
    ...
    expect(run({ targetSha: SHA, repo: 'o/r', capS: '5' }).code).toBe(1);
  });
...
describe('await-mcp-test-run.sh — inputs are validated before the first API call', () => {
  ...
  it('exits 2 — not 5 or 6 — when a required binary is missing from PATH', () => {
    ...
    // PATH is narrowed to the stub dir alone — `gh` resolves, `node` does not. The inner
    // shell is invoked through `$BASH` (the running shell's own absolute path) rather than
    // by name, because a PATH that hides node hides bash too, and a 127 would prove
    // nothing about the probe.
    const r = spawnSync('bash', ['-c', 'PATH="$STUB" "$BASH" "$TARGET_SCRIPT"'], {
      cwd: process.cwd(),
      env: {
        ...process.env,
    ...
    expect(`${r.stdout}${r.stderr}`).toContain('is not on PATH');
  });
});
