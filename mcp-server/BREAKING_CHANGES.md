...
- 6 Radar URIs.
- **17** tool names (`fill_information_request_list_xlsx` added by 0.59.0 / BL-140; `list_irl_requests` added by the 0.37.0 per-question-removal work; tool names are NOT manifest-hash inputs — the count here is descriptive). **What this figure counts**, stated 2026-08-28 because three docs quote three different tool counts and none said its basis: **distinct stdio tools, excluding the deprecated `search_radar_cache` alias**. The other two are also correct on their own bases — **18** stdio _registrations_ including that alias ([`ARCHITECTURE.md`](./src/docs/ARCHITECTURE.md), whose sentence names the stdio-only pair), and **16** on the **remote Worker** (`EXPECTED_REMOTE_TOOL_COUNT` in `tests/integration/helpers/mcp-registry.ts`, the only one of the three under test), which omits both `search_radar_offline` and `search_radar_cache` because they need `node:fs` (see `src/tools/_local-only.ts`). This ledger's series has held to its own basis since 15 → 16 → 17; the exact stdio roster is pinned by `tests/integration/protocol-roundtrip.test.ts`.
