...
- `embedIrlGeneratorSource` **strips directive comment lines** at the embed boundary — both prompts (`gst_information_request_list`, `gst_irl_ingestion`) and both modes receive a comment-free body. Because the strip restores the pre-tag bytes, the 7 `irl-ingestion-body-hash-stability` hashes did NOT drift and `gst_irl_ingestion` needs no version bump.
