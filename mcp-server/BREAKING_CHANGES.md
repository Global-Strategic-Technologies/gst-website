...
- **Same fix on two sibling prompts**: `gst_information_request_list` `0.0.7` → `0.0.8` (**five** optional args — `targetName`, `companyName`, `projectName`, `customRequests` and `productSummary` — all returned the identical `-32602`, and `targetName`'s own description reads "Omit to emit the universal template", so the all-blank form was the documented happy path. `transactionContext` moves to `enumFromWire` and therefore also gains case-folding: `Buy-Side` now succeeds where it previously failed) and `gst_comparable_engagements_memo` `0.0.2` → `0.0.3` (its two arguments had **no `.describe()` at all**, so Desktop rendered blank uninterpretable fields; the empty-string handling there was already correct and is hygiene only).
