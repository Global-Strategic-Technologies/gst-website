...
```
7286e5a349798caf91404e2efee97960e08f6a5a7f8b318af20a8929bcc855af
```
...
- 6 Radar URIs.
- **17** tool names (`fill_information_request_list_xlsx` added by 0.59.0 / BL-140; `list_irl_requests` added by the 0.37.0 per-question-removal work; tool names are NOT manifest-hash inputs — the count here is descriptive). **What this figure counts**, stated 2026-08-28 because **four** docs quote three different tool counts and none said its basis: **distinct stdio tools, excluding the deprecated `search_radar_cache` alias**. The other two are also correct on their own bases — **18** stdio _registrations_ including that alias ([`ARCHITECTURE.md`](./src/docs/ARCHITECTURE.md), whose sentence names the stdio-only pair; the server [`README.md`](./README.md) and [`src/docs/testing/README.md`](./src/docs/testing/README.md) publish the same pair), and **16** on the **remote Worker** (`EXPECTED_REMOTE_TOOL_COUNT`), which omits both `search_radar_offline` and `search_radar_cache` because they need `node:fs` (see `src/tools/_local-only.ts`). This ledger's series has held to its own basis since 15 → 16 → 17. **All ten published figures are now bound to the registry** by the repo-root guard `<repo>/tests/integration/mcp-published-tool-count.test.ts` (website workspace), which reads `src/server.ts` and `src/tools/_local-only.ts`; the exact stdio roster is pinned by this workspace's own `mcp-server/tests/integration/protocol-roundtrip.test.ts`.
- **12** prompt `name@version` tuples — **0.66.0 bumped eleven**, every prompt except `gst_irl_populate` (unchanged at `0.3.0`, confirmed by rendering it against master): minor for `gst_target_quick_look` → `0.4.0`, `gst_irl_create` → `0.2.0`, `gst_irl_sweep` → `0.5.0`, `gst_irl_extract` → `0.4.0` and the five other `irlEvidencePrecedence()` consumers → `0.2.0`; patch for `gst_irl_ingestion` → `0.30.3` and `gst_radar_brief_today` → `0.0.6` (the prompt-wording audit). **0.65.0 bumped four** (`gst_irl_sweep` → `0.4.0`, `gst_irl_extract` → `0.3.0`, `gst_target_quick_look` → `0.3.0`, `gst_irl_ingestion` → `0.30.2`; the TechPar `rdOpEx` placeholder replaced by `null` + `irl-absent`, BL-163). **0.64.0 bumped three** (`gst_irl_sweep` → `0.3.0`, `gst_irl_extract` → `0.2.0`, `gst_target_quick_look` → `0.2.0`; the `infraHostingAnnual` selection rule, BL-163). **0.63.0 moved three of them and recycled a name** (`gst_irl_ingestion` → `0.30.1`, a stale-name-literal patch carried by the renames): `gst_irl_create@0.2.0` renamed to `gst_irl_populate@0.3.0` (the handoff it names also moved to `gst_irl_sweep`), and `gst_information_request_list@0.0.9` renamed to `gst_irl_create@0.1.0`, taking the name the first rename vacated — so `gst_irl_create` identifies a **different prompt** before and after 0.63.0 (see that stanza's callout). `gst_irl_fill@0.1.0` renamed to `gst_irl_create@0.2.0` by 0.62.0; `gst_irl_extract@0.1.0` added and `gst_irl_sweep` bumped to `0.2.0` by 0.61.0 (the extract-only split); `gst_irl_sweep@0.1.0` added by 0.60.0 (trust-the-operator rebuild PR1, coexisting with `gst_irl_ingestion@0.30.0` until live verification clears its removal); `gst_irl_fill@0.1.0` added by 0.59.0 (BL-140), the first NEW tuple since the manifest was baselined at nine. **Seven of the prior nine moved in 0.56.0** — `gst_irl_ingestion` to `0.29.0` (the IRL extract record; `extract-only` reachable without `filledIrl`), and the six prompts that now carry `irlEvidencePrecedence()` to `0.1.0`: `gst_target_quick_look`, `gst_diligence_kickoff`, `gst_diligence_handoff_memo`, `gst_architecture_layer_review`, `gst_comparable_engagements_memo`, `gst_regulatory_exposure_brief`. `gst_radar_brief_today` sat at `0.0.5` (provenance caveat added — see the 0.48.2 stanza below) until 0.66.0. `gst_irl_ingestion`'s prior milestones: capped `irlSource`, inlined VDR taxonomy, blank-field handling, the flattened-body refusal withdrawn, the body states its own resolved run parameters, `compute_techpar` runs in a stated mode, the server derives `fillRatio`.
...
**Prompt grades** (a prompt with both kinds of change takes the higher grade)

- **Minor — an instruction changed:**
  - `gst_target_quick_look` 0.3.0 → 0.4.0: the evidence branch renders the V2 MTTR rule (P1 not P0, `mttrHours: null` rather than a placeholder), which agrees with the prompt's own 3a/3b contract where the V1 wording named a dossier (J) list the brief does not have.
  - `gst_irl_create` 0.1.0 → 0.2.0: the three `productSummary` rules agree — annotate a bullet the summary already answers, never compress or drop it.
  - `gst_irl_sweep` 0.4.0 → 0.5.0 and `gst_irl_extract` 0.3.0 → 0.4.0: a new taxonomy framing (`embeddedTaxonomyFramingForReconcile`) replaces the generator's "reproduce it as-is" continuation, which contradicted bodies that reconcile against the taxonomy and never reproduce it. The sweep's "call each tool ONCE" now names its two exceptions (the ICG fetch-then-seed pair, one `search_regulations` call per framework).
  - The five other `irlEvidencePrecedence()` consumers — `gst_architecture_layer_review`, `gst_comparable_engagements_memo`, `gst_diligence_handoff_memo`, `gst_diligence_kickoff`, `gst_regulatory_exposure_brief` — 0.1.0 → 0.2.0: the verified-vs-asserted clause now has a branch for a v2 record (`_meta.recordVersion` `2.0`), which carries no `_meta.irlBodyHash` and so cannot be upgraded by hash. `gst_target_quick_look` carries the same clause; its grade is already minor. `gst_diligence_kickoff` and `gst_diligence_handoff_memo` also stop calling the `_audit` sibling required — it has been optional since 0.60.0 — while still asking for it; the six clauses now state the body-cache lifetime per transport (stdio LRU eviction, Worker TTL) from the same `IRL_BODY_CACHE_LIFETIME_TEXT` phrase the tool descriptions use.
- **Patch — wording or a fact only:**
  - `gst_irl_ingestion` 0.30.2 → 0.30.3, the 0.30.0 → 0.30.1 precedent: backlog-ID archaeology (the `BL-045` / `BL-063-*` rule-ID family the tools return stays), "no longer / now" phrasing, volume markers and sentence-count caps removed; Steps 1, 1a, 4a and 6a stop calling the optional `_audit` sibling required; the per-section "MUST close … non-optional" deeplink rule stated once (its reason already lived under Voice + format); the stale `claude-opus-4-7` model-id example; a stray backtick; the interactive Step 4 named the internal `Bl076BodyCacheMissError` class where the wire code is `cache-miss`. No directive was added or removed.
  - `gst_radar_brief_today` 0.0.5 → 0.0.6: the body claimed a 24-hour cache TTL, which was wrong on every transport (the Worker's radar cache lifetime is 6 hours, matching the 6-hourly refresh cron — `CACHE_TTL_SECONDS` in `src/content/radar-live-store.ts`; stdio reads a seeded snapshot). The body now says only that the snapshot is a periodically refreshed cache, keeping deployment specifics out of it as its degraded-path test requires.

The other prompts drop the same archaeology and numeric sentence caps (dated `BL-…` phase tags, "older server build" hedges, "3-4 sentences" limbs) alongside the graded changes above.

**Tool-description corrections** (no schema change)

- `prepare_irl_body`, `compose_dossier_envelope` and `validate_irl_provenance` describe the server-side body cache as it behaves: the size cap and cache lifetime (4 hours on the remote Worker; the stdio cache keeps the most recent 16 entries with no time limit), and the error codes a caller actually receives (`cache-miss`, `hash-mismatch`, `invalid-input`) in place of internal class names such as `Bl076BodyCacheMissError`.
- `get_latest_insights` states its boundary against `search_radar` (no Wire tier, no `deeplink`) and the FYI cap, so a `limit` above the cap reads as the no-op it is.
- `assess_infrastructure_cost_governance` enumerates its question IDs from the ICG domain catalog, so a cold call can build a valid answers map.
- `estimate_tech_debt_cost`, `search_regulations`, `generate_diligence_agenda` and `compute_techpar` self-document their parameters.
- Backlog-ID archaeology is removed from tool descriptions.
- The `search_radar_cache` alias description no longer promises removal "in 0.2.0", a version this package passed long ago; the alias remains deprecated and its removal stays tracked in the backlog.

See [`ARCHITECTURE.md` § IRL body provenance](./src/docs/ARCHITECTURE.md#irl-body-provenance-bl-123) and [§ Persistence posture](./src/docs/ARCHITECTURE.md#persistence-posture--what-the-server-holds-and-for-how-long) for the body cache, and [§ URI stability and the manifest hash](./src/docs/ARCHITECTURE.md#uri-stability-and-the-manifest-hash) for the tuple rule.

**Client impact**: none required. A client pinned to a prompt by name keeps resolving it; one that compares `promptVersion` sees the new values above.

---

## 0.65.0 — 2026-09-17 — `compute_techpar` accepts absent money figures

**Additive and wire-compatible, graded minor.** Every payload valid under 0.64.0 still parses and computes identically. **Prompts**: `gst_irl_sweep` 0.3.0 → 0.4.0, `gst_irl_extract` 0.2.0 → 0.3.0, `gst_target_quick_look` 0.2.0 → 0.3.0, `gst_irl_ingestion` 0.30.1 → 0.30.2 — bumped because their previous bytes were served. **Manifest hash**: rebaselined (four tuples moved). **Body hashes**: all 16 `gst_irl_ingestion` scenarios rebaselined (the mode rule renders in all three builders; verified to be the only drift).

**What changed** (BL-163 item 1)

- **Nullable money fields.** `infraPersonnel`, `rdOpEx`, `rdCapEx`, `engCost`, `prodCost` and `toolingCost` accept `null` at the MCP boundary when the source material does not supply the figure. `arr` and `infraHostingAnnual` stay non-null. The handler computes nulls as 0; the website engine and wizard are unchanged.
- **New response key `extractionOnly`** (always present, `string[]`): the null fields the chosen mode uses. A null in a field the mode discards (`rdOpEx` under `deepdive`, the components under `quick`) is not listed. Clients that reject unknown keys must allow it.
- **New `_audit` source `irl-absent`**, paired with a null input. The handler rejects `irl-absent` with a non-null value (`BL-163-TECHPAR-NULL-REQUIRED-FOR-ABSENT-SOURCE`) and a null value under any other source (`BL-163-TECHPAR-ABSENT-SOURCE-REQUIRED-FOR-NULL`).
- **`_audit.rdOpEx` is optional under `deepdive`** (still required under `quick`, `BL-163-TECHPAR-QUICK-RDOPEX-AUDIT-REQUIRED`). `quick` with `rdOpEx: null` is rejected as `invalid-input`.
- **Prompt rule**: `TECHPAR_MODE_RULE` / `_V2` no longer prescribe the forced placeholder (`rdOpEx: 0` + `irl-annualized-stated` + `Section --`). Under `deepdive` they instruct `rdOpEx: null` with no `_audit.rdOpEx`, and a blank Section-02 component as `null` + `irl-absent` instead of 0. `gst_target_quick_look` also drops two stale claims that `_audit` is required (optional since 0.60.0).

**Client impact**: none required. The old placeholder shape remains valid; callers can adopt `null` to stop a missing component from reading as a real zero.

---

## 0.64.0 — 2026-09-17 — `search_portfolio` omits batched filters from its deeplink

**One tool-output change and one prompt-rule addition, graded minor** (precedent: 0.49.0's `search_regulations` change). **Prompts**: `gst_irl_sweep` 0.2.0 → 0.3.0, `gst_irl_extract` 0.1.0 → 0.2.0, `gst_target_quick_look` 0.1.0 → 0.2.0 — bumped, not rebaselined, because each prompt's previous bytes were served. **Manifest hash**: rebaselined (three tuples moved; the deeplink change is invisible to it).
...
- **`infraHostingAnnual` selection rule** (BL-163 item 2): a new `INFRA_HOSTING_ANNUALIZATION_RULE` in `extraction-rules.ts` — annualize the multi-month Section 03 actual (period total ÷ months × 12), never one month × 12 — now renders in `gst_irl_sweep` and `gst_irl_extract` (via `IRL_EXTRACTION_RULES_SECTION`) and in `gst_target_quick_look`'s evidence branch. BL-126's two runs over one IRL chose the two derivations ($3.40M vs $3.50M) and both passed the handler's cross-check; before this, the rule existed only inline in `gst_irl_ingestion`, which the sweep does not inherit.
...
>
> This is the one change here that fails silently rather than loudly, so it is stated before anything else. Between 0.62.0 and 0.63.0 `gst_irl_create` was the **evidence-population** workflow — inventory evidence, author D/E fills, call `fill_information_request_list_xlsx`, stop. From 0.63.0 it is the **blank-IRL issuer** — emit the universal intake checklist and its empty `.xlsx`.
>
> A client pinned to `/gst_irl_create` against 0.62.0 therefore does **not** get "prompt not found". It gets a working prompt that does the opposite end of the workflow. Anyone who adopted `/gst_irl_create` in the 0.62.0–0.63.0 window (2026-08-26 to 2026-08-28) wants **`/gst_irl_populate`**.
>
> The window is two days and the surface was staging-only for it, which is why the reuse was accepted rather than the name being retired. Recorded here because the manifest hash cannot see it: the hash covers `name@version` tuples, and a name leaving one tuple and arriving on another drifts it exactly as an unrelated pair of renames would.

**What changed**

- **`gst_irl_create`** (formerly `gst_information_request_list`) is the same blank-IRL workflow: same nine arguments, same one-shot and interactive branches, same companion tool (`generate_information_request_list_xlsx`, whose name does not move), same `consumesTargetEvidence` exclusion. Operator ruling 2026-08-28: the IRL prompts share a `gst_irl_*` namespace. Only the self-naming line, the module/test/golden filenames, and the tool's published description move. Graded a **minor** bump, matching how 0.62.0 and the populate rename above graded an identity change on an unchanged body.
- **`gst_irl_ingestion` moves for a reason of its own** — a patch, not a rename. It calls itself "the bookend to `gst_information_request_list`" in all four of its rendered body branches and in its published description, so leaving it untouched would have shipped a body pointing a model at a slash command that no longer resolves. Prose-only correction of a stale name literal; same grade as the `0.21.0` → `0.21.1` stale-literal fix. All 16 body hashes drift because the sentence renders unconditionally on every branch (verified by building each, not inferred).
...
**Client impact**: `/gst_information_request_list` no longer resolves — invoke `/gst_irl_create`. `/gst_irl_create` still resolves but **reaches a different prompt** than it did in 0.62.0 — see the callout above; invoke `/gst_irl_populate` for the evidence-population workflow. Arguments and behavior are otherwise unchanged on both prompts, and a client following the populate prompt is now directed to a different, coexisting successor prompt for ingestion.
...
## 0.59.0 — 2026-08-23 — BL-140: the IRL can be populated from evidence; new tool + new prompt, frozen path untouched

**Additive only — nothing renamed, nothing removed.** **Tool**: `fill_information_request_list_xlsx` (17th tool). **Prompt**: `gst_irl_fill@0.1.0` (10th tuple). **Manifest hash**: rebaselined (one added tuple).

**What changed**

- **New tool `fill_information_request_list_xlsx`**: produces a POPULATED IRL `.xlsx` from model-authored per-row `fills` — sourcing reference into File Location (D) under the ADR-0021 grammar (no em-dash, no parens, no control characters, `; `-joined segments, bracketed non-document origins), answer into Comments (E). Both fields required per entry, so "every answered row carries a well-shaped D" and "the fill never writes D without E" are structural. Rows evidence cannot answer stay blank — the partially populated workbook IS the follow-up ask. Stops at the artifact; never invokes the sweep.
- **New prompt `gst_irl_fill`**: evidence inventory → authored fills under the sourcing grammar → the fill tool with a server-computed scoping payload → stop-at-artifact. Deliberately NOT `consumesTargetEvidence` (the clause's upgrade path would instruct the sweep tools); the exclusion and reason are recorded in the biconditional guard.
- **The entire pre-existing IRL path is frozen and byte-proven**: the five IRL tools, both prior IRL prompts, and `npm run irl:extract` are untouched; `generateIrlXlsxBuffer` gained an additive optional prefill parameter with the frozen generator's output pinned byte-identical by an entry-level golden captured from pre-change code.

**Client impact**: none for existing consumers — no name, URI, argument, or output shape moved. New capability is opt-in by calling the new surfaces.
...
**Why it went unnoticed for four versions.** The requirement was real and written down — in the BL-033 risk line, [`OPERATOR_RUNBOOK.md`](../src/docs/development/OPERATOR_RUNBOOK.md), and the `/hub/mcp/` marketing copy on the then-unmerged `feat/mcp-website-marketing` branch, whose parity test then asserted the page tells prospects radar content "should not be auto-actioned" (that callout and its assertions were removed at operator direction on 2026-08-17 — see the BL-033 AC in [`BACKLOG.md`](../src/docs/development/BACKLOG.md); the argument below is strengthened by the removal, not weakened, since the requirement's only remaining home is the executable surface this version added). It existed in **no** surface that actually emitted the content: not this prompt body, not any of the other eight, not a tool description, and not the recorded golden. The golden encoded the same omission, so every comparison against it agreed. Found by the BL-119 cycle-2 UAT run, which tested the requirement rather than the recorded output.
