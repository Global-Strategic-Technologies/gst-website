...
/**
 * Taxonomy framing for `gst_irl_sweep` / `gst_irl_extract`. Additive (the
 * `embeddedTaxonomyFraming(true)` branch is pinned by the ingestion body-hash
 * baseline): those bodies use the embed to reconcile minimally-formatted
 * replies, so the generator's "reproduce it as-is" continuation contradicts
 * them — neither reproduces the taxonomy.
 */
export function embeddedTaxonomyFramingForReconcile(): string {
  return `${TAXONOMY_LEAD} use it as given, without checking it against another source. **It is NOT the filled IRL and must not be swept.** A blank template arriving where a populated one is expected reads like a paste that lost its answers; it is neither. The filled body is whatever the user supplies through \`filledIrl\`, attaches to the conversation, or pastes in reply.`;
}
