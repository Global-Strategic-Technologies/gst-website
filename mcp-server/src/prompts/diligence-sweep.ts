...
function buildOneShotBody(args: {
  ...
  return [
    ...
    '',
    `Step 1 — Extract the 13 diligence dimensions from the IRL and invoke \`generate_diligence_agenda\`. ${UNKNOWN_PROPAGATION_RULE} Surface the resulting agenda topics + attention areas + the \`deeplink\` URL.`,
    '',
    ...
    '',
    `Step 6 — Invoke \`estimate_tech_debt_cost\` using IRL Section 04. Key inputs: maintenance burden percentage (Section 04 active-maintenance bullet), deployment frequency (Section 04), incidents quarterly trend + MTTR (Section 04), planned remediation budget (Section 04), team size (Section 07 engineering headcount), average salary (Section 07). ${MTTR_P1_RULE} **Surface the \`deeplink\` URL in the dossier** — it opens the Tech Debt Calculator with sliders pre-positioned. Surface the resulting annualized debt-carry cost + payback projection + deeplink.`,
    '',
