...
function buildOneShotBody(args: {
  ...
  return [
    ...
    '',
    `Step 2 — Pull comparable engagements. Call \`list_portfolio_facets\` first to see the filterable dimensions of GST's 57-engagement portfolio, then call \`search_portfolio\` with filters that match the target's profile (productType, industry-equivalent from Section 01, growthStage, geographies). Pull 3-5 comparables. Surface the resulting code-named matches + the \`deeplink\` URL.`,
    '',
    `Step 3 — Pull the regulatory framework bodies the target is exposed to. Call \`list_regulation_facets\` first to enumerate available jurisdictions, then call \`search_regulations\` once per framework named in IRL Section 09 (e.g., HIPAA, GDPR, BDSG, CNIL, CCPA — whichever the filled IRL surfaces). Cite article numbers verbatim when summarizing obligations; do NOT invent citations beyond what the framework bodies return.`,
    '',
    `Step 4 — Invoke \`compute_techpar\` using the architecture and engineering-cost data from IRL Section 02 + Section 03 + Section 07. Key inputs: engineering FTE count (Section 02), product personnel cost (Section 02), annual build/tooling cost (Section 02), monthly hosting + infra spend (Section 03 — annualize the 3-month average), infrastructure headcount (Section 03), material capex (Section 03), average fully-loaded engineering salary (Section 07). Toggle the capex view per the capex bullet in Section 03. Surface the resulting paradigm assessment + total R&D OpEx.`,
    '',
    `Step 5 — Invoke \`assess_infrastructure_cost_governance\` using IRL Section 03 (hosting model + spend trajectory) + Section 02 (tooling + technical-debt-assessment) + Section 07 (FinOps-adjacent headcount, if any). Capture the ICG maturity scoring across domains and surface 3-5 prioritized recommendations.`,
    '',
    `Step 6 — Invoke \`estimate_tech_debt_cost\` using IRL Section 04. Key inputs: maintenance burden percentage (Section 04 active-maintenance bullet), deployment frequency (Section 04), incidents quarterly trend + MTTR (Section 04), planned remediation budget (Section 04), team size (Section 07 engineering headcount), average salary (Section 07). Surface the resulting annualized debt-carry cost + payback projection.`,
    '',
    `Step 7 — Invoke \`search_radar\` for the target's product segment + geographies (derive search terms from IRL Section 01 product description and Section 00 geographies). Surface the 3-5 most relevant radar items as market-signal context — what is the broader market saying about this space right now?`,
    '',
    ...
    '',
    "  **(C) Architecture + paradigm assessment** — pull from Step 4 (`compute_techpar`). Frame as a 2-3 paragraph read on the target's technical paradigm, R&D OpEx posture, and biggest architectural-cost drivers visible from the IRL.",
    '',
    '  **(D) Infrastructure cost governance assessment** — pull from Step 5 (`assess_infrastructure_cost_governance`). Maturity scores + 3-5 prioritized recommendations, ranked by leverage.',
    '',
    '  **(E) Technical debt assessment** — pull from Step 6 (`estimate_tech_debt_cost`). Annualized carry cost, headline payback projection, and the 1-2 most expensive debt categories the IRL surfaced (e.g., legacy services flagged for rewrite).',
    '',
    '  **(F) Regulatory exposure** — pull from Step 3. One subsection per jurisdiction/framework, citing verbatim article numbers. Cross-jurisdictional transfer mechanism review if the IRL Section 09 flags cross-border data flows.',
    '',
    ...
    '',
    "  **(H) Market signal** — pull from Step 7. 2-3 bullet summary of what the radar items reveal about the target's market timing.",
    '',
    ...
    "- Honor every tool's `deeplink` field when surfaced — pass it through as a clickable Hub link, do not invent URLs.",
...
const INTERACTIVE_BODY = [
  ...
  '',
  `Step 3. Compose the unified dossier — nine sections (A–I) following the gst_diligence_sweep one-shot layout (target snapshot · diligence agenda · architecture · ICG · tech debt · regulatory · comparables · market signal · synthesis). Reference the canonical VDR taxonomy (\`${VDR_RESOURCE_URI}\`) verbatim for follow-up document requests in the synthesis section.`,
  '',
  "Voice: dossier-quality. The output reads as a single coherent partner-level document. Surface concrete numbers from the IRL verbatim. Honor every tool's `deeplink` field as a clickable Hub link. Do NOT fabricate data the IRL did not supply.",
].join('\n');
