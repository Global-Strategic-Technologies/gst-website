    ...
    messages: [
      {
        ...
        content: {
          ...
          text: [
            ...
            '',
            '  **Adaptation note for 2b (this prompt is not the ingestion sweep).** The rule above is written in IRL-sweep terms and directs surfacing blanks "in (J)" and marking a dossier section — this prompt emits neither a (J) gap list nor a dossier. Map both directives onto this prompt\'s own output: a null (absent) component goes under the brief\'s "Assumptions / unknowns" heading, named with the Section 02 reference that would have answered it and the consequence (a missing component understates total technology cost and moves the zone verdict in the flattering direction). The Section-02 / reference language in the rule is meaningful here precisely because the record carries those references. Build each `_audit` entry from the covering fact — `Section NN — <the fact\'s excerpt>` — rather than from the `Section --` sentinel; a real figure and a norm-derived guess must not arrive at the same provenance grade. "The rule above" means both rules quoted in 2b — the TechPar mode rule and the hosting annualization rule.',
            '',
            '  **Both branches: every field below must be PRESENT in BOTH modes.** Beyond `arr`, `stage`, `mode`, `capexView` and `growthRate`, the schema demands `exitMultiple`, `engFTE`, `engCost`, `prodCost`, `toolingCost`, `infraHostingAnnual`, `infraPersonnel`, `rdOpEx` and `rdCapEx` — omitting any one is a validation rejection, not a default. Every money field except `arr` and `infraHostingAnnual` accepts `null` when nothing supplies it; pass null rather than 0, and the response lists the gaps in `extractionOnly`. `deepdive` DISCARDS `rdOpEx` (pass null); `quick` discards the three components (pass null) and reads `rdOpEx` directly, so under `quick` it must be a number. Whatever you supplied from stage norms rather than from evidence is listed under "Assumptions / unknowns".',
            '',
            ...
            '',
            '  **3a — Send the `_audit` sibling.** It is optional, but validated when present, and this prompt relies on it. It carries `mttrSource` and `incidentsSource`, each one of `irl-stated` | `irl-open` | `irl-absent` | `irl-scope-mismatch`. The enum has no value meaning "synthesized from stage norms", and that is deliberate: any source other than `irl-stated` forces the paired value to `null`, and the tool REJECTS a non-null value under those sources (it returns a retry directive; nothing coerces).',
            '',
