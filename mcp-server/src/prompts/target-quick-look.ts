    ...
    messages: [
      {
        ...
        content: {
          ...
          text: [
            ...
            '',
            `Step 2 — Unit-economics benchmark (\`compute_techpar\`). Pass \`stage: '${args.stage}'\` (the same canonical value); TechPar's MCP wrapper translates locally. Use the supplied arr; choose reasonable defaults for capexView and growthRate where not derivable. **BL-045 PR B Phase 2 — the tool REQUIRES the \`_audit\` sibling.** Supply Tier-3 partner-supplied defaults: \`_audit.monetaryBasis: { currency: "USD", citation: "Section -- — partner-supplied form input — arr from quick-look form" }\` and per-monetary-field \`{ annualizationSource: "irl-annualized-stated", citation: "Section -- — partner-supplied form input — <field>" }\` for arr / infraHostingAnnual / infraPersonnel / rdOpEx / rdCapEx (the engine ignores deepdive sub-fields in quick mode; omit their audits). If you choose a non-USD currency for any reason, supply \`_audit.monetaryBasis.conversionRate\` (USD rate) so the response payload carries it.`,
            '',
