...
const ARRIVAL_AND_INFERENCE = [
  ...
  '- **Engagement context** (voice only — it never changes which tools run): the `> Engagement context:` metadata line if present, else row 0-02. Row 0-02 is absent on most pipeline-generated IRLs (the generator omits it when a context was stated at generation time), so the header line is the source that will usually be there. The line carries display labels — `Sell-side`, `Buy-side`, `Value Creation`, `Unspecified`. **When the line is absent OR reads `Unspecified`, write in universal voice.**',
