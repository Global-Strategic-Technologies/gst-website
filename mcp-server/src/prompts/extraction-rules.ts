 ...
 *
 * BL-163: the wire shape used to force a placeholder — `rdOpEx: 0` plus an
 * `_audit.rdOpEx` claiming `irl-annualized-stated` with a `Section --`
 * citation — because the field and its audit were required in both modes and
 * no audit source meant "absent". Both are now expressible honestly: `rdOpEx`
 * is null under deepdive with no `_audit.rdOpEx`, and a blank component is
 * null with `irl-absent`, reported back in `extractionOnly`.
 */
export const TECHPAR_MODE_RULE =
  '**Run `compute_techpar` in `mode: "deepdive"`. Always.** This is not a judgement call: `deepdive` synthesizes R&D OpEx from `engCost + prodCost + toolingCost`, and IRL Section 02 asks the target for the product-personnel and tooling components directly while `engCost` derives from its FTE breakdown against the Section 07 salary band. `quick` instead takes `rdOpEx` as a direct input, and **no IRL bullet anywhere asks for a total R&D OpEx figure** — so in `quick` mode that required input has no source and gets improvised, which is what produced a 1.9x swing and an inverted zone verdict across two runs of the same IRL. **Wire shape under `deepdive`:** pass `rdOpEx: null` and omit `_audit.rdOpEx` — the engine discards the value in this mode, so there is no figure and no provenance to supply. Do not compute a figure for it; in particular do NOT source it from the Section 04 technical-debt remediation figure: that bullet feeds the Tech Debt Calculator input `remediationBudget`, a different tool, and routing it here has already happened once. **If a Section-02 COMPONENT bullet (`engCost` / `prodCost` / `toolingCost`) is blank or `n/a`**, pass `null` for that component — never 0 as a stand-in — with an `_audit` entry of `{ annualizationSource: "irl-absent", citation: "Section 02 — <which bullet was blank>" }`. The tool computes it as 0 and lists it in the response\'s `extractionOnly`; say so explicitly in the (J) gap list — name the component, the Section 02 bullet that would have answered it, and the consequence, which is that a missing component understates total technology cost and moves the zone verdict in the flattering direction. For those three, do NOT invent an annualization source: they are figures the engine actually uses, so a fabricated provenance there corrupts a number the dossier rests on.';
 ...
 * `assess_infrastructure_cost_governance`. The seeding philosophy prevents
 * the model from defensively flooding the payload with "Not sure" answers and
 * mechanically forcing a Reactive score. It used to justify that by claiming
 * the engine penalizes `-1` more harshly than `0`; BL-126 found that only
 * partly true (`icg-engine.ts` floors each domain at 0, so an all-`-1` domain
 * scores the same as an all-`0` one), and BL-129 corrected the wording.
...
export const UNKNOWN_PROPAGATION_RULE_V2 = [
  ...
  "**For `businessModel` and `operatingModel`**: default to `'unknown'` unless the IRL uses one of the enum values literally (Tier 1) OR provides direct evidence that uniquely maps (Tier 2).",
  '',
  '**Currency normalization (applies before any bracketed-monetary dimension is assigned)**: convert every monetary bullet in a non-USD currency to USD before bracket assignment, and state the conversion inline where the figure is used. Worked example — an IRL bullet "Implied ARR run-rate ~$31M CAD" converts as `$31M CAD × 0.73 USD/CAD ≈ $22.6M USD ⇒ revenueRange: 5-25m`. Worked example — EUR shape: "ARR €18M FY26" converts as `€18M × 1.08 USD/EUR ≈ $19.4M USD ⇒ revenueRange: 5-25m`. Worked example — GBP shape: same form. If the converted value lands within 10% of a bracket boundary (e.g., USD $23-27M against the 25m boundary), pass `\'unknown\'` and surface the currency / conversion question in the Gaps & assumptions list — bracket misassignment compounds downstream so prefer `\'unknown\'` to a fragile commitment. Treating a non-USD bullet as if it were USD is the most common bracketing error in real-world runs.',
  '',
  '**`headcount` scope (the `generate_diligence_agenda` `headcount` field)**: "engineering headcount" means engineering ICs + engineering management only. It excludes product managers, designers, and standalone QA unless QA reports into engineering. Worked example: IRL Section 02 bullet "Engineering ~42: Development team 33 ... Infra/DevOps/DBA 9. Product ~6. ~15 of 48 R&D+Product are contractors" → use 42 (Eng-only), NOT 48 (R&D+Product), and state the distinction inline where the headcount is used (`headcount ← Section 02: Engineering 42; Product 6 excluded`). When the IRL distinguishes "engineering ~N1" from "R&D + Product ~N2" or similar, N1 is always the right input. Lumping product/design into engineering headcount mis-routes the agenda to higher-tier probes.',
  '',
  '**`dataSensitivity` bucket boundaries (Tier 2 guidance)**: the enum is `low` / `moderate` / `high`. Bucket boundaries:',
  '- **`low`**: employee PII only (names, schedules, wages, performance, HR-IDs) and/or operational metadata + telemetry; no regulated category; no customer/shopper PII at scale; no PHI; no PCI card data; no government-classified data.',
  '- **`moderate`**: customer/shopper PII at scale; financial-transaction metadata (not card numbers); employee PII combined with customer PII at scale; non-card financial data.',
...
/**
 * V2 of `TECHPAR_MODE_RULE`: keeps always-`deepdive`, the `rdOpEx: null`
 * wire shape and its rationale, the Section-04-remediation anti-mapping, and
 * the blank-component gap-list rule; drops the `_audit` wire shape (the new
 * prompts send bare payloads).
 */
export const TECHPAR_MODE_RULE_V2 =
  '**Run `compute_techpar` in `mode: "deepdive"`. Always.** This is not a judgement call: `deepdive` synthesizes R&D OpEx from `engCost + prodCost + toolingCost`, and IRL Section 02 asks the target for the product-personnel and tooling components directly while `engCost` derives from its FTE breakdown against the Section 07 salary band. `quick` instead takes `rdOpEx` as a direct input, and **no IRL bullet anywhere asks for a total R&D OpEx figure** — so in `quick` mode that required input has no source and gets improvised, which is what produced a 1.9x swing and an inverted zone verdict across two runs of the same IRL. **Wire shape under `deepdive`: pass `rdOpEx: null`** — the engine discards the value in this mode; do not compute a figure for it. In particular do NOT source it from the Section 04 technical-debt remediation figure: that bullet feeds the Tech Debt Calculator input `remediationBudget`, a different tool. **If a Section-02 COMPONENT bullet (`engCost` / `prodCost` / `toolingCost`) is blank or `n/a`**, pass `null` for that component — never 0 as a stand-in. The tool computes it as 0 and lists it in the response\'s `extractionOnly`; say so explicitly in the Gaps & assumptions list — name the component, the Section 02 bullet that would have answered it, and the consequence, which is that a missing component understates total technology cost and moves the zone verdict in the flattering direction.';
...
export const MTTR_P1_RULE_V2 =
  '**MTTR input — use P1 (the workhorse number):** if Section 04 lists MTTR separately for P0 and P1 (e.g., "Mean time to resolution P0 2.4h, P1 7.8h"), pass the P1 value to the tool. P0s are rare (typically one or two per year at this scale); P1s drive the steady-state incident-carrying cost the model is computing. Use the P1 value itself — not the P0 number, a midpoint, or an average: the engine multiplies MTTR × incidents linearly, so picking the wrong scalar understates carrying cost by the full ratio (P1/P0 ≈ 3× for typical operations). **MTTR unfilled**: if Section 04 lists no MTTR value, marks MTTR as OPEN, or says "not yet tracked" / "n/a", pass `mttrHours: null` instead of a placeholder (24h, 8h, or any arbitrary anchor) — a fabricated MTTR passes through the engine\'s linear multiplier and produces an unrecoverable false carrying-cost number. The tool elides the field and returns it in `extractionOnly`, and the dossier marks the Tech Debt section extraction-only for that field. Surface the missing MTTR in the Gaps & assumptions list as a target follow-up (e.g., "Pull 24-month JQL for client-incident MTTR over the period; replace the omitted-field marker once available"). **Incident frequency input:** use the most recent quarterly count from the trend, converted to monthly. If Section 04 shows a declining trend (e.g., "FY24-Q1 8 incidents... FY25-Q4 4 incidents"), use the most-recent quarter\'s monthly equivalent (4/3 ≈ 1.3/month), not an inflated round number. **If incident counts are themselves OPEN/unfilled**, apply the same guard — pass null, mark extraction-only, surface in Gaps & assumptions.';
...
export const IRL_EXTRACTION_RULES_SECTION = [
  '## Extraction rules (engine math — each one prevents a specific wrong number)',
  '',
