...
const BL_045_VERIFY_DIRECTIVE = [
  ...
  'conditionalTriggers:',
  '  considered: [<every trigger you evaluated, fired or not>]',
  '  fired: [<subset that actually fired>]',
  ...
  '- **`conditionalTriggers` block (BL-058 expansion)** — three lists to make trigger reasoning transparent:',
  '  - `considered`: every trigger you EVALUATED at any point — fired or not. Operators audit completeness here (did you even consider EU_AI_ACT for an AI-deploying target?).',
  ...
  '- **`response` block (BL-058)** — catches truncation/continuation pathologies that invalidate the audit artifact:',
...
const INTERACTIVE_BODY = [
  ...
  '',
  '`conditionalTriggers` block (BL-058 expansion) reporting discipline: `considered` is every trigger you evaluated at any point; `fired` is the subset you decided to fire; `suppressedWithRationale` lists `{trigger, whyNot}` for every trigger in `considered` but NOT in `fired`. Operators audit completeness here — an empty `considered` list for an AI-deploying target with regulatory-disclosure sections is itself a red flag.',
  '',
