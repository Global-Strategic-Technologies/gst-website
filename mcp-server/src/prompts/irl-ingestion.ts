...
const BL_045_VERIFY_DIRECTIVE = [
  ...
  'precheckIterations: <int — count of validate_irl_provenance calls BEFORE firstEnvelopeCall; null when firstEnvelopeCall is null>',
  ...
  '- `precheckIterations` (BL-056) counts `validate_irl_provenance` invocations made BEFORE `firstEnvelopeCall`. Excludes any post-envelope provenance probes. Healthy band on a thorough partner-paste IRL is 1–3. `0` means the BL-051 precheck directive was elided; `4` (the cap) means the model could not converge precheck to the 0.90 coverage threshold — surface either to the operator. High `precheckIterations` + `selfCorrectionCalls: 0` is the healthy post-BL-051 pattern (precheck did the work upfront); low `precheckIterations` + high `selfCorrectionCalls` is the pre-BL-051 anti-pattern resurfaced. Set to `null` only when `firstEnvelopeCall` is null.',
...
const INTERACTIVE_BODY = [
  ...
  '',
  '`precheckIterations` (BL-056) reporting discipline: count every `validate_irl_provenance` call you made BEFORE the first `compose_dossier_envelope`. Do NOT include post-envelope provenance probes. `0` is honest if you genuinely elided the BL-051 precheck directive — but understand that operators will read it as non-compliance. Set to `null` only when no envelope call happened at all (interactive-paste-request scenario). DO NOT omit this field.',
  '',
