...
const BL_045_VERIFY_DIRECTIVE = [
  ...
  "  - **`defaultFiredFrameworks` (BL-062)**: frameworks the model fires via the gate-5 Section-09 evidence path (NOT via conditional-trigger evaluation). One entry per framework named in the IRL's Section 09. Empty list `[]` is honest when Section 09 named no frameworks. This list and `considered` partition cleanly — no framework appears in both.",
...
const INTERACTIVE_BODY = [
  ...
  '',
  "`conditionalTriggers` block (BL-058 expansion + BL-062) reporting discipline: `considered` is every CONDITIONAL TRIGGER you evaluated — currently EU_AI_ACT and NIS2 are the only named conditional triggers; do NOT list Section-09-enumerated frameworks here. `fired` is the subset of `considered` you decided to fire. `suppressedWithRationale` lists `{trigger, whyNot}` for every trigger in `considered` but NOT in `fired`. **`defaultFiredFrameworks` (BL-062)** is the separate list of Section-09-enumerated frameworks fired via the gate-5 evidence path — frameworks named verbatim in the IRL's Section 09 (GDPR, UK GDPR, PIPEDA, POPIA, Australia Privacy Act, etc.). The two lists partition cleanly: no framework appears in both. Empty `considered: []` for an AI-deploying target is a red flag; empty `defaultFiredFrameworks: []` is honest when Section 09 named no frameworks.",
  '',
