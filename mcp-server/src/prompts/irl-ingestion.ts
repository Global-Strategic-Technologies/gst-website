...
const argsSchema = z.object({
  filledIrl: stringFromWire(z.string().min(200).optional())
    .optional()
    .describe(
      'Optional. Omit to enter interactive mode (the model will ask you to paste it). The populated Information Request List returned by the target — the entire markdown body, all 10 sections. Pasting into a single-line client field collapses the line breaks; the run still works, and the audit block reports newlines: 0 so you can see why the body will not hash-match your source file.'
    ),
  ...
  partnerLead: stringFromWire(z.string().min(1).optional())
    .optional()
    .describe(
  ...
  projectCodeName: stringFromWire(z.string().min(1).optional())
    .optional()
    .describe(
...
const META_JSON_FENCE_DIRECTIVE = [
  ...
  '  "modelVersion": "<your model id at invocation time, e.g. claude-opus-5>",',
  ...
  '  "transactionContext": "<copy from the Run parameters block above>",',
...
// BL-125 (#5) — these two directives carried six conditional back-references
// to the RUN-AUDIT block ("if you are emitting a RUN-AUDIT block…"), and the
// block only renders at `auditLevel: debug` in full mode. The hedging was
// correct and nothing malfunctioned, but one of them pointed at "the block's
// own section" for rules that were absent, and a model reading them at
// `enhanced` re-derived two documented behaviours as anomalies and reported
// them as findings. Both consts become functions of `showRunAudit` so the
// references render only where the section they reference does. Neither is
// exported, so there are no callers outside this module.
const buildEnvelopePrecheckDirective = (showRunAudit: boolean): string =>
  [
    '## Envelope precheck — citation iteration via `validate_irl_provenance` (BLOCKING — full mode, every audit level)',
    '',
    'BEFORE calling `compose_dossier_envelope`, run the citation-iteration loop on `validate_irl_provenance`. The envelope tool is heavyweight (its input is ~30KB on a full sweep: claims + gaps + filledIrl + meta); the verifier is purpose-built for fast iteration (input is just `filledIrl` + `claims`). Converge citation correctness on the cheap tool, then call the expensive one ONCE on the clean set — a pre-converged set verifies at 80-90% on the first envelope call and avoids repeated heavyweight calls.',
    '',
    'Loop:',
    '',
    ...
    '',
    'Then — and only then — call `compose_dossier_envelope` with the clean claims set. Target: 1 envelope call total, with no self-correction re-calls.' +
      (showRunAudit
        ? ' In the RUN-AUDIT block below that target is `totalEnvelopeCalls: 1`, `selfCorrectionCalls: 0`.'
        : ''),
    '',
...
const buildEnvelopeCompositionDirective = (showRunAudit: boolean): string =>
  [
    ...
    '',
    '- `promptName` / `promptVersion` / `modelVersion` — `promptName` is `gst_irl_ingestion`; `promptVersion` is server-derived (the tool overrides whatever you send); `modelVersion` is your own model id. **`mode` / `auditLevel` / `transactionContext` are stated in the Run parameters block at the top of this body — copy them from there, do not re-derive them from what rendered.**',
    ...
    "- `irlSource` — REQUIRED. Pass `partner-paste-verbatim-prepop` on this path (the directive appears above, you skipped `prepare_irl_body`, the server pre-populated the cache from the operator's prompt arg). **The server checks this claim and may CAP it.** It holds a record of who actually wrote the cached body; if that record says `prepare_irl_body` rather than the prompt render, your `-prepop` is recorded as `partner-paste-verbatim` and the gap list carries a `provenance-gap:` entry saying so. Nothing is ever upgraded, and a reconstruction or `placeholder` claim is never touched." +
      (showRunAudit
        ...
        : ''),
    '- `requireVerbatimBody` — accuracy-critical run gate. **Its resolved value is stated in the Run parameters block at the top of this body; copy it from there** — the server reads the gate only from the tool input you supply. Under `true` the server REFUSES any `irlSource` other than the partner-paste forms (`partner-paste-verbatim` or `partner-paste-verbatim-prepop`); both are accepted, so on this prepop path the gate passes. Under `false` omit the field from the tool call rather than passing `false` explicitly.',
    '',
    ...
    '',
    // The FIELD is named at every audit level: the envelope returns it at every
    // level, and a model that does not know it exists cannot use it. Only the
    // transcription rules are `debug`-scoped — BL-071 pins the unconditional
    // mention and BL-122 established that the reporting contract has exactly one
    // home. Gating the whole paragraph on `showRunAudit` (attempted in BL-125,
    // caught by BL-071's test) collapses that distinction.
    '**`serverToolCallCounts` (server-authoritative)**. The envelope output also returns `serverToolCallCounts` — a server-authoritative snapshot of tool calls (`{ attempted, succeeded, rejected, errored }` per tool) — alongside **`countersScope`**, which states HOW FAR BACK that snapshot reaches.' +
      (showRunAudit
        ? ' In the RUN-AUDIT block below they become `toolCallCounts` and `countersScope`, copied BOTH VERBATIM. DO NOT self-narrate the counters or estimate from memory: the server counts are the source of truth, and model self-narration has demonstrated drift across runs.'
        : ''),
    '',
...
const RUN_AUDIT_DIRECTIVE = [
  ...
  '  newlines: <int — newline count of that same body, copied from `serverCachedBodyNewlines` in the envelope output>',
  ...
  'countersScope: session | run | request | null  # in FULL mode copy VERBATIM from compose_dossier_envelope output; states how far back toolCallCounts reaches. In extract-only there is no envelope call and therefore no snapshot — report null',
  'toolCallCounts:  # in FULL mode copy VERBATIM from compose_dossier_envelope output `serverToolCallCounts`. In extract-only report {} — the server emitted no snapshot to copy',
  '  validate_irl_provenance: { attempted: N, succeeded: N, rejected: N, errored: N }',
  '  compose_dossier_envelope: { attempted: N, succeeded: N-1, rejected: N, errored: N }  # the envelope is in-flight while it computes its own snapshot, so succeeded trails attempted by one. Expected, not a defect — copy as served',
  ...
  '- **`filledIrl` block** — operator cross-checks the body the model actually submitted against what the partner sent:',
  ...
  '- **`precheck` block** — observability for the precheck stopping rule:',
  ...
  '  - `precheck.outcome`: `converged` (coverage hit 0.90 healthily), `hit-cap` (4-iteration cap reached without convergence), `never-attempted` (precheck directive not followed — flag), `abandoned-after-error` (precheck was attempted but you gave up after one or more failures — flag; the prompt directive expects you to recover honestly, not abandon).',
  ...
  '  - **Compaction fallback**: if `response.compactionEvents > 0`, `toolErrors` MAY be partial because pre-compaction failures may have been summarized away. In that case, include `<partial-due-to-compaction>` as the FIRST entry (literal string in `errorClass`), then enumerate what you can recover.',
  ...
  '- `meaningfulRecallsHaveDifferentInputs` distinguishes healthy iteration from transport thrash:',
  ...
  '- **`gatesElided` block** — structured as `{tool, rationale}` per elided gate (e.g., `[{tool: search_radar, rationale: target not technology-investment-themed}]`) so operators can audit gate-skip reasoning.',
  '- **`response` block** — catches stream pathologies that invalidate the audit artifact:',
  ...
  '  - **`response.compactionEvents`**: count of HOST-triggered auto-compaction events you can detect during this workflow session. THREE valid states with strict semantics:',
...
// ─── BL-125 — run parameters the model must COPY, never infer ─────────
//
// Until BL-125 no builder stated its own resolved `mode` / `auditLevel` /
// `transactionContext`. The model had to infer them from which sections
// rendered, and in three production runs out of three it reported `enhanced`
// — including one the operator invoked at `debug`. It then passed `enhanced`
// to `compose_dossier_envelope`, which withheld `metaFenceMarkdown` exactly as
// contracted, so `promptVersion` came back null. `auditLevel: debug` was
// unreachable through the model even when the server rendered it correctly.
//
...
// lives in one place instead of three that drift.
...
/** BL-125 — the verbatim-body gate, stated only where a consumer exists. */
function verbatimGateBullet(requireVerbatimBody?: boolean): string {
  return requireVerbatimBody === true
    ? '- Verbatim-body gate: **true**. Pass `requireVerbatimBody: true` to `compose_dossier_envelope`. The server then REFUSES any run whose `irlSource` is not a `partner-paste-verbatim` variant — you cannot self-degrade to a reconstruction under this flag.'
    : '- Verbatim-body gate: **false** (the default — drafting and exploration, where the gap list disclosing a reconstruction is enough). Omit `requireVerbatimBody` from the `compose_dossier_envelope` call rather than passing `false`.';
}
...
function buildOneShotBody(args: {
  ...
  return [
    ...
    '```',
    '',
    '## Sweep plan — execute the steps in order; a step whose inclusion gate fails is elided per § Tool inclusion gates, not skipped silently',
    ...
    '',
    `Step 1 — Extract the 13 diligence dimensions from the IRL, then invoke \`generate_diligence_agenda\` with the dimension values AND the \`_audit\` sibling (optional, but validated when present — send it) that carries per-dimension provenance + calibration metadata. ${UNKNOWN_PROPAGATION_RULE}`,
    '',
    `**Step 1a — Schema-enforced audit shape (send it — optional, but a wrong shape is rejected).** The \`generate_diligence_agenda\` tool's input schema takes a sibling \`_audit\` field next to the 13 dimensions. Each dimension's audit entry carries \`tier\` (1/2/3) + \`citation\` (in the form "Section NN — <substantial excerpt>") plus dimension-specific calibration fields. Build the \`_audit\` sibling from the calibration rules in Step 1b below; if the shape or any calibration field is wrong the tool returns \`isError: true\` with the BL-045 rule citation naming exactly what to fix — read it and retry.`,
    '',
    ...
    '',
    '0. **Tier-discipline universal rule**: `tier` and `value` are coupled across every dimension, and the tool rejects a mismatched pair — this is the most common cause of a `generate_diligence_agenda` retry:',
    '',
    ...
    '',
    `**Step 4a — TechPar audit shape (send \`_audit\` — it is optional, but validated when present, and a wrong shape is rejected).** The \`_audit\` schema enforces TWO things for compute_techpar: (1) a SINGLE declared currency basis for all monetary inputs, and (2) PER-FIELD annualization provenance, so the same IRL annualizes the same way on every run. Follow the Critical anti-fabrication rules below to shape \`_audit\`; on a malformed sibling the tool rejects with a structured BL-045 diagnostic naming the failed field and fix — read it and retry.`,
    '',
    ...
    '',
    '2. **YTD annualization REQUIRES `ytdMonths`** — if a monetary field was annualized from a YTD figure (the most common case for IRLs reporting against a partial fiscal year), the audit DEMANDS you declare how many months of YTD actuals you extrapolated. Without it, one IRL can yield R&D% figures that differ severalfold between runs (9.1% / 13.9% / 31.2% on a single fixture). The tool rejects `annualizationSource: "ytd-annualized-with-period"` without `ytdMonths`. To find the YTD period: cross-reference the IRL\'s recurring-revenue monthly figure × YTD months → should equal the YTD total. For example: recurring $2.64M/mo × 3 = $7.92M YTD ≈ $7.86M YTD → ytdMonths = 3.',
    '',
    ...
    '',
    '**Step 6a — MTTR + incident-count fabrication guard (the tool ENFORCES this).** Send the `estimate_tech_debt_cost` tool its `_audit` sibling (optional, but validated when present) with `mttrSource` and `incidentsSource` enum values. For EACH:',
    '',
    ...
    VDR_FOLDER_TAXONOMY,
    '',
    '  **(A) Target snapshot** — a short profile pulled from IRL Section 00 + 01. Quick-look voice — a partner orients on it in one read. Include any open-question flags where the IRL gave non-definitive answers.',
    '',
    ...
    '',
    '  **(C) Architecture + paradigm assessment** — pull from Step 4 (`compute_techpar`). Close with the `compute_techpar` `deeplink` as a single "Open TechPar Wizard" link so the partner can refine inputs in-browser. Frame the body as a concise read on the target\'s technical paradigm, R&D OpEx posture, and biggest architectural-cost drivers visible from the IRL.',
    '',
    '  **(D) Infrastructure cost governance assessment** — pull from Step 5 (`assess_infrastructure_cost_governance`). Close with the `assess_infrastructure_cost_governance` `deeplink` as a single "Open ICG Wizard" link. Body: maturity scores + 3-5 prioritized recommendations, ranked by leverage.',
    '',
    '  **(E) Technical debt assessment** — pull from Step 6 (`estimate_tech_debt_cost`). Close with the `estimate_tech_debt_cost` `deeplink` as a single "Open Tech Debt Calculator" link with sliders pre-positioned to the IRL-derived inputs. Body: annualized carry cost, headline payback projection, and the 1-2 most expensive debt categories the IRL surfaced (e.g., legacy services flagged for rewrite).',
    '',
    '  **(F) Regulatory exposure** — pull from Step 3. Close each framework subsection with the `search_regulations` `deeplink` for that framework as an "Open in Regulatory Map" link (one deeplink per framework — they filter the map to different region+category combinations). Body: one subsection per jurisdiction/framework, quoting the `keyRequirements` bullets verbatim and identifying each framework by name + effective date — the records carry no article numbers. Cross-jurisdictional transfer mechanism review if the IRL Section 09 flags cross-border data flows.',
    '',
    '  **(G) Comparable engagements** — pull from Step 2. Close with the `search_portfolio` `deeplink` as a single "Open Hub: Comparable engagement view" link. Body: for each of the 3-5 matched code-named engagements, write why this one is relevant and the lesson it carries, briefly.',
    '',
    '  **(H) Market signal** — pull from Step 7. Close with the `search_radar` `deeplink` as a single "Open Radar Feed" link filtered to the relevant category. Body: a brief summary of what the radar items reveal about the target\'s market timing.',
    '',
    '  **(I) Synthesis + recommendation** — handoff-memo voice (mirror `gst_diligence_handoff_memo`). A tight integration of the above: what does the dossier collectively recommend? Where are the biggest unanswered questions even after the high-fidelity intake? What 5-7 VDR follow-up documents (using verbatim labels from `' +
      VDR_RESOURCE_URI +
    ...
    '',
    buildEnvelopePrecheckDirective(showRunAudit),
    '',
    buildEnvelopeCompositionDirective(showRunAudit),
...
function extractOnlyProcedureSections(opts: { mintsOwnProvenance: boolean }): string[] {
  return [
    ...
    '',
    `**Extraction step 2 — Dimension extraction worksheet (REQUIRED).** Apply this extraction discipline to derive the 13 \`generate_diligence_agenda\` dimensions: ${UNKNOWN_PROPAGATION_RULE}`,
    '',
    ...
    '',
    'If an inclusion gate fails for a tool (per § Tool inclusion gates above), emit a fence labeled `elided: <tool-name>` with `{ "reason": "<which gate predicate failed>", "irlSection": "<which IRL section would have satisfied it>" }` instead of the payload.',
    '',
    GAP_LIST_DIRECTIVE,
...
function buildExtractOnlyBody(args: {
  ...
  partnerLead?: string;
  projectCodeName?: string;
  // BL-125 — threaded for the run-parameters block ONLY. This builder remains
  // exempt from the audit-level GATE (ADR-0017): every directive below renders
  // at all three levels, and the tests assert that positively. It states the
  // value because its meta fence is model-authored — no envelope call runs
  // here to supply one — so this is the surface where an inferred `auditLevel`
  // goes straight into the artifact unchecked.
  auditLevel: AuditLevel;
}): string {
  const targetClause = args.targetName
    ? `The target is **${args.targetName}**.`
...
function buildInteractiveBody(args: {
  ...
  // them again. Threading them is only half the fix; Step 1 below must also
  // stop asking for what it was already given, or the body states a value at
  // the top and requests it four lines later.
  auditLevel: AuditLevel;
  targetName?: string;
  transactionContext?: (typeof transactionContextValues)[number];
  partnerLead?: string;
...
${ENG_COST_DEDUP_RULE}`,
    ...
    '',
    VDR_FOLDER_TAXONOMY,
    // BL-125 (#6) — this body ignored `enhanced` entirely: it computed only
    // `showRunAudit`, so `standard` and `enhanced` rendered byte-identically.
    // The dossier still carried (K), because the envelope returns
    // `provenanceFooterMarkdown` at `enhanced` and Step 4 says to transcribe
    // whatever it returns — so an interactive run emitted a provenance footer
    // WITHOUT the blocking self-check that backs it, while a paste run at the
    ...
    ...(showRunAudit
      ? [
          ...
          '  outcome: converged | hit-cap | never-attempted | abandoned-after-error',
          '  errorsEncountered: [<{errorClass: string, recoveryAction: string}, ...> — empty list if none]',
          'countersScope: session | run | request  # copy VERBATIM from compose_dossier_envelope output; states how far back toolCallCounts reaches',
          'toolCallCounts:  # copy VERBATIM from compose_dossier_envelope output `serverToolCallCounts`',
          '  validate_irl_provenance: { attempted: N, succeeded: N, rejected: N, errored: N }',
          ...
          'totalEnvelopeCalls: <int — CUMULATIVE across the session>',
          'meaningfulRecallsHaveDifferentInputs: <bool — true | false | null; transport-vs-iteration discriminator; null when selfCorrectionCalls=0>',
          'conditionalTriggers:',
...
export const irlIngestionPrompt: GstPrompt<typeof argsSchema> = {
  ...
  build: (args) => {
    ...
    if (!args.filledIrl) {
      ...
      // buildExtractOnlyBody — but it still STATES the level, because its meta
      // fence is model-authored and nothing else would supply the value.
      bodyText = buildExtractOnlyBody({ ...args, filledIrl: args.filledIrl, auditLevel });
    } else {
