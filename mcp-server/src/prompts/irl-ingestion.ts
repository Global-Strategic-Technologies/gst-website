...
function prepareIrlBodyStep(opts: { bodyBindingHash?: string }): string {
  ...
  return [
    ...
    '',
    'This is the ONLY tool call this mode makes. It computes nothing about the target: it hashes the bytes, seeds the server-side body cache, and returns the canonical `irlBodyHash` plus a `mintedAt` timestamp. Without it the record you are about to emit would travel with no auditable statement of where its bytes came from — and this record is meant to travel.',
    '',
    'From the response:',
    '',
