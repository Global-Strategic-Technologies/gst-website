...
const BL_045_VERIFY_DIRECTIVE = [
  ...
  '  compose_dossier_envelope: { attempted: N, succeeded: N }',
  '  <other tools used>: { attempted: N, succeeded: N }',
  ...
  '- **`toolCallCounts` block (BL-058)** — ground-truth cross-check on self-reported counters above. Operators compare `validate_irl_provenance.succeeded` to `precheck.iterations` and `compose_dossier_envelope.succeeded` to `totalEnvelopeCalls`. Discrepancies surface model self-report unreliability. Include every tool you invoked, even if zero times (omit-if-zero is acceptable for tools you never considered).',
...
const INTERACTIVE_BODY = [
  ...
  '',
  '`toolCallCounts` block (BL-058) reporting discipline: ground-truth cross-check on every counter above. `validate_irl_provenance.succeeded` must equal `precheck.iterations`; `compose_dossier_envelope.succeeded` must equal `totalEnvelopeCalls`. Include every tool you invoked even if zero times for a tool you tried. Tools you never considered may be omitted.',
  '',
