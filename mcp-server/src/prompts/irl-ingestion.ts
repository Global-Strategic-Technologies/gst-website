...
const ENVELOPE_COMPOSITION_DIRECTIVE = [
  ...
  "- `irlSource` — REQUIRED. How the bytes in `filledIrl` were assembled. Pass `partner-paste-verbatim` when the operator pasted the IRL markdown directly into the prompt arg, `model-reconstruction-from-xlsx` when you parsed an xlsx attachment, `model-reconstruction-trimmed` when you assembled markdown without a verbatim source. The server auto-appends a `provenance-gap:` entry to (J) when the value indicates reconstruction — this is intentional and surfaces BL-049's hash-bind tautology in non-partner-paste modes (where you control both `filledIrl` and `irlBodyHash`, so the hash-bind is `pass-internal` only). Match whatever value you will also report in `filledIrl.source` of the BL-045-VERIFY block.",
