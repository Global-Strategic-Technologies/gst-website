...
const ENVELOPE_PRECHECK_DIRECTIVE = [
  ...
  '',
  '   **BL-079 Part B (v0.31.0+) — partner-paste prepop mode**: if a `**Body-binding hash:**` directive appears in this prompt body above (one-shot mode where the operator supplied `filledIrl` as a prompt arg), the server has ALREADY pre-populated the IRL body cache. **SKIP `prepare_irl_body` entirely** — do NOT call it. Instead, pass `{ irlBodyHash, citations }` (NOT `{ filledIrl, citations }`) to every `validate_irl_provenance` iteration, copying the directive value verbatim into `irlBodyHash`. The body never flows through your tool-call output stream — emission per validate call drops from full-body to 16 hex chars. Report `irlSource: partner-paste-verbatim-prepop` (NOT `partner-paste-verbatim`) in your compose call AND in the VERIFY block. Empirical motivation: full-body emission on >10KB bodies hits the output stream ceiling and produces `transport-timeout` errors (observed 2026-06-07 at 51KB: `precheck.outcome: abandoned-after-error`).',
  '',
  '   **Interactive / xlsx-reconstruction mode**: no `**Body-binding hash:**` directive present. Use the legacy `{filledIrl, citations}` shape on validate, and call `prepare_irl_body` before compose as before. Report `irlSource: model-reconstruction-*` per the actual assembly mode.',
...
const ENVELOPE_COMPOSITION_DIRECTIVE = [
  ...
  "- `irlSource` — REQUIRED. How the IRL body bytes were assembled. Pass `partner-paste-verbatim-prepop` when a `**Body-binding hash:**` directive appears in this prompt body (BL-079 Part B prepop path — server pre-populated cache from the operator's prompt arg, you skipped `prepare_irl_body`), `partner-paste-verbatim` when the operator pasted the IRL markdown directly AND you called `prepare_irl_body` to seed the cache (legacy partner-paste), `model-reconstruction-from-xlsx` when you parsed an xlsx attachment, `model-reconstruction-trimmed` when you assembled markdown without a verbatim source. The server auto-appends a `provenance-gap:` entry to (J) when the value indicates reconstruction — this is intentional and surfaces BL-049's hash-bind tautology in non-partner-paste modes (where you control both the body and `irlBodyHash`, so the hash-bind is `pass-internal` only). Match whatever value you will also report in `filledIrl.source` of the BL-045-VERIFY block.",
