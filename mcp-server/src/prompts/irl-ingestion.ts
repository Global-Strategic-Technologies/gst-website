...
const ENVELOPE_PRECHECK_DIRECTIVE = [
  ...
  '',
  '   **BL-079 Part A (v0.30.5+) — body-by-hash on validate, strongly recommended for bodies >10KB**: call `prepare_irl_body({ filledIrl })` ONCE BEFORE this precheck loop begins (you would call it before `compose_dossier_envelope` anyway — just hoist that call earlier). It returns `irlBodyHash`. Then pass `{ irlBodyHash, citations }` to EVERY `validate_irl_provenance` iteration INSTEAD OF `{ filledIrl, citations }`. The server re-hydrates the body from the same cache `compose_dossier_envelope` reads from — citation matching runs against the exact operator-paste bytes with NO model emission per iteration. **Empirical motivation**: on bodies >10KB, full-body emission to `validate_irl_provenance` hits the model output stream ceiling and produces `transport-timeout` errors that abandon the precheck loop (observed 2026-06-07 night at 51KB: `validate_irl_provenance: { attempted: 2, succeeded: 1, errored: 1 }`, `precheck.outcome: abandoned-after-error`). With `irlBodyHash`, each validate call emits only the citations + a 16-hex hash — emission cost is sub-1KB regardless of body size. The schema accepts EITHER `filledIrl` OR `irlBodyHash` OR both (`filledIrl` takes precedence when both supplied); the only hard rule is at least one. Cache miss returns `Bl076BodyCacheMissError` directing you to re-call `prepare_irl_body`.',
  '',
...
const ENVELOPE_COMPOSITION_DIRECTIVE = [
  ...
  '- **BL-076 (v0.17.0+) — DO NOT pass `filledIrl` to `compose_dossier_envelope`**. The IRL body is no longer a public input field on this tool. **Call `prepare_irl_body({ filledIrl: <body> })` FIRST**; it caches the body server-side keyed by the canonical `irlBodyHash` and returns that hash. Then pass ONLY `irlBodyHash` to `compose_dossier_envelope` — the server re-hydrates the body from the cache for internal provenance verification. (Rationale: in prior versions, emitting the full body to `compose_dossier_envelope` as tool args cost the model 5–15 minutes per call in output-token generation time; the cache eliminates that cost.) If you skip `prepare_irl_body` and call `compose_dossier_envelope` first, the server returns `Bl076BodyCacheMissError` directing you to call `prepare_irl_body` first.',
  "- `irlBodyHash` — has two valid sources depending on prompt invocation mode. **One-shot mode** (this body): copy verbatim from the prompt body's `**Body-binding hash:**` directive — that is the partner-authoritative `pass-bound` form. **Interactive mode** (no `filledIrl` arg supplied at invocation): no directive will appear in your prompt body, so compute `sha256(filledIrl).slice(0,16)` of the body bytes you yourself intend to submit and pass THAT (or — preferred under BL-076 — call `prepare_irl_body({filledIrl})` and use the returned `irlBodyHash` verbatim; the server computes it for you, eliminating in-head sha256 risk). The envelope tool verifies `sha256(rehydratedBody).slice(0,16) === irlBodyHash` after cache re-hydrate and rejects on mismatch. In your BL-045-VERIFY block, report `pass-bound` ONLY when the directive existed; report `pass-internal` when you computed (or via prepare_irl_body, the server computed for your model-supplied body) the hash in interactive mode. See § Step 5 — verification harness for the discipline rules.",
  "- `irlSource` — REQUIRED. How the IRL body bytes were assembled. Pass `partner-paste-verbatim` when the operator pasted the IRL markdown directly into the prompt arg, `model-reconstruction-from-xlsx` when you parsed an xlsx attachment, `model-reconstruction-trimmed` when you assembled markdown without a verbatim source. The server auto-appends a `provenance-gap:` entry to (J) when the value indicates reconstruction — this is intentional and surfaces BL-049's hash-bind tautology in non-partner-paste modes (where you control both the body and `irlBodyHash`, so the hash-bind is `pass-internal` only). Match whatever value you will also report in `filledIrl.source` of the BL-045-VERIFY block.",
  '- `requireVerbatimBody` — BL-070 accuracy-critical run gate. If the operator invoked this prompt with `requireVerbatimBody: true`, you MUST pass that value through VERBATIM to `compose_dossier_envelope.requireVerbatimBody`. In that mode the server REFUSES any `irlSource !== "partner-paste-verbatim"`; you cannot self-degrade to xlsx-reconstruction when the flag is set. If the operator did not paste the IRL markdown directly AND set the flag, return a graceful error directing the operator to re-invoke with the markdown pasted into the `filledIrl` prompt arg. If the operator did not set the flag, omit `requireVerbatimBody` from the tool call (defaults to false — drafting / exploration mode).',
  '',
  ...
  '',
  '**BL-071 — `serverToolCallCounts` (server-authoritative)**. The envelope output also returns `serverToolCallCounts` — a server-authoritative snapshot of every tool call in this session (`{ attempted, succeeded, rejected, errored }` per tool). Copy this OBJECT VERBATIM into the BL-045-VERIFY block `toolCallCounts` field. DO NOT self-narrate the counters or estimate from memory — the server counts are the source of truth; model self-narration has demonstrated drift (sonnet fabricated a tool-call that was never made; opus omitted a tool from its self-report; a third run reported the same retry data inconsistently across two YAML surfaces). The server-emitted counts close this drift.',
  '',
  '**Precheck derivation rules (BL-071)** — also derive from `serverToolCallCounts`. Empirically the `precheck` block has drifted from `toolCallCounts` for the SAME event:',
...
const BL_045_VERIFY_DIRECTIVE = [
  ...
  '  source: partner-paste-verbatim | model-reconstruction-from-xlsx | model-reconstruction-trimmed | placeholder',
  '  fingerprint:',
