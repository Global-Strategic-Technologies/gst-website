    ...
    messages: [
      {
        ...
        content: {
          ...
          text: [
            ...
            '',
            'Step 4. For EACH selected comparable, write one paragraph covering:',
            ...
            '',
            'Step 5. Close with a short "what this means for the current deal" framing that synthesizes the lessons across the shortlist into a directional view.',
            '',
            'Step 6. Append an "Open in Hub" footer that lists every `deeplink` URL returned by the `search_portfolio` calls (one per filter combination explored). Each link should be labelled by the filter it represents — e.g., `Open in Hub: Healthcare / Buy-Side · Logistics / Buy-Side`. The deeplink opens `/ma-portfolio` with the same filter chips pre-active so the deal team lands on the exact filtered view the memo references. If no deeplink is present in a tool response, omit that link — never invent a URL.',
            '',
