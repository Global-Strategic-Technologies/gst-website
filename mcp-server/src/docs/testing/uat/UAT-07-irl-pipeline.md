...
| `filledIrl.bytes` ≠ the source file's size                                            | The paste was altered in transit — see the note above                        | Small drift: record it and continue. Large drift: **Fail**, the dossier is bound to bytes that are not the partner's  |
