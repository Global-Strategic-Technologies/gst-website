...
2. **Public endpoints (no auth)** — `GET /health` returns the JSON health payload (`src/observability/health.ts`: Upstash probe + cached Inoreader status + `GIT_SHA` build provenance); `GET /status` renders a public HTML status page over the same payload plus the alert evaluator's last-run summary, edge-cached 60s (`src/observability/status-page.ts`); `GET /server.json` (alias `GET /.well-known/mcp`) returns the registry document (`src/registry-metadata.ts`, BL-152: the official-registry `server.json` shape, version from `env.VERSION`, description mirrored from the website JSON-LD and test-bound in both directions; dispatched before the OAuth branch so the provider's `/.well-known/` catch-all does not swallow the alias). Health deliberately never calls Inoreader — it reads a 5-minute-TTL Upstash status cache written by real radar calls, so uptime monitors can't burn API budget.
...
4. **Admin re-auth surface** — `/admin/inoreader/reauth/*` (operator-driven Inoreader OAuth recovery) is gated by the separate `MCP_ADMIN_KEY`, not team bearers.
   4b. **Self-serve trial mint** — `POST /trial/signup` (BL-155 Slice 2, `src/trial/signup.ts`) is public by design: a stranger has no bearer. Its gate is inside the handler — an IP-keyed limiter, a hardened Turnstile siteverify (hostname + action asserted, explicit timeout), and a one-trial-per-identity lease on `mcp:trial:ident:<hmac-of-ip>` — and **every dependency fails closed**: unbound Upstash/KV/secrets, a Redis throw, or an unreachable Turnstile is a 503 with nothing minted, the inverse of the Worker's usual fail-open substrate. A repeat signup inside the 30-day identity window rotates the existing record's secret rather than minting a second client. The response is CORS-wrapped with the env-aware wrapper (below). Production stays dark (503) until its secrets are set, which is gated on Slice 3.
...
**Staging-only extra origins (BL-155 Slice 2).** The trial signup form is exercised from Vercel previews and `localhost` against staging before it ships, and those origins must never join the production set. `corsHeadersForEnv` adds a `TRIAL_EXTRA_ORIGINS` list (`[env.staging.vars]`) when — and only when — `ENV_NAME` is `staging`: exact origins, or one-label preview-suffix patterns written `https://*-<team>.vercel.app`; a bare `*.vercel.app` (any Vercel-hosted site) is rejected at parse and logged. Only the preflight branch and the trial branch use the env-aware wrapper; every authenticated path keeps `corsHeadersFor` / `withCors` unchanged, so this cannot widen what an MCP client can read. Verification method: `tests/unit/auth/cors-env.test.ts` pins the production set byte-identical in every env and the look-alike refusals.
...
| ------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | --- | --------- | ----------------------------------------------------------- |
| `blob1` | `event_type` (discriminator: `tool_invocation`, `resource_read`, `prompt_invocation`, `prompt_span`, `rate_limit_decision`, `inoreader_call`, `health_check`, `cron_outcome`, `audit_batch`, `trial_signup`, `tier_denial`, `scope_denial`, `oauth_consent`, + BL-045 counter events) |     | `blob6`   | `status_code` (string)                                      |
| `blob2` | `name` (tool/prompt/cron slug/egress category)                                                                                                                                                                                                                                        |     | `blob7`   | `zone1` (`'1'`/`'0'`, `inoreader_call` only)                |
| `blob3` | `keyOwner` (PII-free `MCP_KEY_*` suffix)                                                                                                                                                                                                                                              |     | `blob8`   | `client_ref` (`OAUTH:<clientId>`; per-client, BL-155)       |
| `blob4` | `outcome` (`success`/`error`/per-type enum)                                                                                                                                                                                                                                           |     | `double1` | `duration_ms`                                               |
| `blob5` | `correlation_id` (`prompt_span` only)                                                                                                                                                                                                                                                 |     | `double2` | `seq` (prompt-span step index)                              |
|         |                                                                                                                                                                                                                                                                                       |     | `index1`  | `keyOwner` mirror (AE sampling key; `__none__` when absent) |

`blob8` is the per-client dimension and is deliberately **not** the index: `keyOwner` is constant per tier to keep `index1` roster-sized, and AE allows one index per data point. Distinct actives is `count(DISTINCT blob8)`, which has **no sample correction** — the corrected `count` is the plain row counter, not this — so it is exact while unsampled, a lower bound once not. Per-client volume (`GROUP BY blob8` + `sum(_sample_interval)`) is sample-correct. See [ADR-0031](../../../src/docs/adr/0031-per-client-analytics-identity-is-a-blob.md).
