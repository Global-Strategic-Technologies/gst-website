...
Every Inoreader call site (cron, live tool, or the `/radar/snapshot` SSR endpoint) routes failures through `src/lib/inoreader-failure-handler.ts`, which opens the breaker in `src/ratelimit/circuit-breaker.ts` on `inoreader-rate-limit` signals and tags a Sentry message with the zone-diagnostic headers; while open, radar reads serve cached data and skip upstream calls until the cool-down elapses.
