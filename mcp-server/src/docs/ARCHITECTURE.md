...
MCP-over-HTTP is served by Cloudflare's `agents` SDK (`agents@^0.20.1`): `createMcpHandler` from `agents/mcp` adapts an `McpServer` instance to the Worker's Web `Request`/`Response` runtime, serving JSON-RPC at `/mcp`. The handler matches that path **exactly** and 404s anything else — including sub-paths. (`worker.ts`'s `isRoutedPath` still prefix-matches `/mcp/`, which is harmless — such requests reach the handler and get its 404 instead of ours — but the "sub-paths for session resume" rationale in its comment is obsolete: protocol-level sessions no longer exist, and neither handler generation has ever served a sub-path.)

The `agents` SDK spans two MCP SDK generations, and **we are deliberately on the older one**: `@modelcontextprotocol/sdk@^1.29.0` (the v1 single-package family), whose `WebStandardStreamableHTTPServerTransport` is the Workers-compatible transport and which caps at protocol revision `2025-11-25`. `agents@0.20.1` _also_ resolves `@modelcontextprotocol/{core,client,server}@2.0.0` — the split-package generation that speaks `2026-07-28` — and passing a v1 `McpServer` to `createMcpHandler` now emits a deprecation warning on every request, with removal scheduled for the SDK's next major. Migrating to the v2 server factory, and the full gap against `2026-07-28`, is [BL-106](../../../src/docs/development/MCP_SERVER_SPEC_2026_07_28_ALIGNMENT_BL-106.md).

`nodejs_compat` in `mcp-server/wrangler.toml` exists solely for the `agents` SDK's transitive Node deps (`mimetext`, `mime-types`); our own Worker code path stays Web-API-only.
