...
2. A browser tab opens the Worker's consent page. **Paste your `MCP_KEY_*` value** (from your password manager) into the key field and click Approve — that one paste is how the grant knows who you are; your delegated access is bounded by your key's scopes. (Since BL-155 the same field also accepts an M2M client credential as `<clientId>:<secret>` — that is how a self-serve trial or an external pilot connects; their access is bounded by the record's scopes and tier.)
