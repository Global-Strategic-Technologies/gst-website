...
**When to call this tool**: a partner holds evidence but no filled IRL — pre-LOI screening, a data room opened before the IRL went out, sell-side prep, or topping up coverage from filings. Pair with the \`gst_irl_fill\` prompt, which walks the model through authoring \`fills\` under the sourcing grammar.
