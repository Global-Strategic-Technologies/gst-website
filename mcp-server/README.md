...
| `search_radar_offline`                  | Snapshot-only Radar search (mirrors the /hub/radar website). Never makes live Inoreader calls. **Stdio-only** — see BL-032 Q12. Renamed from `search_radar_offline` in BL-032 Phase 4b; deprecated alias retained for one release ([BREAKING_CHANGES.md](BREAKING_CHANGES.md)). | `{ category? }` — see [`src/docs/radar/CONTRACT.md`](src/docs/radar/CONTRACT.md). BL-031.95 Phase 3.A refactor stripped `query`/`tier`/`since`/`limit` to mirror the website's single-filter surface; cache TTL is 24h. |
