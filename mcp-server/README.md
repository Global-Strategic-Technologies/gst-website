...
| **`search_radar`** _(BL-032 Phase 4c)_                  | **Live** Radar search — calls Inoreader with a 6h Upstash cache. Sister to `search_radar_offline` (same shape; different source). Per-key 5/min, 50/day budget; circuit breaker opens on Inoreader 429.                                                                                                                                                                                                                                                                                                                                   | **both**       | `{ category? }` — see [`src/docs/tools/radar/CONTRACT.md` § Live tool surface](src/docs/tools/radar/CONTRACT.md#live-tool-surface-bl-032-phase-4c)                                                                                                                                    |
...
| **`generate_information_request_list_xlsx`** _(BL-044)_ | Generates the canonical GST Information Request List as a downloadable fillable `.xlsx`. Pure-function pipeline: library load → markdown parse → customize (section filter + custom requests) → XLSX render → base64. Returns `{ filename, base64, mimeType, byteLength, sectionCount, bulletCount, canonicalUrl }`. Reads the same `gst://library/information-request-list` Resource the prompt embeds; with no configuration args the file stays byte-identical to the printed article, and configuration args scope it per engagement. | **both**       | `{ targetName?, transactionContext?, productSummary?, companyName?, projectName?, includeSections?, customRequests?, showCanonicalReference? }` (all optional; first three mirror the `gst_information_request_list` prompt args, the rest are the 2026-07 configurability follow-up) |
...
| `gst_information_request_list`    | `{ targetName?, companyName?, projectName?, transactionContext?, includeSections?, customRequests?, showCanonicalReference?, productSummary? }` (all optional; wire-string args coerced via `arrayFromWire` / `booleanFromWire`) | `gst://library/information-request-list` + `generate_information_request_list_xlsx` _(BL-044)_                                                                                                                                                                                                                     | Paste-ready one-page intake checklist (VDR-9 taxonomy + Basics prelude), **configurable to full parity with the Hub generator** (company/project title, section pick-list, custom per-section requests, canonical-row toggle). When called with args, **also emits a downloadable fillable `.xlsx`** via the orchestrated tool (forwarding the exact configuration). Pair with `gst_diligence_kickoff` once filled.                                        |
