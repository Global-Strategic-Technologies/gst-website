...
| `gst_information_request_list`    | `{ targetName?, transactionContext?, productSummary? }` (all optional)               | `gst://library/information-request-list`                                         | Paste-ready one-page intake checklist (VDR-9 taxonomy + Basics prelude). Pair with `gst_diligence_kickoff` once filled. |
