...
| `search_radar_cache`                    | Snapshot-only Radar search (mirrors the /hub/radar website). Never makes live Inoreader calls. | `{ category? }` — see [`src/docs/radar/CONTRACT.md`](src/docs/radar/CONTRACT.md). BL-031.95 Phase 3.A refactor stripped `query`/`tier`/`since`/`limit` to mirror the website's single-filter surface; cache TTL is 24h. |
...
> - **Phase 3 — Radar.** Capability-mirror refactor: dropped `query` / `tier` / `since` / `limit` from `search_radar_cache` to mirror the website's single category-pill filter (the cache itself has a 24h TTL, so a `since` filter would have been redundant against the website UX). `gst_radar_brief_today` lost its `sinceHours` argument in v0.0.2. URL state added to `/hub/radar` page via `src/utils/radar-url.ts`; MCP wrapper emits `deeplink`. Authored `src/docs/radar/CONTRACT.md` + `src/docs/radar/USAGE.md`.
