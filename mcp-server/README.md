...
| `search_portfolio`                                      | Wraps `filterProjects` — searches the anonymized M&A dataset                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 | both           | `{ search?, theme? = 'all', engagement? = 'all' }` (no `limit` — removed in BL-031.95 under the capability-mirror invariant; `theme` / `engagement` also accept arrays per BL-064) — see [`src/docs/tools/portfolio/CONTRACT.md`](src/docs/tools/portfolio/CONTRACT.md) |
...
| `search_regulations`                                    | Faceted search across the 123-framework Regulatory Map; returns the resolved Resource URI                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    | both           | `{ jurisdiction?, category?, query?, limit? = 20 }` — see [`src/docs/tools/regulatory-map/CONTRACT.md`](src/docs/tools/regulatory-map/CONTRACT.md)                                                                                                                      |
...
| `gst_radar_brief_today`           | `{ category? }`                                                                                                                                                                                                                                                                                      | `gst://radar/fyi/latest`                                                                                                                                                                                                                                                                     | Daily / pre-meeting digest of recent annotated FYI items in the GST Take voice.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               |
...
| `gst_irl_ingestion`               | `{ filledIrl?, targetName?, transactionContext?, partnerLead?, projectCodeName?, mode?, auditLevel?, requireVerbatimBody? }` (all optional; `filledIrl` is first so it leads the slash-command form)                                                                                                 | `generate_diligence_agenda` + `compute_techpar` + `assess_infrastructure_cost_governance` + `estimate_tech_debt_cost` + `search_regulations` + `search_radar` + `search_portfolio` + `list_portfolio_facets` + `list_regulation_facets` + `gst://irl/source` + `gst://library/vdr-structure` | One-IRL → full diligence dossier (scenario-neutral: buy-side / sell-side / value-creation / unknown). Parallel fan-out across 9 tools behind explicit inclusion gates, with the canonical IRL taxonomy embedded and the VDR folder taxonomy inlined; emits an auditable dossier with meta JSON fence, per-section audit fences, (J) gap list, (K) provenance footer, and Open-in-Hub deeplinks. Supports `extract-only` mode (JSON payloads only, no synthesis). **`auditLevel`** controls how much audit apparatus the dossier carries: `standard` (default) is a clean partner-facing document, `enhanced` adds the (K) provenance footer and per-section audit fences, `debug` adds the run-audit block and the meta JSON fence. Provenance verification runs at every level — the level governs only what is shown, so signoff runs use `debug`. **`requireVerbatimBody`** refuses any run whose IRL body was not pasted verbatim. A body whose line breaks were destroyed in transit (single-line client input fields collapse a multi-line paste) is refused outright, and `irlSource` is capped against a server-held provenance record rather than taken as asserted. |
...
> `stage` takes a **canonical funding-stage** value (`seed` | `series-a` | `series-b` | `series-c` | `pe` | `enterprise`) — see [ADR-0001](../src/docs/adr/0001-stage-taxonomy-adapter.md). This example previously showed `"Scaling Growth"`, which is a portfolio **growth-stage** label and is rejected by the schema. `arr` is quoted because prompt arguments arrive as wire strings and are coerced.
...
Returns `{ matches: Project[], totalMatched: number, returned: number }` — the `"platform"` query is deliberately broad and matches a large share of the dataset; tighten with `theme` or `engagement` (`"Buy-Side"` / `"Sell-Side"`) to narrow. There is no `limit` argument: BL-031.95 removed it under the capability-mirror invariant, because the website renders all matches.
...
> - `search_portfolio { search: "platform", limit: 3 }`: returned `totalMatched: 42, returned: 3` — top three matches **Voss** (Cross-Border Payments, Sell-Side, $156M ARR), **Ecological Eagle** (Government Affairs, Buy-Side, $74M ARR), **Atlas** (Healthcare RCM, Buy-Side, $67M ARR). _(Recorded against the April 2026 surface and left as observed. `limit` no longer exists — BL-031.95 removed it — and the dataset has grown since, so do not copy this call or its count.)_
...
2. Run `search_portfolio` with `{ "search": "platform" }`. Compare to `https://globalstrategic.tech/ma-portfolio` with "platform" in the search box — the match set and its order should align. (`limit` was removed in BL-031.95; the tool mirrors the page's render-all behaviour, so both sides return the same full set.)
...
2. `node build.mjs` — esbuild bundles `src/index.ts` into a single `dist/index.js`. The MCP SDK and zod stay external (resolved from `node_modules` at runtime); everything else is inlined, including `projects.json`.
