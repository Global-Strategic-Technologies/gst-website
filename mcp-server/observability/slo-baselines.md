...
| tool/resource/prompt latency | latency p95               | no traffic   | **deferred**                         | zero client events in window (intermittent team usage — quiet weeks); calibrate from the BL-033 synthetic latency probe (`latency-probe.yml`, US-region baseline + per-region runs) |
