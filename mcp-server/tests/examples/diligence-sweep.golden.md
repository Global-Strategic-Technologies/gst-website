...
**2. TechPar engCost / infraPersonnel double-count.** Same live exercise: when the IRL lists engineering FTEs as `"58 total — 38 product + 8 SRE + 3 security + 7 data + 2 platform DX"`, the model populated `engCost` with 58 × salary AND `infraPersonnel` with 8 × salary — counting the 8 SRE engineers twice (once in R&D OpEx via deepdive's `engCost + prodCost + toolingCost` synthesis, once in the standalone `infraPersonnel` field). This inflated total tech/ARR from ~91.5% to ~95.6% — ~4 percentage points of phantom cost. The central thesis (hosting at ~55% of ARR, critical zone) survives the correction, but specific quoted numbers like "$1.32B exit drag" are sensitive to this error. v0.0.3 fix: add explicit deduplication guidance to Step 4 — "`engCost` is the cost of the R&D engineering headcount that is NOT also booked as infra personnel. If IRL Section 02 lists, e.g., 58 total including 8 SRE, then `infraPersonnel` covers the 8 SRE and `engCost` covers the remaining 50 × avg-salary."

Bundling both fixes into one v0.0.3 patch on the same redeploy cycle is the lowest-friction shape. Manifest hash rolls, semver patch bump, no surface-area change to clients.
