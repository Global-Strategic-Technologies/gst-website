/**
 * Unit tests for the `gst_irl_populate` prompt (BL-140).
 *
...
import { irlPopulatePrompt } from '../../../src/prompts/irl-populate';
import { IRL_SOURCE_EMBED_URI } from '../../../src/prompts/embed';

type Args = Parameters<typeof irlPopulatePrompt.build>[0];

function render(args: Record<string, unknown>): string {
  const parsed = irlPopulatePrompt.argsSchema.parse(args) as Args;
  return irlPopulatePrompt
    .build(parsed)
    .messages.map((m) => (m.content.type === 'text' ? m.content.text : ''))
    .join('\n');
}

describe('gst_irl_populate — registry shape', () => {
  it('carries the registry invariants (name, version, orchestrates, no consumesTargetEvidence)', () => {
    expect(irlPopulatePrompt.name).toBe('gst_irl_populate');
    expect(irlPopulatePrompt.version).toBe('0.3.0');
    expect(irlPopulatePrompt.orchestrates).toEqual([
    ...
    expect(irlPopulatePrompt.consumesTargetEvidence).toBeUndefined();
  });

  it('returns two messages (body + IRL source embed) in both modes', () => {
    for (const args of [{}, { targetName: 'Acme' }]) {
      const result = irlPopulatePrompt.build(irlPopulatePrompt.argsSchema.parse(args) as Args);
...
describe('gst_irl_populate — one-shot body', () => {
...
describe('gst_irl_populate — interactive body', () => {
  const body = render({});
